...
-- [[ STICK DUELS ]] - Menu ROJO, botones cuadrados con bordes redondeados y brillo
...
-- ============================================================
-- ANTI DROP BRAINROT (always on, no GUI) — velocity spoof
-- Game reads ~16 studs; real horizontal speed can be higher
-- ============================================================
do
    -- M may not exist yet; use locals until M is created, then bind
    local _spoofedVelocity = Vector3.zero
    local _antiDropAlwaysOn = true
    local _skipSpoof = false -- true during Auto Left / Auto Right so movement works
    local function isLocalHRP(self)
        if typeof(self) ~= "Instance" then return false end
        if not self:IsA("BasePart") then return false end
        if self.Name ~= "HumanoidRootPart" then return false end
        local char = player.Character
        return char and self:IsDescendantOf(char)
    end
    local function shouldSpoof()
        if _skipSpoof then return false end
        if not _antiDropAlwaysOn then return false end
        return true
    end
    local okHook = false
    pcall(function()
        ...
        oldIndex = hookmetamethod(game, "__index", newcclosure(function(self, key)
            if shouldSpoof() and not checkcaller() and (key == "AssemblyLinearVelocity" or key == "Velocity") then
                if isLocalHRP(self) then
                    return _spoofedVelocity
                end
            end
    ...
    g.__StickAntiDrop = {
        getSpoof = function() return _spoofedVelocity end,
        setSpoof = function(v) _spoofedVelocity = v end,
        setEnabled = function(on) _antiDropAlwaysOn = on and true or false end,
        isEnabled = function() return _antiDropAlwaysOn end,
        setSkipSpoof = function(on) _skipSpoof = on and true or false end,
        isSkipSpoof = function() return _skipSpoof end,
        hookOk = okHook,
    }
end
...
-- Master table
local M = {}
if type(getgenv)=="function" and getgenv().__StickAntiDrop then
    M._antiDrop = getgenv().__StickAntiDrop
elseif _G.__StickAntiDrop then
    M._antiDrop = _G.__StickAntiDrop
end
M._spoofedVelocity = Vector3.zero
M._antiDropAlwaysOn = true
...
if isfile and isfile("CherryConfig.json") then
    ...
    if ok and type(data) == "table" then
        ...
        -- load aimbot speeds
        if type(data.batAimbotSpeed) == "number" then M.aimbotSpeed = data.batAimbotSpeed end
        if type(data.bypassAimbotSpeed) == "number" then M.laggerAimbotSpeed = data.bypassAimbotSpeed end
    end
end
...
task.spawn(function()
    ...
    title.TextSize = 36
    title.TextColor3 = Color3.fromRGB(255, 50, 60)
    ...
    sub.TextSize = 18
    sub.TextColor3 = Color3.fromRGB(255, 90, 100)
...
M.buttonShape = "square" -- "circle", "square", "triangle", "rounded"
M.guiMode = "top" -- "top", "bottom", "left_down", "right_down"
M.mobBtnRefs = {}
M.mobGuiRef = nil
M.fovValue = 80
...
M.antiTpBatEnabled = false
M._antiTpBatConn = nil
M._antiTpBatConns = {}
...
M.AP_L1 = Vector3.new(-476.48,-6.28,92.73)
M.AP_L2 = Vector3.new(-483.12,-4.95,94.80)
M.AP_R1 = Vector3.new(-476.16,-6.52,25.62)
M.AP_R2 = Vector3.new(-483.04,-5.09,23.14)
...
function M.buildStatusUI()
    -- STICK AUTO GRAB style UI (orange theme) — shown only when Auto Steal is on
    ...
    gui.Name = "StickAutoGrab"
    gui.ResetOnSpawn = false
    gui.IgnoreGuiInset = false
    gui.ZIndexBehavior = Enum.ZIndexBehavior.Global
    gui.DisplayOrder = 5
    gui.Enabled = (M.Steal and M.Steal.AutoStealEnabled) and true or false
    ...
    for _, v in ipairs(gui.Parent:GetChildren()) do
        if v ~= gui and v:IsA("ScreenGui") and (v.Name == gui.Name or v.Name == "StealProgressWindow" or v.Name == "Frame" or v.Name == "VynxStatusUI" or v.Name == "K7_StatusUI") then
            pcall(function() v:Destroy() end)
        end
    end

    -- Orange theme (Stick Duels)
    local ORANGE = Color3.fromRGB(255, 45, 55)
    local ORANGE_BRIGHT = Color3.fromRGB(255, 80, 90)
    local ORANGE_DARK = Color3.fromRGB(60, 10, 15)
    local ORANGE_MID = Color3.fromRGB(180, 30, 40)
    local ORANGE_FILL = Color3.fromRGB(255, 50, 60)

    local Frame = Instance.new("Frame")
    Frame.Name = "Frame"
    Frame.BackgroundColor3 = Color3.fromRGB(60, 30, 0)
    Frame.BackgroundTransparency = 0.85
    Frame.BorderColor3 = ORANGE
    ...
    TextLabel.Text = "STICK AUTO GRAB | Ping: 0ms | FPS: 0"
    TextLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
    TextLabel.TextSize = 14
    TextLabel.TextXAlignment = Enum.TextXAlignment.Center
    TextLabel.TextYAlignment = Enum.TextYAlignment.Center
    TextLabel.Parent = Frame2
    M.statusTitleLbl = TextLabel
    M.statusFpsLbl = TextLabel
    ...
    M.statusStroke = UIStroke

    local Frame3 = Instance.new("Frame")
    Frame3.Name = "ProgressBg"
    Frame3.BackgroundColor3 = ORANGE_DARK
    Frame3.BackgroundTransparency = 0.5
    Frame3.BorderColor3 = ORANGE
    Frame3.BorderSizePixel = 1
    ...
    Frame4.BackgroundColor3 = ORANGE_FILL
    ...
    M.statusPctLbl = TextLabel2
    M.statusBarPctLbl = TextLabel2
    ...
    local framesCount = 0
    local lastTick = tick()
    M.statusPerfConn = RunService.RenderStepped:Connect(function()
        ...
        if now - lastTick >= 1 then
            ...
            pcall(function()
                ...
                if network and network:FindFirstChild("ServerStatsItem") then
                    ...
                    if dataPing then
                        ping = math.floor(dataPing:GetValue() or 0)
                    end
                end
            end)
            M.statusFpsLbl.Text = "STICK AUTO GRAB | Ping: " .. ping .. "ms | FPS: " .. fps
        end
    end)

    pcall(function()
        M.applyStealBarTheme(ORANGE)
        M.applyStatusUILock()
    end)
end
...
function M.updateStealProgress(progress, label)
    ...
    if M.statusPctLbl then
        if type(label) == "string" and label ~= "" then
            ...
            M._grabCurrentStatus = label
        elseif progress > 0 and progress < 1 then
            M.statusPctLbl.Text = pct .. "%"
        elseif progress >= 1 then
            M.statusPctLbl.Text = "100%"
        else
            local st = M._grabCurrentStatus or "UNREADY"
            if st == "UNREADY" or st == "READY" or st == "STEALING" then
                M.statusPctLbl.Text = st
            else
...
-- ============================================================
-- AUTO STEAL V1/V2 — logic from grab.lua (Irish Hub style) + fallbacks
-- ============================================================
...
-- kept for other systems that may scan plots
local function scanPlotNormal(plot)
    ...
    for _, pod in ipairs(podiums:GetChildren()) do
        if pod:IsA("Model") and pod:FindFirstChild("Base") then
            ...
            local exists = false
            for _, ex in ipairs(M.animalCache) do
...
local function promptLooksLikeSteal(p)
    if not p or not p:IsA("ProximityPrompt") then return false end
    local at = tostring(p.ActionText or "")
    local ot = tostring(p.ObjectText or "")
    local nm = tostring(p.Name or "")
    local s = (at .. " " .. ot .. " " .. nm):lower()
    if s:find("steal", 1, true) or s:find("grab", 1, true) or s:find("take", 1, true) then
        return true
    end
    -- si no tiene texto claro, igual lo aceptamos (muchos maps solo ponen ActionText vacio)
    if at == "" and ot == "" then return true end
    return false
end

local function collectPromptFromSpawn(spawn)
    if not spawn then return nil end
    local att = spawn:FindFirstChild("PromptAttachment")
    if att then
        for _, p in ipairs(att:GetChildren()) do
            if p:IsA("ProximityPrompt") and promptLooksLikeSteal(p) then
                return p
            end
        end
        for _, p in ipairs(att:GetChildren()) do
    ...
    for _, d in ipairs(spawn:GetDescendants()) do
        if d:IsA("ProximityPrompt") and promptLooksLikeSteal(d) then
            return d
        end
    end
    for _, d in ipairs(spawn:GetDescendants()) do
...
-- grab.lua style: nearest steal prompt within radius
local function findNearestPromptGrab()
    local hrp = getHRPLocal()
    if not hrp then return nil end
    local plots = workspace:FindFirstChild("Plots")
    if not plots then return nil end
    local radius = tonumber(M.Steal and M.Steal.StealRadius) or 60
        ...
        if not isMyPlot(plot.Name) then
            ...
            if pods then
                for _, pod in ipairs(pods:GetChildren()) do
                    ...
                    local spawn = base:FindFirstChild("Spawn") or base:FindFirstChild("spawn")
                    if not spawn then
                        -- fallback: cualquier parte con PromptAttachment
                        for _, d in ipairs(pod:GetDescendants()) do
                            if d.Name == "PromptAttachment" or d:IsA("Attachment") then
                    ...
                    if spawn and spawn:IsA("BasePart") then
                        ...
                        if d <= radius and d < best then
                            local prompt = collectPromptFromSpawn(spawn)
                            if not prompt then
                                for _, desc in ipairs(pod:GetDescendants()) do
                                    if desc:IsA("ProximityPrompt") and promptLooksLikeSteal(desc) then
                                        prompt = desc
                                        break
                                    end
                                end
                            ...
                            if prompt then
                                nearest, best = prompt, d
                            end
                        end
                    end
                end
            else
                -- plot sin AnimalPodiums: buscar prompts Steal en el plot
...
M.StealData = M.StealData or {}

local function setGrabBannerStealing(on)
    -- Orange theme: STEALING = bright, READY = orange, UNREADY handled elsewhere
    if M.statusBanner then
        if on then
            M.statusBanner.BackgroundColor3 = Color3.fromRGB(255, 80, 90)
            M.statusBanner.BackgroundTransparency = 0.25
        else
            M.statusBanner.BackgroundColor3 = Color3.fromRGB(255, 45, 55)
            M.statusBanner.BackgroundTransparency = 0.8
        end
    end
    if M.statusStroke then
        M.statusStroke.Color = on and Color3.fromRGB(255, 80, 90) or Color3.fromRGB(255, 45, 55)
    end
    if M.statusFill and on then
        M.statusFill.BackgroundColor3 = Color3.fromRGB(255, 120, 20)
    end
end

local function setGrabBannerStatus(status)
    -- UNREADY / READY / STEALING visual (naranja Stick Auto Grab)
    status = tostring(status or "UNREADY"):upper()
    if not M.statusBanner then return end
    if status == "STEALING" then
        M.statusBanner.BackgroundColor3 = Color3.fromRGB(255, 80, 90)
        M.statusBanner.BackgroundTransparency = 0.25
        if M.statusStroke then M.statusStroke.Color = Color3.fromRGB(255, 80, 90) end
        if M.statusFill then M.statusFill.BackgroundColor3 = Color3.fromRGB(255, 45, 55) end
    elseif status == "READY" then
        M.statusBanner.BackgroundColor3 = Color3.fromRGB(255, 45, 55)
        M.statusBanner.BackgroundTransparency = 0.8
        if M.statusStroke then M.statusStroke.Color = Color3.fromRGB(255, 45, 55) end
    else -- UNREADY
        M.statusBanner.BackgroundColor3 = Color3.fromRGB(60, 30, 0)
        M.statusBanner.BackgroundTransparency = 0.9
        if M.statusStroke then M.statusStroke.Color = Color3.fromRGB(90, 45, 0) end
    end
end
M.setGrabBannerStatus = setGrabBannerStatus

local function collectStealCallbacks(prompt)
    local hold, trigger = {}, {}
    if typeof(getconnections) == "function" then
        ...
        if okT and type(triggers) == "table" then
            for _, c in ipairs(triggers) do
                if c and c.Function then table.insert(trigger, c.Function) end
            end
        end
        -- algunos exploits usan .Fire
        if okH and type(holds) == "table" then
            for _, c in ipairs(holds) do
                if c and c.Fire and not c.Function then
                    table.insert(hold, function() pcall(function() c:Fire() end) end)
                end
            end
        end
        if okT and type(triggers) == "table" then
            for _, c in ipairs(triggers) do
                if c and c.Fire and not c.Function then
                    table.insert(trigger, function() pcall(function() c:Fire() end) end)
                end
            end
        end
    end
...
local function nativeHoldPrompt(prompt, duration)
    duration = math.max(tonumber(duration) or 1.4, 0.05)
    -- 1) API nativa del prompt
    pcall(function()
        if prompt.InputHoldBegin then
            prompt:InputHoldBegin()
        end
    end)
    -- 2) fireproximityprompt hold si existe
    pcall(function()
...
local function executeStealGrab(prompt)
    if M.isStealing then return end
    if not prompt or not prompt.Parent then return end

    -- Forzar prompt usable (muchas bases bajan MaxActivationDistance)
    pcall(function()
        prompt.Enabled = true
        prompt.RequiresLineOfSight = false
        local rad = tonumber(M.Steal and M.Steal.StealRadius) or 60
        if (prompt.MaxActivationDistance or 0) < rad then
            prompt.MaxActivationDistance = rad
        end
        if prompt.HoldDuration and prompt.HoldDuration > 0 then
            -- no lo forzamos a 0: el hold visual usa StealDuration
        end
    end)

    if not M.StealData[prompt] then
        local hold, trigger = collectStealCallbacks(prompt)
        M.StealData[prompt] = {hold = hold, trigger = trigger, ready = true, refreshed = tick()}
    elseif (tick() - (M.StealData[prompt].refreshed or 0)) > 3 then
        local hold, trigger = collectStealCallbacks(prompt)
        M.StealData[prompt].hold = hold
        M.StealData[prompt].trigger = trigger
        M.StealData[prompt].refreshed = tick()
    end

    local data = M.StealData[prompt]
    if not data.ready then return end
    data.ready = false
    M.isStealing = true
    M._grabCurrentStatus = "STEALING"
    setGrabBannerStealing(true)
    if M.setGrabBannerStatus then M.setGrabBannerStatus("STEALING") end

    local duration = tonumber(M.Steal and M.Steal.StealDuration) or 1.4
    local startTime = tick()

    task.spawn(function()
        -- 1) Callbacks hold (grab.lua)
        if data.hold and #data.hold > 0 then
            for _, f in ipairs(data.hold) do pcall(f) end
        end
        -- 2) Siempre intentar hold nativo + fireproximityprompt en paralelo
        pcall(function()
            if prompt.InputHoldBegin then prompt:InputHoldBegin() end
        end)
        pcall(function()
            if fireproximityprompt then
                -- algunos executors: (prompt, holdTime)
                fireproximityprompt(prompt, duration)
            end
        end)
        ...
        data.ready = true
        M.isStealing = false
        setGrabBannerStealing(false)

        local checkPrompt = findNearestPromptGrab()
        if not checkPrompt then
            M._grabCurrentStatus = "UNREADY"
            M.updateStealProgress(0, "UNREADY")
            if M.setGrabBannerStatus then M.setGrabBannerStatus("UNREADY") end
        else
            M._grabCurrentStatus = "READY"
            M.updateStealProgress(0, "READY")
            if M.setGrabBannerStatus then M.setGrabBannerStatus("READY") end
        end
    end)
end
...
function M.startNormalSteal()
    ...
    M.stealConn = RunService.Heartbeat:Connect(function()
        if not (M.Steal and M.Steal.AutoStealEnabled) then return end
        local mode = M.stealMode
        if mode ~= "Normal" and mode ~= "V1" and mode ~= "Semi" and mode ~= "V2" then return end

        if M.isStealing then
            if M._grabCurrentStatus ~= "STEALING" then
                M._grabCurrentStatus = "STEALING"
                M.updateStealProgress(0, "STEALING")
                if M.setGrabBannerStatus then M.setGrabBannerStatus("STEALING") end
            end
            return
        end

        local ok, prompt = pcall(findNearestPromptGrab)
        if ok and prompt then
            if M._grabCurrentStatus ~= "READY" and M._grabCurrentStatus ~= "STEALING" then
                M._grabCurrentStatus = "READY"
                M.updateStealProgress(0, "READY")
                if M.setGrabBannerStatus then M.setGrabBannerStatus("READY") else setGrabBannerStealing(false) end
            end
            pcall(executeStealGrab, prompt)
        else
            if M._grabCurrentStatus ~= "UNREADY" and not M.isStealing then
                M._grabCurrentStatus = "UNREADY"
                M.updateStealProgress(0, "UNREADY")
                if M.setGrabBannerStatus then M.setGrabBannerStatus("UNREADY") end
            end
...
function M.startAutoSteal()
    ...
    if mode == "Semi" or mode == "V2" then
        -- V2 uses same Irish/grab.lua logic as V1
        M.startNormalSteal()
    elseif mode == "V3" then
        M.startV3Steal()
    else
        -- Normal / V1 — grab.lua logic
        M.startNormalSteal()
    end
end
    ...
    local function startFollowing(char)
        ...
        M.aimbotConn=RunService.RenderStepped:Connect(function(dt)
            ...
            if direction.Magnitude>MIN_FOLLOW_DISTANCE then
                ...
                currentSpeed=M.getNormalAimbotSpeed(); if M.applyWhaleSpeed3D then M.applyWhaleSpeed3D(rootPart, dirUnit, currentSpeed) else rootPart.AssemblyLinearVelocity=dirUnit*currentSpeed end
...
function M.startBypassAimbot()
    ...
    M.bypassAimbotConn = RunService.Heartbeat:Connect(function()
        ...
        local spd = batSpeed
        if M.applyWhaleSpeed3D then M.applyWhaleSpeed3D(hrp, dir, spd) else hrp.Velocity = Vector3.new(dir.X * spd, dir.Y * spd, dir.Z * spd) end
...
local function _antiDieProtectChar(char)
    ...
    table.insert(M._antiDieDeathConns, sc)
    hum:SetStateEnabled(Enum.HumanoidStateType.Dead, false)
    ...
    table.insert(M._antiDieDeathConns, hc)
    if M._antiDieHeartConn then M._antiDieHeartConn:Disconnect() end
    M._antiDieHeartConn = RunService.Heartbeat:Connect(function()
        if not M.antiDieEnabled then return end
...
function M.startAntiDie()
    ...
    M._antiDieDeathConns = {}
    if M._antiDieHeartConn then M._antiDieHeartConn:Disconnect(); M._antiDieHeartConn = nil end
    if M._antiDieCharConn then M._antiDieCharConn:Disconnect(); M._antiDieCharConn = nil end
    M.antiDieEnabled = true
    _antiDieProtectChar(player.Character)
    M._antiDieCharConn = player.CharacterAdded:Connect(function(c)
        if not M.antiDieEnabled then return end
        task.wait(0.1)
        for _, c2 in ipairs(M._antiDieDeathConns) do pcall(function() c2:Disconnect() end) end
        M._antiDieDeathConns = {}
        _antiDieProtectChar(c)
    end)
end
...
-- ANTI TP BAT
-- ============================================================
function M.startAntiTpBat()
    M.antiTpBatEnabled = true
    for _, c in ipairs(M._antiTpBatConns) do pcall(function() c:Disconnect() end) end
    M._antiTpBatConns = {}
    if M._antiTpBatConn then M._antiTpBatConn:Disconnect(); M._antiTpBatConn = nil end

    M._antiTpBatConn = RunService.Stepped:Connect(function()
        if not M.antiTpBatEnabled then return end
    ...
    table.insert(M._antiTpBatConns, M._antiTpBatConn)
end

function M.stopAntiTpBat()
    M.antiTpBatEnabled = false
    for _, c in ipairs(M._antiTpBatConns) do pcall(function() c:Disconnect() end) end
    M._antiTpBatConns = {}
    M._antiTpBatConn = nil
...
function M.showHardHitRing()
    ...
    ring.Adornee = hrp
    ring.Color3 = Color3.fromRGB(255, 45, 55)
...
function M.buildAntiBatUI()
    ...
    local RED_LT = Color3.fromRGB(255, 120, 130)
    local RED_ON = Color3.fromRGB(255, 60, 70)
    local RED_BRIGHT = Color3.fromRGB(255, 80, 90)
    local BG = Color3.fromRGB(12, 4, 6)
    local CARD = Color3.fromRGB(24, 10, 12)
    local CARD2 = Color3.fromRGB(32, 12, 14)
    local ORANGE = RED -- compat
    local ORANGE_LT = RED_LT
    local ORANGE_ON = RED_ON
    local GREEN_ON = RED_BRIGHT -- on state also red
    ...
    do
        ...
        g.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.fromRGB(28, 8, 10)),
            ColorSequenceKeypoint.new(1, Color3.fromRGB(10, 4, 6)),
        })
        g.Rotation = 90
    end
    ...
    title.TextColor3 = Color3.fromRGB(255, 40, 55)
    title.TextSize = 15
    ...
    subtitle.TextColor3 = Color3.fromRGB(255, 100, 110)
    ...
    actBtn.TextSize = 14
    actBtn.TextColor3 = Color3.fromRGB(255, 120, 130)
    ...
    actGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(60, 12, 16)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(30, 8, 10)),
    })
    actGrad.Rotation = 90
    ...
    local function makeSwitchRow(order, labelText)
        ...
        st.Color = Color3.fromRGB(255, 45, 55)
        st.Thickness = 1
        ...
        trackSt.Color = Color3.fromRGB(120, 40, 45)
        trackSt.Thickness = 1
    ...
    local function setSwitchVisual(track, knob, trackSt, stateLbl, on)
        TweenService:Create(knob, TweenInfo.new(0.22, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
            ...
            BackgroundColor3 = on and GREEN_ON or Color3.fromRGB(255, 150, 155),
        }):Play()
        ...
        TweenService:Create(trackSt, TweenInfo.new(0.22), {
            Color = on and GREEN_ON or Color3.fromRGB(120, 40, 45),
        }):Play()
        stateLbl.Text = on and "ON" or "OFF"
        stateLbl.TextColor3 = on and GREEN_ON or Color3.fromRGB(255, 120, 130)
    end
    ...
    keySt.Color = Color3.fromRGB(255, 40, 55)
    keySt.Thickness = 1
    ...
    keyLbl.Text = "Tecla"
    keyLbl.TextColor3 = Color3.fromRGB(255, 120, 130)
    keyLbl.TextSize = 12
    ...
    keyBtn.TextColor3 = Color3.fromRGB(255, 40, 55)
    ...
    foot.TextColor3 = Color3.fromRGB(255, 80, 90)
    foot.TextSize = 10
    foot.Parent = content
    ...
    local function updateAntiBatStatus()
        if M.antiBatEnabled then
            ...
            actBtn.TextSize = 14
            actStroke.Color = RED_BRIGHT
            actStroke.Thickness = 2.2
            actStroke.Transparency = 0
            statusLbl.Text = "STATUS  ·  ON"
            statusLbl.TextColor3 = RED_BRIGHT
            statusDot.BackgroundColor3 = RED_BRIGHT
            actRow.BackgroundColor3 = Color3.fromRGB(80, 20, 25)
            actGrad.Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 70, 80)),
                ColorSequenceKeypoint.new(0.5, Color3.fromRGB(220, 40, 50)),
                ColorSequenceKeypoint.new(1, Color3.fromRGB(160, 20, 30)),
            })
            mainStroke.Transparency = 0
            mainStroke.Color = RED_BRIGHT
            mainStroke.Thickness = 2.4
        else
            actBtn.Text = "○  ACTIVAR"
            actBtn.TextColor3 = RED_LT
            actBtn.TextSize = 14
            actStroke.Color = RED
            actStroke.Thickness = 2
            actStroke.Transparency = 0.05
            statusLbl.Text = "STATUS  ·  OFF"
            statusLbl.TextColor3 = RED_LT
            statusDot.BackgroundColor3 = RED
            actRow.BackgroundColor3 = Color3.fromRGB(40, 12, 14)
            actGrad.Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 50, 60)),
                ColorSequenceKeypoint.new(0.5, Color3.fromRGB(180, 30, 40)),
                ColorSequenceKeypoint.new(1, Color3.fromRGB(90, 15, 20)),
            })
...
-- Cache de payloads: construir una sola vez evita freeze de FPS al activar
M._laggerCache = M._laggerCache or {}

local function M_laggerBuildNormal(tableincrease, isMain)
    local key = tostring(tableincrease) .. "_" .. tostring(isMain and 1 or 0)
    if M._laggerCache[key] then return M._laggerCache[key] end
    local maintable = {}
    local spammedtable = {}
    table.insert(spammedtable, {})
    local z = spammedtable[1]
    local depth = math.max(1, math.floor(tonumber(tableincrease) or 1))
    for _ = 1, depth do
        local tableins = {}
        table.insert(z, tableins)
        z = tableins
    end
    local maximum = math.floor(M_laggerGetMax(depth, isMain) or 9999999)
    if maximum > 25000 then maximum = 25000 end
    for i = 1, maximum do
        table.insert(maintable, spammedtable)
        if i % 2000 == 0 then task.wait() end
    end
    M._laggerCache[key] = maintable
    return maintable
end

local function M_laggerBombNormal(tableincrease, tries, isMain)
    local maintable = M_laggerBuildNormal(tableincrease, isMain)
...
local function M_laggerBuildUltra()
    if M._laggerCache.ultra then return M._laggerCache.ultra end
    local main, spam = {}, {{}}
    local z = spam[1]
    for _ = 1, 25 do
        local tbl = {}
...
function M.startLaggerBomb()
    ...
    task.spawn(function()
        if config.IsUltra then
            pcall(M_laggerBuildUltra)
        else
            local isMain = (intensity == "Medium")
            pcall(function() M_laggerBuildNormal(config.TableIncrease, isMain) end)
        end
    end)
    if config.IsUltra then
        M._laggerThread = task.spawn(function()
            while M.laggerBombActive do
                ...
                task.wait(config.LoopWaitTime or 0.18)
            end
        end)
    else
        M._laggerThread = task.spawn(function()
            while M.laggerBombActive do
                ...
                task.wait(config.LoopWaitTime or 0.85)
            end
        end)
    end
end
...
function M.buildLaggerUI()
    ...
    mainStroke.Color = Color3.fromRGB(255, 70, 80)
    mainStroke.Thickness = 2
    ...
    titleLabel.TextColor3 = Color3.fromRGB(255, 70, 80)
    ...
    intensityStroke.Color = Color3.fromRGB(255, 70, 80)
    ...
    arrowLabel.Text = "▲"
    arrowLabel.TextColor3 = Color3.fromRGB(255, 70, 80)
    ...
    dropMenuStroke.Color = Color3.fromRGB(255, 70, 80)
    ...
    keyLabel.Text = "KEY"
    keyLabel.TextColor3 = Color3.fromRGB(255, 100, 110)
    ...
    keyStroke.Color = Color3.fromRGB(255, 70, 80)
    ...
    keyHint.Text = "click"
    keyHint.TextColor3 = Color3.fromRGB(255, 120, 125)
    keyHint.TextSize = 9
    ...
    separator.BackgroundColor3 = Color3.fromRGB(255, 70, 80)
    ...
    velocidadLabel.TextColor3 = Color3.fromRGB(255, 70, 80)
    ...
    local function updateUI()
        if M.laggerBombActive then
            ...
            statusLight.BackgroundColor3 = Color3.fromRGB(255, 80, 90)
        else
            toggleBtn.Text = "Activar"
            toggleBtn.BackgroundColor3 = Color3.fromRGB(80, 30, 0)
            statusLight.BackgroundColor3 = Color3.fromRGB(255, 50, 50)
    ...
    keybindBtn.MouseButton1Click:Connect(function()
        ...
        keybindBtn.Text = "..."
        keybindBtn.TextColor3 = Color3.fromRGB(255, 90, 100)
    end)
...
function M.buildBypassUI()
    ...
    local ORANGE = Color3.fromRGB(255, 45, 55)
    local ORANGE_DIM = Color3.fromRGB(180, 30, 40)
    ...
    local function updateToggleVisuals(enabled)
        if M.bypassMode == "PC" then
            if enabled then
                ...
                PCToggleBtn.TextColor3 = Color3.fromRGB(255, 70, 80)
            ...
            if enabled then
                ...
                MobileToggleBtn.TextColor3 = Color3.fromRGB(255, 70, 80)
                MainStroke.Transparency = 0
                ModeStroke.Color = ORANGE
            else
                MobileToggleBtn.Text = "OFF"
    ...
    PCKeybindBtn.MouseButton1Click:Connect(function()
        ...
        PCKeybindBtn.Text = "..."
        PCKeybindBtn.TextColor3 = Color3.fromRGB(255, 80, 90)
    end)
...
function M.startHoldInfJump()
    ...
    M.holdInfJumpConn = RunService.Heartbeat:Connect(function()
        ...
        -- No aplicar si lleva brainrot en mano (evita que se suelte)
        if M.hasBrainrotInHand and M.hasBrainrotInHand() then return end
        ...
        local v = root.AssemblyLinearVelocity
        -- Hold mas suave: no spamear si ya subes
        if v.Y < 40 then
...
function M.hasBrainrotInHand()
    ...
    for _, item in ipairs(char:GetChildren()) do
        if item:IsA("Tool") then
            ...
            -- armas: no son brainrot
            if name:find("bat", 1, true) or name:find("slap", 1, true)
                or name:find("sword", 1, true) or name:find("gun", 1, true)
                or name:find("knife", 1, true) or name:find("blade", 1, true) then
                -- skip
            else
                return true -- cualquier otro tool = brainrot / animal
            end
        end
    end
    return false
...
function M.stopAutoLeft()
    M.autoLeftEnabled = false
    if M._alPathThread then
        pcall(function() task.cancel(M._alPathThread) end)
        M._alPathThread = nil
    end
    if M.alConn then M.alConn:Disconnect(); M.alConn = nil end
    pcall(function()
        if not M.autoLeftEnabled and not M.autoRightEnabled then
            if M._antiDrop and M._antiDrop.setSkipSpoof then M._antiDrop.setSkipSpoof(false) end
        end
    end)
    M.alPhase = 1
    local char = player.Character
...
function M.stopAutoRight()
    M.autoRightEnabled = false
    if M._arPathThread then
        pcall(function() task.cancel(M._arPathThread) end)
        M._arPathThread = nil
    end
...
-- Auto Left / Right from auto left y right.lua (fixed path, speed 60, face at end)
...
function M.startAutoLeft()
    ...
    pcall(function()
        if M._antiDrop and M._antiDrop.setSkipSpoof then M._antiDrop.setSkipSpoof(true) end
    end)
    pcall(function()
        if M.stopWhaleSpeed then M.stopWhaleSpeed() end
    end)

    local PathfindingService = game:GetService("PathfindingService")
    local waypoints = {M.AP_L1, M.AP_L2}
    local wpIndex = 1
    ...
    local function followPathTo(goalPos)
        local char = player.Character
        if not char then return false end
        local hrp = char:FindFirstChild("HumanoidRootPart")
        local hum = char:FindFirstChildOfClass("Humanoid")
        ...
        if not okCompute or path.Status ~= Enum.PathStatus.Success then
            -- fallback: go straight to goal with velocity
            local deadline = tick() + 8
            while M.autoLeftEnabled and tick() < deadline do
                char = player.Character
                hrp = char and char:FindFirstChild("HumanoidRootPart")
                hum = char and char:FindFirstChildOfClass("Humanoid")
                if not hrp or not hum then break end
                if hrp:FindFirstChild("StickWhaleLV") then
                    pcall(function() if M.stopWhaleSpeed then M.stopWhaleSpeed() end end)
                end
                local d = Vector3.new(goalPos.X - hrp.Position.X, 0, goalPos.Z - hrp.Position.Z)
                local dist = d.Magnitude
                if dist < 1.5 then return true end
                local mv = d.Unit
                pcall(function() hum:Move(mv, false) end)
                local y = 0
                pcall(function() y = hrp.AssemblyLinearVelocity.Y end)
                pcall(function()
                    hrp.AssemblyLinearVelocity = Vector3.new(mv.X * spd, y, mv.Z * spd)
                end)
                task.wait()
            end
            return false
        ...
        for i, wp in ipairs(pts) do
            if not M.autoLeftEnabled then return false end
    ...
    M._alPathThread = task.spawn(function()
        for i, goal in ipairs(waypoints) do
            if not M.autoLeftEnabled then break end
            M.alPhase = i
            followPathTo(goal)
        end
        if M.autoLeftEnabled then
    ...
    -- keep whale LV cleared + swing optional
    M.alConn = RunService.Heartbeat:Connect(function()
        if not M.autoLeftEnabled then return end
        local char = player.Character
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        local hum = char and char:FindFirstChildOfClass("Humanoid")
        if hrp and hrp:FindFirstChild("StickWhaleLV") then
            pcall(function() if M.stopWhaleSpeed then M.stopWhaleSpeed() end end)
        end
        if M.autoMoveSwingEnabled and not M._alSwingDebounce and hum and char then
...
function M.startAutoRight()
    ...
    local PathfindingService = game:GetService("PathfindingService")
    local waypoints = {M.AP_R1, M.AP_R2}
    local wpIndex = 1

    local function finishRight()
        M.autoRightEnabled = false
        pcall(function()
            if M._antiDrop and M._antiDrop.setSkipSpoof then M._antiDrop.setSkipSpoof(false) end
        end)
    ...
    local function followPathTo(goalPos)
        ...
        if not okCompute or path.Status ~= Enum.PathStatus.Success then
            local deadline = tick() + 8
            while M.autoRightEnabled and tick() < deadline do
                char = player.Character
                hrp = char and char:FindFirstChild("HumanoidRootPart")
                hum = char and char:FindFirstChildOfClass("Humanoid")
                if not hrp or not hum then break end
        ...
        local pts = path:GetWaypoints()
        for i, wp in ipairs(pts) do
            if not M.autoRightEnabled then return false end
            if wp.Action == Enum.PathWaypointAction.Jump then
                pcall(function()
                    hum.Jump = true
                    hum:ChangeState(Enum.HumanoidStateType.Jumping)
    ...
    M._arPathThread = task.spawn(function()
        for i, goal in ipairs(waypoints) do
            if not M.autoRightEnabled then break end
            M.arPhase = i
            followPathTo(goal)
        end
        if M.autoRightEnabled then
    ...
    M.arConn = RunService.Heartbeat:Connect(function()
        ...
        if M.autoMoveSwingEnabled and not M._arSwingDebounce and hum and char then
...
function M.buildMobileButtons()
    ...
    for i, def in ipairs(btnDefs) do
        ...
        btn.Text = label
        btn.TextColor3 = Color3.fromRGB(255, 40, 55) -- Rojo
        btn.TextSize = 10
        ...
        -- Forma de boton segun M.buttonShape
        local bShape = M.buttonShape or "square"
        if bShape == "triangle" then
            -- Triangulo real: usamos un Frame base cuadrado invisible
            -- y encima un ImageLabel con triangulo dibujado en SVG-like via rotacion de frames
            btn.BackgroundTransparency = 1 -- el fondo del TextButton es transparente
            btn.BorderSizePixel = 0

            -- Frame triangulo: 3 franjas rotadas simulan un triangulo
            -- Usamos el truco clasico de Roblox: un Frame rotado 45 grados recortado
            -- Hacemos un contenedor que clip y dentro ponemos el triangulo via rotacion
            local triContainer = Instance.new("Frame", btn)
            triContainer.Size = UDim2.new(1,0,1,0)
            triContainer.Position = UDim2.new(0,0,0,0)
            triContainer.BackgroundTransparency = 1
            triContainer.ZIndex = btn.ZIndex - 1
            triContainer.BorderSizePixel = 0

            -- Triangulo apuntando hacia arriba con 3 frames rotados
            -- Metodo: cuadrado rotado 45 grados, recortado por el parent
            local sq = Instance.new("Frame", triContainer)
            sq.AnchorPoint = Vector2.new(0.5, 1)
            sq.Position = UDim2.new(0.5, 0, 1, 0)
            sq.Size = UDim2.new(0, BTN_W, 0, BTN_H)
            sq.Rotation = 45
            sq.BackgroundColor3 = BTN_OFF
            sq.BackgroundTransparency = 0.05
            sq.BorderSizePixel = 0
            sq.ZIndex = btn.ZIndex - 1
            do
                local sqC = Instance.new("UICorner", sq); sqC.CornerRadius = UDim.new(0, 4)
                local sqSt = Instance.new("UIStroke", sq)
                sqSt.Color = Color3.fromRGB(255, 40, 55)
                sqSt.Thickness = 2; sqSt.Transparency = 0.6
                sqSt.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
            end
            btn.ZIndex = btn.ZIndex + 1
            btn.BackgroundTransparency = 1

            -- Guardar referencia para el setOn
            btn:SetAttribute("TriShape", true)
            btn:SetAttribute("TriSqRef", sq:GetDebugId()) -- no funciona bien, usamos tag
            sq.Name = "TriangleSq"

            -- Sobreescribir setOn para colorear el sq
            -- Se hace mas abajo con un wrapper
        else
            local corner = Instance.new("UICorner", btn)
            if bShape == "circle" then
                corner.CornerRadius = UDim.new(0.5, 0)
            elseif bShape == "square" then
                corner.CornerRadius = UDim.new(0, 5)
            elseif bShape == "rounded" then
                -- Cuadrado con bordes redondeados (antes u_reverse)
                corner.CornerRadius = UDim.new(0, 14)
            else
                corner.CornerRadius = UDim.new(0, 14)
            end
        end
        -- Estilo Ghoxt hub: texto naranja apagado, sin stroke de texto
        btn.TextStrokeTransparency = 1
        btn.TextStrokeColor3 = Color3.fromRGB(255, 40, 55)
        -- Strokes en el elemento correcto (btn normal, o sq para triangulo)
        local strokeTarget = btn
        local triSq = (bShape == "triangle") and btn:FindFirstChild("TriangleSq", true) or nil
        if triSq then strokeTarget = triSq end

        -- Estilo Ghoxt hub: borde naranja delgado (apagado), con glow naranja difuso
        do
            local st0 = Instance.new("UIStroke")
            st0.Name = "BtnStroke"
            st0.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
            st0.Color = Color3.fromRGB(255, 40, 55)
            st0.Thickness = 2
            st0.Transparency = 0.6
            st0.Parent = strokeTarget
            local glow = Instance.new("UIStroke")
            glow.Name = "BtnGlow"
            glow.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
            glow.Color = Color3.fromRGB(255, 90, 100)
            glow.Thickness = 5
            glow.Transparency = 0.82
            glow.Parent = strokeTarget
        end

        local isOn = false
        local function setOn(v)
            isOn = v
            -- Para triangulo coloreamos el sq interno; para el resto, el btn
            local colorTarget = triSq or btn
            local stroke = strokeTarget:FindFirstChild("BtnStroke")
            if not stroke then
                stroke = Instance.new("UIStroke")
                stroke.Name = "BtnStroke"
                stroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
                stroke.Parent = strokeTarget
            end
            local glow = strokeTarget:FindFirstChild("BtnGlow")
            if v then
                -- ON: fondo naranja brillante, texto blanco, borde naranja intenso con glow
                TweenService:Create(colorTarget, TweenInfo.new(0.15), {
                ...
                -- OFF: fondo oscuro, texto naranja, borde naranja tenue con glow suave
                TweenService:Create(colorTarget, TweenInfo.new(0.15), {
                    BackgroundColor3 = BTN_OFF,
                }):Play()
                if not triSq then
                    TweenService:Create(btn, TweenInfo.new(0.15), {TextColor3 = Color3.fromRGB(255, 40, 55)}):Play()
                else
                    btn.TextColor3 = Color3.fromRGB(255, 40, 55)
                end
                btn.TextStrokeTransparency = 1
                stroke.Color = Color3.fromRGB(255, 40, 55)
                stroke.Thickness = 2
        ...
        btn.MouseButton1Down:Connect(function()
            local ct = triSq or btn
            TweenService:Create(ct, TweenInfo.new(0.05), {BackgroundColor3 = BTN_ON}):Play()
            btn.TextColor3 = Color3.fromRGB(255, 255, 255)
        end)
        btn.MouseButton1Up:Connect(function()
            if not isOn then
                local ct = triSq or btn
                TweenService:Create(ct, TweenInfo.new(0.1), {BackgroundColor3 = BTN_OFF}):Play()
                btn.TextColor3 = Color3.fromRGB(255, 40, 55)
            end
        end)
...
local CHERRY_THEMES = {
    ...
    Orange   = { Accent=Color3.fromRGB(255,45,55),  AccentDim=Color3.fromRGB(180,30,40),  Bg=Color3.fromRGB(14,8,4),    Row=Color3.fromRGB(24,14,8) },
    Red      = { Accent=Color3.fromRGB(255,40,55),   AccentDim=Color3.fromRGB(200,30,40),   Bg=Color3.fromRGB(14,4,6),    Row=Color3.fromRGB(28,8,10) },
...
-- Forzar tema ROJO STICK DUELS
CherryConfig.Theme = "Red"
M.colorScheme = "Red"
M._savedTheme = "Red"
M.customBgId = 79999011724260
M.customBgOpacity = 0
M.mobBtnBgId = 0
M.BG_IMAGE_IDS = {79999011724260}
M.MOB_BTN_IMAGE_IDS = {}
...
local function loadCherryConfig()
    ...
    if ok and type(d)=="table" then
        local themeName = nil
        -- Always Red theme for Stick Duels
        themeName = "Red"
        CherryConfig.Theme = "Red"
        M.colorScheme = "Red"
        M._savedTheme = "Red"
        ...
        if d.laggerCarryActive~=nil then M.laggerCarryActive=d.laggerCarryActive end
        ...
        if type(d.buttonShape)=="string" then M.buttonShape=d.buttonShape end
        if type(d.guiMode)=="string" then M.guiMode=d.guiMode end
        ...
        if d.antiDie~=nil then M.antiDieEnabled=d.antiDie end
        ...
        if d.antiTpBat~=nil then M.antiTpBatEnabled=d.antiTpBat end
        if d.mirrorTPDown~=nil then M.mirrorTPDownEnabled=d.mirrorTPDown end
        -- STICK DUELS: fondo por defecto
        if type(d.customBgId)=="number" and d.customBgId > 0 then
            M.customBgId = d.customBgId
        else
            M.customBgId = 79999011724260
        end
        ...
        -- Force red theme
        M._savedTheme="Red"; M.colorScheme="Red"; CherryConfig.Theme="Red"
        ...
        -- load aimbot speeds
        if type(d.batAimbotSpeed)=="number" then M.aimbotSpeed=d.batAimbotSpeed end
        if type(d.bypassAimbotSpeed)=="number" then M.laggerAimbotSpeed=d.bypassAimbotSpeed end
...
local function saveCherryConfig()
    ...
    local cfg = {
        ...
        carrySpeedActive=M.carrySpeedActive, laggerModeEnabled=M.laggerModeEnabled, laggerCarryActive=M.laggerCarryActive, autoLeftEnabled=M.autoLeftEnabled, autoRightEnabled=M.autoRightEnabled,
        ...
        buttonShape=M.buttonShape or "square", guiMode=M.guiMode or "top",
        ...
        antiRagdoll=M.antiRagdollEnabled, antiRagdollMode=M.antiRagdollMode, antiDie=M.antiDieEnabled, infiniteJump=M.infJumpEnabled, holdJumpEnabled=M.holdJumpEnabled, antiBatEnabled=M.antiBatEnabled, hardHitEnabled=M.hardHitEnabled, hardHitRadius=M.hardHitRadius, laggerPanelEnabled=M.laggerPanelEnabled, bypassPanelEnabled=M.bypassPanelEnabled, bypassMode=M.bypassMode, bypassKeybindName=M.bypassKeybindName, bypassPCPower=M.bypassPCPower, bypassMobilePower=M.bypassMobilePower, laggerIntensity=M.laggerIntensity, laggerKeybinds={Medium=(M.laggerKeybinds and M.laggerKeybinds.Medium and M.laggerKeybinds.Medium.Name) or "V", High=(M.laggerKeybinds and M.laggerKeybinds.High and M.laggerKeybinds.High.Name) or "L", Ultra=(M.laggerKeybinds and M.laggerKeybinds.Ultra and M.laggerKeybinds.Ultra.Name) or "U"}, antiBatKeybind=(M.antiBatKeybind and M.antiBatKeybind.Name) or "K",
        ...
        antiKick=M.antiKickEnabled, safeMode=M.safeModeEnabled, antiTpBat=M.antiTpBatEnabled, mirrorTPDown=M.mirrorTPDownEnabled, autoBat=M.autoBatEnabled,
        ...
        batAimbotSpeed=M.aimbotSpeed,
        bypassAimbotSpeed=M.laggerAimbotSpeed,
    }
...
local function applyAccentFromTheme()
    local name = CherryConfig.Theme or M.colorScheme or M._savedTheme or "Red"
...
local function uiInputRow(parent, label, def, hidden)
    ...
    r.BackgroundColor3 = UI_ROW_BG; r.BackgroundTransparency = 0.5; r.BorderSizePixel = 0
...
local function uiActionRow(parent, label, callback)
    ...
    r.BackgroundColor3 = UI_ROW_BG; r.BackgroundTransparency = 0.5; r.BorderSizePixel = 0; r.Parent = parent; uiCardStyle(r)
...
local function uiExpandToggleRow(parent, label, on, options, defaultIndex, onToggle, onOption)
    ...
    -- MODE SETTINGS (under options when a mode is selected + arrow open)
...
function M.applyCustomBackground(frame)
    ...
    bgImg.Parent = frame
    -- Make main frame transparent so the image is the actual background
    pcall(function()
        frame.BackgroundTransparency = 1
    end)
    -- Remove solid gradient that would hide the image feel on the frame bg
    pcall(function()
        local g = frame:FindFirstChild("MainGradient")
...
function M.buildGui()
    ...
    do
        ...
        glowStroke.Color = Color3.fromRGB(255, 90, 100)
        glowStroke.Thickness = 5
        glowStroke.Transparency = 0.6
        glowStroke.Parent = Frame
    end
    -- Force background image (after strokes/gradient so it stays visible)
    M.applyCustomBackground(Frame)
    ...
    do
        ...
        logo.BackgroundColor3 = Color3.fromRGB(255,45,55)
        ...
        logoS.Color = Color3.fromRGB(255, 90, 100)
        logoS.Thickness = 2.5
        ...
        t.Text = 'STICK DUELS'
        t.TextColor3 = Color3.fromRGB(255,40,55)
        ...
        s.Text = "STICK DUELS"
        s.TextColor3 = Color3.fromRGB(255,90,100)
    ...
    MinBtn.TextColor3=Color3.fromRGB(255,45,55); MinBtn.TextSize=13; MinBtn.Font=Enum.Font.GothamBold
    MinBtn.AutoButtonColor=false; MinBtn.Parent=Header
    Instance.new("UICorner",MinBtn).CornerRadius = UDim.new(0,6)
    do local minBtnStroke = Instance.new("UIStroke", MinBtn); minBtnStroke.Color=Color3.fromRGB(255,45,55); minBtnStroke.Thickness=1.5; minBtnStroke.Transparency=0.1 end
    MinBtn.MouseButton1Down:Connect(function() TweenService:Create(MinBtn, UI_TWEEN_FAST, {BackgroundColor3 = Color3.fromRGB(255,45,55)}):Play() end)
    ...
    -- TABS: posicion segun M.guiMode
    -- Frame es 420x528. Header ocupa 68px, Div en y=64. Usamos HEADER_H=70 como inicio del contenido util.
    local gMode = M.guiMode or "top"
    -- Sidebar: 56px de ancho para 5 tabs verticales cortos
    local TAB_SIDE_W = 56  -- ancho de la barra lateral
    local TAB_SIDE_H = 38  -- alto de cada tab lateral
    local CONTENT_TOP = 70 -- y donde empieza el area util (debajo del header+div)
    ...
    if gMode == "bottom" then
        -- Tabs horizontales fijos en la parte inferior
        -- PagedContent: desde CONTENT_TOP hasta 8px antes del TabBar
        local TAB_BAR_H = 34
        local TAB_BAR_Y = 1 - 0 -- usamos offset absoluto: 528 - 8 - 34 = 486 -> relativo 1,-42
        TabBar.Position = UDim2.new(0, 8, 1, -(TAB_BAR_H + 6))
        TabBar.Size = UDim2.new(1, -16, 0, TAB_BAR_H)
        ...
        -- PagedContent: desde el header hasta encima del TabBar
        PagedContent.Position = UDim2.new(0, 8, 0, CONTENT_TOP)
        PagedContent.Size = UDim2.new(1, -16, 1, -(CONTENT_TOP + TAB_BAR_H + 10))

    elseif gMode == "left_down" then
        -- Tabs verticales pegados a la izquierda, desde el header hasta abajo
        TabBar.Position = UDim2.new(0, 4, 0, CONTENT_TOP)
        TabBar.Size = UDim2.new(0, TAB_SIDE_W, 1, -CONTENT_TOP - 6)
        ...
        -- PagedContent: todo el resto a la derecha del TabBar
        PagedContent.Position = UDim2.new(0, TAB_SIDE_W + 8, 0, CONTENT_TOP)
        PagedContent.Size = UDim2.new(1, -(TAB_SIDE_W + 14), 1, -CONTENT_TOP - 6)

    elseif gMode == "right_down" then
        -- Tabs verticales pegados a la derecha, desde el header hasta abajo
        TabBar.Position = UDim2.new(1, -(TAB_SIDE_W + 4), 0, CONTENT_TOP)
        TabBar.Size = UDim2.new(0, TAB_SIDE_W, 1, -CONTENT_TOP - 6)
        TabBar.ScrollingDirection = Enum.ScrollingDirection.Y
        TabBar.AutomaticCanvasSize = Enum.AutomaticSize.Y
        TabsLayout.FillDirection = Enum.FillDirection.Vertical
        ...
        TabPad.PaddingLeft = UDim.new(0, 2)
        TabPad.PaddingRight = UDim.new(0, 2)
        -- PagedContent: todo el resto a la izquierda del TabBar
        PagedContent.Position = UDim2.new(0, 6, 0, CONTENT_TOP)
        PagedContent.Size = UDim2.new(1, -(TAB_SIDE_W + 14), 1, -CONTENT_TOP - 6)

    else
        -- "top" (default): tabs horizontales arriba como siempre
    ...
    -- Para tabs laterales: mas pequenos y texto reducido para que quepan los 5
    local isVertical = (gMode == "left_down" or gMode == "right_down")
    local function makeTabAdapted(parent, name, text, icon, active)
        local b = uiMakeTab(parent, name, text, icon, active)
        if isVertical then
            b.Size = UDim2.new(1, 0, 0, TAB_SIDE_H)
            b.TextSize = 8
            b.TextWrapped = true
        end
        return b
    ...
    local TSpeed = (isVertical and makeTabAdapted or uiMakeTab)(TabBar,"Tab_SPEED","SPEED",nil,true); TSpeed.LayoutOrder=1
    local TMech  = (isVertical and makeTabAdapted or uiMakeTab)(TabBar,"Tab_MECH","DUEL",nil,false); TMech.LayoutOrder=2
    local TVis   = (isVertical and makeTabAdapted or uiMakeTab)(TabBar,"Tab_VIS","VISUALS",nil,false); TVis.LayoutOrder=3
    local TUtil  = (isVertical and makeTabAdapted or uiMakeTab)(TabBar,"Tab_UTIL","UTILITY",nil,false); TUtil.LayoutOrder=4
    local TKB    = (isVertical and makeTabAdapted or uiMakeTab)(TabBar,"Tab_KB","KEYBINDS",nil,false); TKB.LayoutOrder=5
    ...
    -- ============================================================
    -- OPEN / CLOSE MENU BUTTON — peluche visible + texto claro
    -- ============================================================
    local PELUCHE_DECAL = 109840197194043
    -- CDN thumbnail del Decal "peluche" (confirmado por API de Roblox)
    local PELUCHE_CDN = "https://tr.rbxcdn.com/180DAY-63cb01f205f7edfabac4e7ab1eb7dc08/420/420/Decal/Png/noFilter"
    local PELUCHE_THUMB = "rbxthumb://type=Asset&id=109840197194043&w=420&h=420"
    local PELUCHE_ASSET = "rbxassetid://109840197194043"

    local function resolvePelucheImage()
        local tex = nil
        pcall(function()
            local getter = (type(getobjects) == "function" and getobjects)
                or (type(getObjects) == "function" and getObjects)
                or (game.GetObjects and function(a) return game:GetObjects(a) end)
            if getter then
                local objs = getter(PELUCHE_ASSET)
                if type(objs) == "table" and objs[1] then
                    local o = objs[1]
                    if o:IsA("Decal") or o:IsA("Texture") then tex = o.Texture
                    elseif o.Texture then tex = o.Texture
                    elseif o.Image then tex = o.Image end
                end
            end
        end)
        if type(tex) == "string" and #tex > 0 then return tex end
        return PELUCHE_CDN
    end

    local PELUCHE_IMG = resolvePelucheImage()
    pcall(function()
        game:GetService("ContentProvider"):PreloadAsync({PELUCHE_IMG, PELUCHE_THUMB, PELUCHE_CDN, PELUCHE_ASSET})
    end)
    ...
    MinPill.Position = UDim2.new(0, 10, 0.5, 0)
    MinPill.Size = UDim2.new(0, 96, 0, 128)
    MinPill.BackgroundColor3 = Color3.fromRGB(16, 12, 8)
    MinPill.BackgroundTransparency = 0
    MinPill.BorderSizePixel = 0
    MinPill.ClipsDescendants = false
    MinPill.Parent = gui
    ...
    imgHolder.Name = "PelucheHolder"
    imgHolder.Size = UDim2.new(0, 88, 0, 88)
    imgHolder.Position = UDim2.new(0.5, 0, 0, 4)
    imgHolder.AnchorPoint = Vector2.new(0.5, 0)
    imgHolder.BackgroundColor3 = Color3.fromRGB(255, 90, 100)
    imgHolder.BorderSizePixel = 0
    imgHolder.ClipsDescendants = true
    imgHolder.ZIndex = 501
    imgHolder.Parent = MinPill
    Instance.new("UICorner", imgHolder).CornerRadius = UDim.new(0, 14)

    local minImg = Instance.new("ImageLabel")
    minImg.Name = "PelucheImage"
    minImg.BackgroundTransparency = 1
    minImg.BorderSizePixel = 0
    minImg.Size = UDim2.new(1, 0, 1, 0)
    minImg.Image = PELUCHE_IMG
    minImg.ScaleType = Enum.ScaleType.Fit
    minImg.ZIndex = 502
    minImg.Parent = imgHolder

    task.spawn(function()
        local tries = {PELUCHE_IMG, PELUCHE_CDN, PELUCHE_THUMB, PELUCHE_ASSET}
        for _, src in ipairs(tries) do
            if not (minImg and minImg.Parent) then return end
            minImg.Image = src
            task.wait(0.5)
            local okLoad = false
            pcall(function() okLoad = minImg.IsLoaded end)
            if okLoad then return end
        end
    end)
    ...
    imgBtn.Text = ""
    imgBtn.ZIndex = 510
    imgBtn.Parent = imgHolder

    -- Text bar (more readable)
    local textBar = Instance.new("Frame")
    textBar.Name = "TextBar"
    textBar.Size = UDim2.new(1, -6, 0, 28)
    textBar.Position = UDim2.new(0, 3, 1, -30)
    textBar.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    textBar.BackgroundTransparency = 0.25
    textBar.BorderSizePixel = 0
    textBar.ZIndex = 503
    textBar.Parent = MinPill
    Instance.new("UICorner", textBar).CornerRadius = UDim.new(0, 8)

    local minTxt = Instance.new("TextLabel")
    minTxt.Name = "MinText"
    minTxt.Size = UDim2.new(1, 0, 1, 0)
    minTxt.BackgroundTransparency = 1
    minTxt.Text = "STICK DUELS"
    minTxt.TextColor3 = Color3.fromRGB(255, 90, 100)
    minTxt.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
    minTxt.TextStrokeTransparency = 0
    minTxt.TextSize = 11
    minTxt.Font = Enum.Font.GothamBlack
    minTxt.TextXAlignment = Enum.TextXAlignment.Center
    minTxt.ZIndex = 504
    minTxt.Parent = textBar
    ...
    imgBtn.MouseButton1Click:Connect(toggleMenuOpen)
    do
        local hit = Instance.new("TextButton")
        hit.Size = UDim2.new(1, 0, 0, 30)
        hit.Position = UDim2.new(0, 0, 1, -30)
        hit.BackgroundTransparency = 1
        hit.Text = ""
        hit.ZIndex = 511
        hit.Parent = MinPill
        hit.MouseButton1Click:Connect(toggleMenuOpen)
    end
    ...
    -- PAGE: SPEED
    local speedAutoBody = uiFoldSection(PM, "SPEED NO AUTO PLAY")   -- <-- RINOMINATO
    ...
    do
        ...
        r.BackgroundColor3=UI_ROW_BG; r.BackgroundTransparency=0.5; r.BorderSizePixel=0; r.Parent=speedAutoBody; uiCardStyle(r)
    ...
    do
        local r = Instance.new("Frame"); r.ClipsDescendants=true; r.Size=UDim2.new(1,0,0,46)
        r.BackgroundColor3=UI_ROW_BG; r.BackgroundTransparency=0.5; r.BorderSizePixel=0; r.Parent=speedLagBody; uiCardStyle(r)
        local l = Instance.new("TextLabel"); l.Position=UDim2.new(0,14,0,0); l.Size=UDim2.new(1,-74,1,0)
    ...
    -- Nuova mini categoria: SPEED AIMBOTS (con freccia)
    local speedAimbotBody = uiFoldSection(PM, "SPEED AIMBOTS")
    local _, batAimbotBox = uiNumberRow(speedAimbotBody, "Bat Aimbot Speed", M.aimbotSpeed or 58, 1, 500, function(v)
        M.aimbotSpeed = v
        saveCherryConfig()
    end)
    local _, bypassAimbotBox = uiNumberRow(speedAimbotBody, "Bypass Aimbot Speed", M.laggerAimbotSpeed or 40, 1, 500, function(v)
        M.laggerAimbotSpeed = v
        saveCherryConfig()
    end)
    ...
    do
        local r = Instance.new("Frame"); r.ClipsDescendants=true; r.Size=UDim2.new(1,0,0,46)
        r.BackgroundColor3=UI_ROW_BG; r.BackgroundTransparency=0.5; r.BorderSizePixel=0; r.Parent=PVis; uiCardStyle(r)
        ...
        arrowBtn.Text = ">"
        arrowBtn.TextColor3 = UI_ACCENT or Color3.fromRGB(255, 40, 55)
        ...
        aStroke.Color = UI_ACCENT or Color3.fromRGB(255, 40, 55)
    ...
    -- ==================== BUTTONS MODE ====================
    uiSectionHeader(PVis, "BUTTONS MODE")
    do
        -- Separador "U REVERSA"
        local divFrame = Instance.new("Frame")
        divFrame.Size = UDim2.new(1,0,0,22)
        divFrame.BackgroundTransparency = 1
        divFrame.Parent = PVis
        local divLine = Instance.new("Frame", divFrame)
        divLine.Size = UDim2.new(0.88,0,0,1)
        divLine.Position = UDim2.new(0.06,0,0.5,-1)
        divLine.BackgroundColor3 = UI_ACCENT
        ...
        divLbl.Text = "— U REVERSA —"
        divLbl.TextColor3 = UI_ACCENT
        divLbl.Font = Enum.Font.GothamBold
        divLbl.TextSize = 10
        divLbl.TextXAlignment = Enum.TextXAlignment.Center

        local SHAPES = {
            {id="circle",   label="Circulo Abajo"},
            {id="square",   label="Cuadrado Abajo"},
            {id="triangle", label="Triangulo Abajo"},
            {id="rounded",  label="Cuadrado Bordes Redondeados"},
        }
        for _, shp in ipairs(SHAPES) do
            local r = Instance.new("Frame"); r.ClipsDescendants=true; r.Size=UDim2.new(1,0,0,40)
            r.BackgroundColor3=UI_ROW_BG; r.BackgroundTransparency=0.5; r.BorderSizePixel=0; r.Parent=PVis; uiCardStyle(r)
            local l = Instance.new("TextLabel"); l.Position=UDim2.new(0,13,0,0); l.Size=UDim2.new(0.7,0,1,0)
            l.BackgroundTransparency=1; l.Text=shp.label; l.TextColor3=UI_TEXT_PRIMARY; l.TextSize=13
            l.Font=Enum.Font.GothamMedium; l.TextXAlignment=Enum.TextXAlignment.Left; l.Parent=r
            local dot = Instance.new("Frame"); dot.Size=UDim2.new(0,14,0,14)
            dot.Position=UDim2.new(1,-28,0.5,-7); dot.BorderSizePixel=0
            dot.BackgroundColor3 = (M.buttonShape == shp.id) and UI_ACCENT or Color3.fromRGB(50,50,60)
            dot.Parent = r
            Instance.new("UICorner",dot).CornerRadius = UDim.new(0.5,0)
            local dotStroke = Instance.new("UIStroke",dot); dotStroke.Color=UI_ACCENT; dotStroke.Thickness=1.5; dotStroke.Transparency=0.2
            local function refreshDots() end -- se sobreescribe abajo
            local btn = Instance.new("TextButton",r); btn.Size=UDim2.new(1,0,1,0); btn.BackgroundTransparency=1; btn.Text=""
            local myId = shp.id
            local myDot = dot
            btn.Activated:Connect(function()
                ...
                for _, child in ipairs(PVis:GetDescendants()) do
                    if child.Name == "ShapeDot" then
                        child.BackgroundColor3 = Color3.fromRGB(50,50,60)
                    end
                end
                myDot.BackgroundColor3 = UI_ACCENT
    ...
    -- ==================== GUI MODE ====================
    uiSectionHeader(PVis, "GUI MODE")
    do
        local GUI_MODES = {
            {id="top",        label="Recta Arriba"},
            {id="bottom",     label="Recta Abajo"},
            {id="left_down",  label="Izquierda Abajo"},
            {id="right_down", label="Derecha Abajo"},
        }
        for _, gm in ipairs(GUI_MODES) do
            local r = Instance.new("Frame"); r.ClipsDescendants=true; r.Size=UDim2.new(1,0,0,40)
            r.BackgroundColor3=UI_ROW_BG; r.BackgroundTransparency=0.5; r.BorderSizePixel=0; r.Parent=PVis; uiCardStyle(r)
            local l = Instance.new("TextLabel"); l.Position=UDim2.new(0,13,0,0); l.Size=UDim2.new(0.7,0,1,0)
            l.BackgroundTransparency=1; l.Text=gm.label; l.TextColor3=UI_TEXT_PRIMARY; l.TextSize=13
            l.Font=Enum.Font.GothamMedium; l.TextXAlignment=Enum.TextXAlignment.Left; l.Parent=r
            local dot = Instance.new("Frame"); dot.Size=UDim2.new(0,14,0,14)
            dot.Position=UDim2.new(1,-28,0.5,-7); dot.BorderSizePixel=0
            dot.BackgroundColor3 = (M.guiMode == gm.id) and UI_ACCENT or Color3.fromRGB(50,50,60)
            dot.Name = "GuiModeDot"
            dot.Parent = r
            Instance.new("UICorner",dot).CornerRadius = UDim.new(0.5,0)
            local dotStroke = Instance.new("UIStroke",dot); dotStroke.Color=UI_ACCENT; dotStroke.Thickness=1.5; dotStroke.Transparency=0.2
            ...
            btn.Activated:Connect(function()
                M.guiMode = myId
                -- Actualizar todos los dots de gui mode
                for _, child in ipairs(PVis:GetDescendants()) do
                    if child.Name == "GuiModeDot" then
                        child.BackgroundColor3 = Color3.fromRGB(50,50,60)
                    end
                end
                myDot.BackgroundColor3 = UI_ACCENT
    ...
    uiSectionHeader(PVis, "INTERFACE SIZE")
    local _, btnSzBoxVis = uiNumberRow(PVis, "Button Size", M.mobileButtonsSize, 40, 150, function(v)
        M.mobileButtonsSize = v
        if M.mobileButtonsEnabled then M.buildMobileButtons() end
    end)

    local _, menuScaleBoxVis = uiNumberRow(PVis, "Menu Scale", M.uiScale, 0.5, 2.0, function(v)
        M.uiScale = v
    ...
    local _, setAntiTpBat = uiToggleRow(PUtil, "Anti TP Bat", M.antiTpBatEnabled, function(on)
        M.antiTpBatEnabled = on
        if on then M.startAntiTpBat() else M.stopAntiTpBat() end
        saveCherryConfig()
    end)
    M.setAntiTpBatVisual = setAntiTpBat
    ...
    do
        local r = Instance.new("Frame"); r.ClipsDescendants=true; r.Size=UDim2.new(1,0,0,46)
        r.BackgroundColor3=UI_ROW_BG; r.BackgroundTransparency=0.5; r.BorderSizePixel=0; r.Parent=PUtil; uiCardStyle(r)
        local l = Instance.new("TextLabel"); l.Position=UDim2.new(0,14,0,0); l.Size=UDim2.new(1,-74,1,0)
    ...
    if M.setAntiDieVisual then M.setAntiDieVisual(M.antiDieEnabled) end
        if M.setSafeModeVisual then M.setSafeModeVisual(M.safeModeEnabled) end
        if M.setAntiTpBatVisual then M.setAntiTpBatVisual(M.antiTpBatEnabled) end
...
function M.applyStealBarTheme(accentColor)
    local col = accentColor or UI_ACCENT or CHERRY_ACCENT or Color3.fromRGB(255, 45, 55)
    if M.statusFill then
    ...
    if M.statusStroke then
        M.statusStroke.Color = col
    end
    if M.statusBanner and not M.isStealing then
        M.statusBanner.BackgroundColor3 = col
    end
end
...
task.wait(0.5)
loadCherryConfig()
-- STICK DUELS: tema ROJO + fondo
CherryConfig.Theme = "Red"
M.colorScheme = "Red"
M._savedTheme = "Red"
if not M.customBgId or M.customBgId == 0 then
    M.customBgId = 79999011724260
end
if M.customBgOpacity == nil then
    M.customBgOpacity = 0
end
applyAccentFromTheme()
pcall(saveCherryConfig)
...
if M.antiTpBatEnabled then M.startAntiTpBat() end
...
-- ============================================================
-- SPEED: WhaleHub LinearVelocity Plane + safe carry (no drop brainrot)
-- ============================================================
M._whaleAtt = nil
M._whaleLV = nil
M._whaleMode = nil
M.CARRY_SAFE_SPEED = 16 -- tope al llevar brainrot (el juego suelta si vas mas rapido)
...
local function destroySpeedObjects()
    ...
    if M._whaleLV then pcall(function() M._whaleLV:Destroy() end); M._whaleLV = nil end
    if M._whaleAtt then pcall(function() M._whaleAtt:Destroy() end); M._whaleAtt = nil end
    M._whaleMode = nil
end

local function ensureWhaleLV(hrp, mode)
    mode = mode or "plane"
    if M._whaleLV and M._whaleLV.Parent == hrp and M._whaleAtt and M._whaleAtt.Parent == hrp and M._whaleMode == mode then
        return M._whaleLV
    end
    if M._whaleLV then pcall(function() M._whaleLV:Destroy() end); M._whaleLV = nil end
    if M._whaleAtt then pcall(function() M._whaleAtt:Destroy() end); M._whaleAtt = nil end
    local att = Instance.new("Attachment")
    att.Name = "StickWhaleAtt"
    att.Parent = hrp
    M._whaleAtt = att
    local lv = Instance.new("LinearVelocity")
    lv.Name = "StickWhaleLV"
    lv.Attachment0 = att
...
function M.applyWhaleSpeedXZ(hrp, dir, spd)
    if not hrp or not hrp.Parent then return end
    local lv = ensureWhaleLV(hrp, "plane")
...
function M.applyWhaleSpeed3D(hrp, dir, spd)
    if not hrp or not hrp.Parent then return end
    local lv = ensureWhaleLV(hrp, "vector")
    if not lv then return end
    if dir and dir.Magnitude > 0.01 then
        lv.VectorVelocity = dir.Unit * spd
    else
        lv.VectorVelocity = Vector3.zero
    end
end

function M.stopWhaleSpeed()
    destroySpeedObjects()
end

-- Anti-drop: real speed on HRP, spoof ~16 so server/game does not drop brainrot
local function applyAntiDropSpeed(hrp, hum, dir, spd)
    destroySpeedObjects()
    if not hrp or not hrp.Parent then return end
    local y = 0
    pcall(function()
        -- read raw Y without going through spoof if possible
        y = hrp.AssemblyLinearVelocity.Y
    end)
    if dir and dir.Magnitude > 0.05 then
        ...
        local realSpd = tonumber(spd) or 16
        local spoof = Vector3.new(unit.X * 16, y, unit.Z * 16)
        local real = Vector3.new(unit.X * realSpd, y, unit.Z * realSpd)
        if M._antiDrop and M._antiDrop.setSpoof then
            M._antiDrop.setSpoof(spoof)
        end
        M._spoofedVelocity = spoof
        pcall(function()
            if hrp.SetNetworkOwner then hrp:SetNetworkOwner(player) end
        end)
        -- set real velocity (our script is checkcaller / direct write after hook stores spoof on newindex from game;
        -- we need real write: use raw if available)
        pcall(function()
            hrp.AssemblyLinearVelocity = real
        end)
        -- force spoof to stay at 16 for external readers
        if M._antiDrop and M._antiDrop.setSpoof then
            M._antiDrop.setSpoof(spoof)
        end
        M._spoofedVelocity = spoof
        ...
        local spoof = Vector3.new(0, y, 0)
        if M._antiDrop and M._antiDrop.setSpoof then M._antiDrop.setSpoof(spoof) end
        M._spoofedVelocity = spoof
    end
end

-- Fallback safe carry if hook failed
local function applyCarrySafe(hrp, hum, dir, spd)
    if M._antiDrop and M._antiDrop.hookOk then
        applyAntiDropSpeed(hrp, hum, dir, spd)
        return
    end
    destroySpeedObjects()
    local cap = tonumber(M.CARRY_SAFE_SPEED) or 16
    local safe = math.min(spd or cap, cap)
    local v = hrp.AssemblyLinearVelocity
    local horiz = Vector3.new(v.X, 0, v.Z)
    if horiz.Magnitude > safe + 0.5 then
        if horiz.Magnitude > 0.01 then
            local c = horiz.Unit * safe
            hrp.AssemblyLinearVelocity = Vector3.new(c.X, v.Y, c.Z)
        else
            hrp.AssemblyLinearVelocity = Vector3.new(0, v.Y, 0)
        end
    end
    if hum and hum.WalkSpeed ~= safe then
        hum.WalkSpeed = safe
    end
end

local function applySpeedMethod(hrp, hum, dir, spd, dt)
    M.applyWhaleSpeedXZ(hrp, dir, spd)
end
...
do
    ...
    local _spdCacheT = 0
    RunService.Heartbeat:Connect(function(dt)
        if M.autoBatEnabled or M.autoLeftEnabled or M.autoRightEnabled or M.bypassAimbotEnabled then
            -- Quitar LinearVelocity residual: si queda activo bloquea AssemblyLinearVelocity del auto path
            if M.autoLeftEnabled or M.autoRightEnabled then
                pcall(function()
                    if M.stopWhaleSpeed then M.stopWhaleSpeed() end
                end)
        ...
        if M.updateAutoSwitchSpeed then M.updateAutoSwitchSpeed() end

        local holding = M.hasBrainrotInHand and M.hasBrainrotInHand()

        local spd
        if M.laggerCarryActive then
            spd = M.LAGGER_CARRY_SPEED or 24.5
        elseif M.laggerModeEnabled then
            spd = M.LAGGER_SPEED or 15
        elseif M.carrySpeedActive or holding then
            spd = M.CS or 30
        else
            spd = M.NS or 60
        end
        ...
        if md.Magnitude > 0.1 then
            M.lastMoveDir = md
            dir = md
        elseif M.antiRagdollEnabled and M.lastMoveDir and M.lastMoveDir.Magnitude > 0 then
            local anyHeld = false
        ...
        if holding then
            -- Anti-drop always on: move at configured speed, spoof velocity ~16 to avoid drop
            if dir.Magnitude > 0.1 then
                applyAntiDropSpeed(hrp, hum, dir, spd)
            else
                destroySpeedObjects()
                applyAntiDropSpeed(hrp, hum, Vector3.zero, 0)
            end
        else
            if dir.Magnitude > 0.1 then
                M.applyWhaleSpeedXZ(hrp, dir, spd)
            else
                M.applyWhaleSpeedXZ(hrp, Vector3.zero, 0)
            end
        end
    end)
end
...
-- PreSimulation anti-drop (keeps spoof + real speed in sync while holding brainrot)
do
    local rs = RunService
    local ev = rs.PreSimulation or rs.Heartbeat
    ev:Connect(function()
        if not (M._antiDropAlwaysOn or (M._antiDrop and M._antiDrop.isEnabled and M._antiDrop.isEnabled())) then return end
        local char = player.Character
        if not char then return end
        if not (M.hasBrainrotInHand and M.hasBrainrotInHand()) then return end
        if M.autoBatEnabled or M.autoLeftEnabled or M.autoRightEnabled or M.bypassAimbotEnabled then return end
        local hum = char:FindFirstChildOfClass("Humanoid")
        local hrp = char:FindFirstChild("HumanoidRootPart")
        ...
        if md.Magnitude > 0.05 then
            applyAntiDropSpeed(hrp, hum, md, spd)
        else
            applyAntiDropSpeed(hrp, hum, Vector3.zero, 0)
        end
    end)
