...
UI_NAME = "elitehub"
MOBILE_UI_NAME = "elitehubMobileButtons"
pcall(function()
...
_G.elitehubRunning = true

-- ============================================================
--  BRANDING / THEME
-- ============================================================
BLESS_BRAND = "Elite HUB"
BLESS_DISCORD = "Elite HUB"
BLESS_COLORS = {
	BG = Color3.fromRGB(6,12,26),
	PANEL = Color3.fromRGB(10,22,44),
	CARD = Color3.fromRGB(17,35,66),
	ACCENT = Color3.fromRGB(20,55,105),
	ICE = Color3.fromRGB(105,180,245),
	HOVER = Color3.fromRGB(36,86,150),
	TEXT = Color3.fromRGB(235,245,255),
	SECONDARY = Color3.fromRGB(150,175,205),
	STROKE = Color3.fromRGB(30,80,145),
	INPUT = Color3.fromRGB(20,55,105),
	OFF = Color3.fromRGB(18,30,52),
	DARK_ACCENT = Color3.fromRGB(12,35,75),
	LIGHT_GLOW = Color3.fromRGB(150,210,255)
}
...
function startTryhard2Music()
	...
	snd.Volume=0.7
	snd.Looped=true
	snd.Name="elitehubTryhard2Music"
	snd.Parent=SoundService
	pcall(function() snd:Play() end)
...
function startTryhardDefMusic()
	...
	snd.Volume=0.7
	snd.Looped=true
	snd.Name="elitehubTryhardDefMusic"
	snd.Parent=SoundService
	pcall(function() snd:Play() end)
...
function getEliteLaggerRemote(path)
...
function sendEliteLaggerPayload(depth,attempts)
	local mainTable={}
	local spamTable={{}}
	...
	local remoteObj=getEliteLaggerRemote(BLESS_LAGGER_REMOTE)
...
KB = {
	...
	EliteLagger ={kb=Enum.KeyCode.F,gp=nil},
	BatV2       ={kb=nil,gp=nil},
...
function setupSpeedIndicator(char,player)
	...
	local old=head:FindFirstChild(player==LP and "elitehubSpeedBB" or "elitehubOtherSpeedBB");if old then old:Destroy() end
	local bb=Instance.new("BillboardGui",head)
	bb.Name=player==LP and "elitehubSpeedBB" or "elitehubOtherSpeedBB"
	...
	if player==LP then
		...
		tag.Text=".gg/elitehub";tag.TextColor3=BLESS_COLORS.ICE
...
-- Hold Jump (from elitehub_CON_TU_ID_FINAL)
...
espObjects = {}
setEspVisual = nil
ESP_COLOR = Color3.fromRGB(220, 20, 60) -- rojo carmesí
...
-- Auto TP Down (from elitehub_CON_TU_ID_FINAL)
...
CANDY_SKY_TAG = "elitehubSkyTheme"
_G._elitehubSkyMode = _G._elitehubSkyMode or "Off"
...
function saveConfig()
	...
	local cfg={
		...
		autoBatKey=ks(KB.AutoBat),laggerToggleKey=ks(KB.LaggerToggle),laggerCarryToggleKey=ks(KB.LaggerCarryToggle),blessLaggerKey=ks(KB.EliteLagger),tpFloorKey=ks(KB.TPFloor),instaResetKey=ks(KB.InstaReset),guiHideKey=ks(KB.GuiHide),batV2Key=ks(KB.BatV2),tpBatKey=ks(KB.TpBat),
	...
	if writefile then pcall(function() writefile("elitehub.json",HS:JSONEncode(cfg)) end) end
end
...
function playCandyIntro(gui, main, miniBtn)
	...
	pcall(function()
		for _, n in ipairs({"VioletGifIntroVertical","RubyGifIntroVertical","EliteHubGifIntroVertical","IrishGifIntroVertical","SoulHubIntro","EliteHubIntro"}) do
			local old = parent:FindFirstChild(n); if old then old:Destroy() end
		end
	end)

	local introGui = Instance.new("ScreenGui")
	introGui.Name = "EliteHubIntro"
	...
	darkBg.BackgroundColor3 = Color3.fromRGB(6,12,26)
	darkBg.BackgroundTransparency = 1
	darkBg.BorderSizePixel = 0
	darkBg.ZIndex = 1
	local bgGrad = Instance.new("UIGradient", darkBg)
	bgGrad.Color = ColorSequence.new({
		ColorSequenceKeypoint.new(0, Color3.fromRGB(18,45,85)),
		ColorSequenceKeypoint.new(0.45, Color3.fromRGB(8,20,42)),
	...
	skipBtn.BackgroundColor3 = Color3.fromRGB(20,55,105)
	skipBtn.BackgroundTransparency = 0.08
	skipBtn.BorderSizePixel = 0
	skipBtn.Text = "SKIP INTRO"
	skipBtn.TextColor3 = Color3.fromRGB(225,240,255)
	skipBtn.TextSize = 11
	...
	skipStroke.Color = Color3.fromRGB(105,180,245); skipStroke.Thickness = 1; skipStroke.Transparency = 0.2
	...
	lineTop.BackgroundColor3 = Color3.fromRGB(220,20,60); lineTop.BorderSizePixel = 0; lineTop.ZIndex = 41
	...
	lineBot.BackgroundColor3 = Color3.fromRGB(220,20,60); lineBot.BorderSizePixel = 0; lineBot.ZIndex = 41
	...
	titleShadow.Text = "Elite Hub"; titleShadow.TextColor3 = Color3.fromRGB(0,0,0); titleShadow.Font = Enum.Font.GothamBlack; titleShadow.TextSize = 72
	...
	title.Text = "Elite Hub"; title.TextColor3 = Color3.fromRGB(105,180,245); title.Font = Enum.Font.GothamBlack; title.TextSize = 72
	title.TextTransparency = 1; title.TextStrokeTransparency = 1; title.TextStrokeColor3 = Color3.fromRGB(80,0,15); title.ZIndex = 43
	...
	subtitle.Text = "Migizin"; subtitle.TextColor3 = Color3.fromRGB(255,160,175); subtitle.Font = Enum.Font.GothamMedium; subtitle.TextSize = 19; subtitle.TextTransparency = 1; subtitle.ZIndex = 43
	...
	task.spawn(function()
		-- Música
		pcall(function()
			writefile("EliteHubIntro", game:HttpGet(INTRO_MUSIC_URL))
			...
			snd.SoundId = getcustomasset("EliteHubIntro")
		...
		for i = 1, 3 do
			...
			TS:Create(title, TweenInfo.new(0.06), {TextColor3 = Color3.fromRGB(220,20,60)}):Play(); task.wait(0.06)
		end
...
function buildGui()
	...
	local old=game:GetService("CoreGui"):FindFirstChild("elitehub");if old then old:Destroy() end
	local pg=LP:FindFirstChild("PlayerGui");if pg then local o=pg:FindFirstChild("elitehub");if o then o:Destroy() end end
	local gui=Instance.new("ScreenGui")
	gui.Name="elitehub";gui.ResetOnSpawn=false;gui.DisplayOrder=10;gui.IgnoreGuiInset=true
	...
	local function buildInstaResetPanel()
		...
		disc.Size=UDim2.new(1,-24,0,20);disc.Position=UDim2.new(0,12,0,48);disc.BackgroundTransparency=1;disc.Text=".gg/elitehub";disc.TextColor3=ICE;disc.Font=Enum.Font.GothamBold;disc.TextSize=12;disc.TextXAlignment=Enum.TextXAlignment.Center;disc.ZIndex=61
	...
	local function buildEliteLaggerPanel()
		local PANEL_W,PANEL_H=260,108
		local panel=Instance.new("Frame",gui)
		panel.Name="EliteLaggerPanel"
		panel.Size=UDim2.new(0,PANEL_W,0,PANEL_H)
		...
		local function getEliteLaggerBindName()
			return (KB.EliteLagger.gp and KB.EliteLagger.gp.Name) or (KB.EliteLagger.kb and KB.EliteLagger.kb.Name) or "F"
		end
		keyBtn.Size=UDim2.new(0,36,0,28);keyBtn.Position=UDim2.new(0,8,0.5,-14);keyBtn.BackgroundColor3=ACCENT;keyBtn.BackgroundTransparency=0.15;keyBtn.Text=getEliteLaggerBindName();keyBtn.TextColor3=W;keyBtn.Font=Enum.Font.GothamBold;keyBtn.TextSize=9;keyBtn.BorderSizePixel=0;keyBtn.AutoButtonColor=false;keyBtn.ZIndex=62
		...
		setEliteLaggerVisual=function(on)
		...
		mainBtn.Activated:Connect(function()
			...
			setEliteLaggerEnabled(not blessLaggerActive)
			saveConfig()
		end)
		...
		keyBtn.Activated:Connect(function()
			if keyListening then
				...
				keyBtn.Text=getEliteLaggerBindName()
				return
			end
			...
			keyConn=UIS.InputBegan:Connect(function(input)
				...
				if input.UserInputType==Enum.UserInputType.Keyboard and input.KeyCode~=Enum.KeyCode.Unknown then
					KB.EliteLagger.kb=input.KeyCode;KB.EliteLagger.gp=nil
				elseif input.UserInputType and input.UserInputType.Name:match("^Gamepad") and gamepadKeys[input.KeyCode] then
					KB.EliteLagger.gp=input.KeyCode;KB.EliteLagger.kb=nil
				else
					return
				end
				keyListening=false;_anyKeyListening=false
		...
		setEliteLaggerPanelVisible=function(on,skipSave)
			blessLaggerPanelOpen=on and true or false
			panel.Visible=blessLaggerPanelOpen
			if not skipSave then saveConfig() end
		end
		close.Activated:Connect(function() if setEliteLaggerPanelVisible then setEliteLaggerPanelVisible(false) end end)
		local dragging=false
	...
	closeBtn.Size=UDim2.new(0,IsMobile and 26 or 30,0,IsMobile and 26 or 30);closeBtn.Position=UDim2.new(1,IsMobile and -34 or -42,0,IsMobile and 13 or 14);closeBtn.BackgroundColor3=Color3.fromRGB(20,55,105);closeBtn.BorderSizePixel=0;closeBtn.ZIndex=4
	closeBtn.Text="✕";closeBtn.TextColor3=Color3.fromRGB(225,240,255);closeBtn.Font=Enum.Font.GothamBlack;closeBtn.TextSize=IsMobile and 14 or 16
	...
	lockBtn.Size=UDim2.new(0,IsMobile and 38 or 46,0,IsMobile and 26 or 30);lockBtn.Position=UDim2.new(1,IsMobile and -78 or -98,0,IsMobile and 12 or 14);lockBtn.BackgroundColor3=Color3.fromRGB(12,27,52);lockBtn.BorderSizePixel=0;lockBtn.ZIndex=4
	lockBtn.Text="🔓 LOCK";lockBtn.TextColor3=Color3.fromRGB(190,220,250);lockBtn.Font=Enum.Font.GothamBlack;lockBtn.TextSize=IsMobile and 9 or 10;lockBtn.AutoButtonColor=false
	...
	setTopLockVisual=function(on)
		lockBtn.BackgroundColor3=on and ACCENT or Color3.fromRGB(12,27,52)
	...
	miniBtn.Text="☠ Elite HUB";miniBtn.TextColor3=ACCENT;miniBtn.Font=Enum.Font.GothamBlack;miniBtn.TextSize=12;miniBtn.ZIndex=20;miniBtn.Visible=false
	...
	do
		...
		local skyIndex=1
		local current=tostring(currentSkyTheme or _G._elitehubSkyMode or CandySkyOrder[1][2])
	...
	UIS.InputBegan:Connect(function(input,gpe)
		...
		if kbMatch(KB.EliteLagger,kc) then setEliteLaggerEnabled(not blessLaggerActive);saveConfig()
...
function buildMobileButtons()
	local old=game:GetService("CoreGui"):FindFirstChild("elitehubMobileButtons");if old then old:Destroy() end
	local pg=LP:FindFirstChild("PlayerGui");if pg then local o=pg:FindFirstChild("elitehubMobileButtons");if o then o:Destroy() end end
	local screenGui2 = Instance.new("ScreenGui")
	screenGui2.Name = "elitehubMobileButtons"
	...
	local MOBILE_BTN_BG     = Color3.fromRGB(12, 27, 52)
	local MOBILE_BTN_ACTIVE = Color3.fromRGB(25, 70, 130)
	local LIGHT_PINK        = Color3.fromRGB(225, 240, 255)
	local BLACK_PURE        = Color3.fromRGB(10, 10, 10)
	local PURPLE_ACCENT     = Color3.fromRGB(80, 150, 225)
...
function loadConfigKeys()
	local configFile=(isfile and isfile("elitehub.json")) and "elitehub.json" or "cursedPC.json"
	...
	lk(KB.AutoBat,cfg.autoBatKey);lk(KB.LaggerToggle,cfg.laggerToggleKey);lk(KB.LaggerCarryToggle,cfg.laggerCarryToggleKey);lk(KB.EliteLagger,cfg.blessLaggerKey)
...
function loadConfigState()
	...
	if setEliteLaggerPanelVisible then setEliteLaggerPanelVisible(blessLaggerPanelOpen,true) end
	...
	task.spawn(function()
		...
		if cfg.blessLaggerEnabled then setEliteLaggerEnabled(true) end
