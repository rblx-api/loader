...
function M.buildStatusUI()
    -- STICK HUB AUTO GRAB style UI (orange theme) — shown only when Auto Steal is on
    ...
    local ORANGE_BRIGHT = Color3.fromRGB(255, 165, 40)
    local ORANGE_DARK = Color3.fromRGB(90, 45, 0)
    local ORANGE_MID = Color3.fromRGB(180, 90, 0)
    local ORANGE_FILL = Color3.fromRGB(255, 120, 20)
    ...
    TextLabel.Text = "STICK HUB AUTO GRAB | Ping: 0ms | FPS: 0"
    ...
    M.statusPerfConn = RunService.RenderStepped:Connect(function()
        ...
        if now - lastTick >= 1 then
            ...
            M.statusFpsLbl.Text = "STICK HUB AUTO GRAB | Ping: " .. ping .. "ms | FPS: " .. fps
        end
    end)
...
-- ============================================================
-- AUTO STEAL V1/V2 — logic from grab.lua (Irish Hub style)
-- ============================================================
...
-- grab.lua: find nearest Steal proximity prompt within radius
            ...
            if d <= radius and d < dist then
                ...
                if att then
                    for _, p in ipairs(att:GetChildren()) do
                        if p:IsA("ProximityPrompt") and p.ActionText and tostring(p.ActionText):find("Steal") then
                            nearest, dist = p, d
                        end
                    end
                end
...
local function setGrabBannerStealing(on)
    if M.statusBanner then
        if on then
            M.statusBanner.BackgroundColor3 = Color3.fromRGB(255, 165, 40)
            M.statusBanner.BackgroundTransparency = 0.3
        else
            M.statusBanner.BackgroundColor3 = Color3.fromRGB(255, 140, 0)
            M.statusBanner.BackgroundTransparency = 0.8
        end
    end
    if M.statusStroke then
        M.statusStroke.Color = on and Color3.fromRGB(255, 165, 40) or Color3.fromRGB(255, 140, 0)
    end
end
...
local function executeStealGrab(prompt)
    ...
    if not M.StealData[prompt] then
        M.StealData[prompt] = {hold = {}, trigger = {}, ready = true}
        if getconnections then
            for _, c in ipairs(getconnections(prompt.PromptButtonHoldBegan)) do
                if c.Function then table.insert(M.StealData[prompt].hold, c.Function) end
            end
            for _, c in ipairs(getconnections(prompt.Triggered)) do
                if c.Function then table.insert(M.StealData[prompt].trigger, c.Function) end
            end
        end
    end
...
function M.startNormalSteal()
    ...
    M.stealConn = RunService.Heartbeat:Connect(function()
        ...
        -- V1 and V2 both use this grab.lua logic
        ...
        local success, prompt = pcall(findNearestPromptGrab)
        if success and prompt then
            ...
            if M._grabCurrentStatus ~= "UNREADY" and not M.isStealing then
                ...
                if M.statusStroke then
                    M.statusStroke.Color = Color3.fromRGB(90, 45, 0)
                end
            end
        end
    end)
...
function M.startAutoLeft()
    ...
    M.alConn = RunService.Heartbeat:Connect(function()
        ...
        local spd = 60 -- fixed like auto left y right.lua
        if M.alPhase == 1 then
...
function M.buildMobileButtons()
    ...
    for i, def in ipairs(btnDefs) do
        ...
        btn.Text = label
        btn.TextColor3 = Color3.fromRGB(255, 170, 50) -- Naranja apagado
        btn.TextSize = 10
        ...
        if bShape == "triangle" then
            ...
            do
                ...
                sqSt.Color = Color3.fromRGB(255, 170, 50)
        ...
        btn.TextStrokeColor3 = Color3.fromRGB(255, 170, 50)
        ...
        local function setOn(v)
            ...
            if v then
                ...
                if not triSq then
                    TweenService:Create(btn, TweenInfo.new(0.15), {TextColor3 = Color3.fromRGB(255, 170, 50)}):Play()
                else
...
function M.applyCustomBackground(frame)
    ...
    bgImg.BackgroundColor3 = Color3.fromRGB(10, 10, 12)
    bgImg.BackgroundTransparency = 0
    bgImg.Image = "rbxassetid://107421418862818"
...
function M.buildGui()
    ...
    MinBtn.TextColor3=Color3.fromRGB(255,140,0); MinBtn.TextSize=13; MinBtn.Font=Enum.Font.GothamBold
    MinBtn.AutoButtonColor=false; MinBtn.Parent=Header
    Instance.new("UICorner",MinBtn).CornerRadius = UDim.new(0,6)
    do local minBtnStroke = Instance.new("UIStroke", MinBtn); minBtnStroke.Color=Color3.fromRGB(255,140,0); minBtnStroke.Thickness=1.5; minBtnStroke.Transparency=0.1 end
    MinBtn.MouseButton1Down:Connect(function() TweenService:Create(MinBtn, UI_TWEEN_FAST, {BackgroundColor3 = Color3.fromRGB(255,140,0)}):Play() end)
    ...
    MinPill.Visible=true; MinPill.Active=true; MinPill.ZIndex=200
    MinPill.AnchorPoint = Vector2.new(0, 0.5)
    MinPill.Position = UDim2.new(0, 8, 0.5, 0)
    MinPill.Size = UDim2.new(0, 80, 0, 36)
    MinPill.BackgroundColor3 = Color3.fromRGB(20, 16, 10)
    MinPill.BackgroundTransparency = 0
    MinPill.BorderSizePixel = 0
    MinPill.Parent = gui
    Instance.new("UICorner", MinPill).CornerRadius = UDim.new(0, 10)
    do
        local pst = Instance.new("UIStroke")
        pst.Color = Color3.fromRGB(255, 160, 0)
        pst.Thickness = 2.5
        pst.Transparency = 0
        pst.Parent = MinPill
        local glow = Instance.new("UIStroke")
    ...
    do
        ...
        b.Text = "STICK DUELS"
        b.TextColor3 = Color3.fromRGB(255, 180, 0)
        b.TextStrokeColor3 = Color3.fromRGB(255, 100, 0)
...
function M.applyStealBarTheme(accentColor)
    local col = accentColor or UI_ACCENT or CHERRY_ACCENT or Color3.fromRGB(255, 140, 0)
    if M.statusFill then
...
-- Al llevar brainrot: SIN LinearVelocity (el juego suelta el tool). Solo WalkSpeed bajo.
local function applyCarrySafe(hrp, hum, dir, spd)
    ...
    -- cortar velocidad residual alta
...
do
    ...
    RunService.Heartbeat:Connect(function(dt)
        ...
        if holding then
            -- CRITICO: no LinearVelocity / no impulse mientras llevas brainrot
            if dir.Magnitude > 0.1 then
                applyCarrySafe(hrp, hum, dir, spd)
            else
                destroySpeedObjects()
                local v = hrp.AssemblyLinearVelocity
