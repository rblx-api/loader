...
local State = {
    ...
    lajaMusicEnabled = Config.lajaMusicEnabled or false,
}

STEAL_RADIUS = Config.grabRadius or 80
STEAL_DURATION = Config.stealDuration or 2.6

local function startFloat() end
local function stopFloat() end
...
local Hitbux = {}
local HITBUX_COLOR = Color3.fromRGB(255, 0, 0)
...
task.spawn(function()
    ...
    local PALETTE = {
        ...
        IdleGrad = ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.fromRGB(225, 29, 72)),
...
function createWaypointMarker()
    ...
    local marker = Instance.new("Part"); marker.Name="KzWaypoint"; marker.Size=Vector3.new(2.5,2.5,2.5); marker.Shape=Enum.PartType.Ball; marker.Anchored=true; marker.CanCollide=false; marker.CastShadow=false; marker.Material=Enum.Material.Neon; marker.Color=Color3.fromRGB(255,0,0); marker.Transparency=0.15; marker.Parent=workspace
...
function createAntiRagdollButton()
    ...
    toggleButton.MouseButton1Click:Connect(function()
        ...
        toggleButton.TextColor3 = isOn and Color3.fromRGB(0, 255, 140) or Color3.fromRGB(255, 255, 255)
        uiStroke.Color = isOn and Color3.fromRGB(0, 255, 140) or Color3.fromRGB(255, 0, 0)
    end)
...
local DARK_ACC = Color3.fromRGB(200, 0, 0)
...
local C_ON_BG = Color3.fromRGB(220, 30, 55)
...
mini.BorderSizePixel=0; mini.Text="CATALYST"; mini.TextColor3=ACCENT; mini.Font=Enum.Font.GothamBlack; mini.TextSize=12; mini.Visible=false
...
function makeDropActionRow(label, parent)
    ...
    actionBtn.MouseButton1Click:Connect(function()
        if _G.stopTpBat then _G.stopTpBat() end
        runDropBrainrot()
        TweenService:Create(actionBtn,TweenInfo.new(0.08),{BackgroundColor3=Color3.fromRGB(150,0,0)}):Play()
...
function buildFullGUI()
            ...
            if sg then
                ...
                if btn then
                    if isOn then
                        ...
                        if stroke then stroke.Color = Color3.fromRGB(0, 255, 140) end
                    else
    ...
    -- Toggle para Laja (música a todo volumen)
    local lajaToggleSetter = makeToggleRow("Laja", State.lajaMusicEnabled, function(on)
        State.lajaMusicEnabled = on
        SetSetting("lajaMusicEnabled", on)
        if on then
            if not _G.LajaSound then
                local sound = Instance.new("Sound")
                sound.SoundId = "https://file.garden/algLafWA1jk8WMfK/LAJA%20-%20NADIE%20TA%20FRIO%20(Letra)(MP3_160K).mp3"
                sound.Looped = true
                sound.Volume = 1
                sound.Parent = game:GetService("SoundService") or LP.Character or workspace
                _G.LajaSound = sound
            end
            if _G.LajaSound then
                _G.LajaSound.Volume = 1
                _G.LajaSound:Play()
            end
        else
            if _G.LajaSound then
    ...
    _G.lajaToggleSetter = lajaToggleSetter
    if not _G.LajaSound then
        local sound = Instance.new("Sound")
        sound.SoundId = "https://file.garden/algLafWA1jk8WMfK/LAJA%20-%20NADIE%20TA%20FRIO%20(Letra)(MP3_160K).mp3"
        sound.Looped = true
    ...
    if State.lajaMusicEnabled and _G.LajaSound then
        _G.LajaSound.Volume = 1
        _G.LajaSound:Play()
    end
    ...
    resetBtn.MouseButton1Click:Connect(function()
        ...
        if _G.lajaToggleSetter then _G.lajaToggleSetter(false) end
...
local Q_ON  = Color3.fromRGB(255, 0, 0)
local Q_BORDER = Color3.fromRGB(255,0,0)
local Q_BORDER_ON = Color3.fromRGB(255,0,0)
local Q_TEXT = Color3.fromRGB(255, 0, 0)
...
function setupChar(char)
    ...
    if head then
        ...
        discordLbl.Text = "GHOX DUELS"
        discordLbl.TextColor3 = Color3.fromRGB(255, 0, 0)
...
function loadConfigState()
    ...
    if type(cfg.lajaMusicEnabled)=="boolean" then
        State.lajaMusicEnabled = cfg.lajaMusicEnabled
        if State.lajaMusicEnabled then
            task.spawn(function()
                task.wait(0.3)
                if not _G.LajaSound then
                    local sound = Instance.new("Sound")
                ...
                if _G.lajaToggleSetter then
                    _G.lajaToggleSetter(true)
                end
            end)
        else
            if _G.LajaSound then
