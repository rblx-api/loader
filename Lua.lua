-- ============================================================
--  SURE HUB v2 (Basado en Clean Duels)
--  Con Auto Steal, ESP Player, Animation Selector
--  Color Azul (estilo Surehub)
--  Menú con pestañas: SPEED, OTHER, CONFIG, KEYBINDS
--  Animaciones: Mago (antes Tryhard), Crazy eliminada
--  INSTA RESET ELIMINADO COMPLETAMENTE
-- ============================================================
if _G.BloodHoundsRunning then return end _G.BloodHoundsRunning = true
...
local function isGamepad(input)
    ...
    return uit == Enum.UserInputType.Gamepad1 or uit == Enum.UserInputType.Gamepad2 or
           uit == Enum.UserInputType.Gamepad3 or uit == Enum.UserInputType.Gamepad4 or
           uit == Enum.UserInputType.Gamepad5 or uit == Enum.UserInputType.Gamepad6 or
           uit == Enum.UserInputType.Gamepad7 or uit == Enum.UserInputType.Gamepad8
end

UIS.InputBegan:Connect(function(inp)
    if inp.KeyCode ~= Enum.KeyCode.ButtonA or not isGamepad(inp) then return end
    _gamepadBtnHeld = true
    if InfJumpState.enabled and InfJumpState.mode == "manual" then _doInfJump() end
end)
...
-- ============================================================
--  ANIMACIONES MODIFICADAS: Mago (antes Tryhard), Crazy eliminada
--  Nuevas: Zombie, Adidas Aura, Vampire, Amazon Unboxed
-- ============================================================
local ANIM_PACKS = {
    Mago = {
    ...
    ["Adidas Aura"] = {
        idle1 = "rbxassetid://110211186840347",
...
local ANIM_PACK_ORDER = {
    ...
    {"Mago", "Mago"},
    {"Zombie", "Zombie"},
    {"Adidas Aura", "Adidas Aura"},
    {"Vampire", "Vampire"},
    {"Amazon Unboxed", "Amazon Unboxed"},
}
...
do
    ...
    RunService.Heartbeat:Connect(function()
        ...
        local inRag = st == Enum.HumanoidStateType.Physics or
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LP then
            ...
            if char and char:FindFirstChild("HumanoidRootPart") and
               char:FindFirstChildOfClass("Humanoid") and
               char:FindFirstChildOfClass("Humanoid").Health > 0 then
...
-- ============================================================
--  INSTA RESET (FUNCIÓN CONSERVADA PARA USO INTERNO)
-- ============================================================
local oldFire
...
function instareset(resetType)
    ...
    task.spawn(function()
        ...
        while attempts < 50 do
            ...
            task.wait(0.05)
            attempts = attempts + 1
            if LP.Character ~= oldChar then break end
        end
    end)
end
...
function findBatForCounter()
    ...
    return nil
endfunction swingBatForCounter(bat, character)
...
function setupMovementAndIndicators(char)
    ...
    movementLoop = RunService.RenderStepped:Connect(function()
        ...
        if not autoBatEnabled and not tpLockEnabled and not autoLeftEnabled and
           not autoRightEnabled and not autoBatV2Enabled then
            local spd
            if laggerToggled then
...
local BackgroundDecals = {
    [1] = "rbxassetid://124833425021074",
    [2] = "rbxassetid://88879074569566",
    [3] = "rbxassetid://87126851304571",
}
...
-- ============================================================
--  GUI PRINCIPAL - ESTILO SUREHUB (AZUL)
--  PESTAÑAS: SPEED, OTHER, CONFIG, KEYBINDS
-- ============================================================
function buildGui()
    local SILVER = Color3.fromRGB(180, 180, 190)
    ...
    if pg then
        local o = pg:FindFirstChild("BloodHounds")
    ...
    bgImage.Image = BackgroundDecals[currentBgIndex] or ""
    ...
    showGui = function()
        if main then
            main.Visible = true
            mainUIScale.Scale = 0
            local tween = TS:Create(mainUIScale, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Scale = 1})
            tween:Play()
        end
    ...
    hideGui = function()
        if main then
            local tween = TS:Create(mainUIScale, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Scale = 0})
            tween:Play()
    ...
    local function animPill(pill, dot, on)
        if on then
            TS:Create(pill, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {BackgroundColor3 = Color3.fromRGB(0, 150, 255)}):Play()
            TS:Create(dot, TweenInfo.new(0.2, Enum.EasingStyle.Back), {
                Position = UDim2.new(1, -19, 0.5, -8),
                BackgroundColor3 = WHITE
            }):Play()
        else
            TS:Create(pill, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {BackgroundColor3 = OFF}):Play()
            TS:Create(dot, TweenInfo.new(0.2, Enum.EasingStyle.Back), {
    ...
    local function mkBox(parent, default, w, xOff, cb)
        ...
        tb.Focused:Connect(function()
            TS:Create(bs, TweenInfo.new(0.12), {Color = Color3.fromRGB(0, 150, 255), Transparency = 0}):Play()
        end)
    ...
    local function mkKeyButton(parent, kbEntry)
        ...
        btn.Activated:Connect(function()
            ...
            pv = btn.Text; li = true; _anyKeyListening = true
            listenStart = tick()
    ...
    do
        ...
        laggerBox = mkBox(row, LAGGER_SPEED_1, 50, 56, function(v)
            if v > 0 and v <= 500 then LAGGER_SPEED_1 = v end
        end)
    end
    ...
    do
        ...
        lagger2Box = mkBox(row, LAGGER_SPEED_2, 50, 56, function(v)
            if v > 0 and v <= 500 then LAGGER_SPEED_2 = v end
        end)
    end
    ...
    do
        ...
        batSpeedBox = mkBox(row, BAT_AIMBOT_SPEED, 50, 56, function(v)
            if v > 0 and v <= 200 then BAT_AIMBOT_SPEED = v end
        end)
    end
    ...
    dropBrainrotSetVisual = mkToggle(speedPage, "Drop Brainrot", function(on)
        if on then
    ...
    do
        ...
        dropModeBtnRef = mkBox(row, dropMode == 1 and "Fling" or "Jump Drop", 60, 65, function(v)
            if v == 1 or v == 2 then
                if dropActive then stopDropBrainrot() end
                dropMode = v
                dropModeBtnRef.Text = dropMode == 1 and "Fling" or "Jump Drop"
        ...
        leftBtn.Position = UDim2.new(1, -108, 0.5, -13)
    ...
    mkSect(otherPage, "| DUEL")

    setBatCounterVisual = mkToggle(otherPage, "Bat Counter", function(on)
        batCounterEnabled = on
        if on then startBatCounter() else stopBatCounter() end
    end)

    setMedusaVisual = mkToggle(otherPage, "Medusa Counter", function(on)
        setMedusaCounterState(on)
    end)

    setMedusaAutoResetVisual = mkToggle(otherPage, "Medusa Auto Reset", function(on)
        setMedusaAutoResetState(on)
    end)

    setAntiRagVisual = mkToggle(otherPage, "Anti Ragdoll", function(on)
        setAntiRag(on)
    end)

    bodyLockSetVisual = mkToggle(otherPage, "Body Lock", function(on)
        bodyLockEnabled = on
        if on then
    ...
    setUnwalkVisual = mkToggle(otherPage, "Unwalk", function(on)
        unwalkEnabled = on
        if on then startUnwalk() else stopUnwalk() end
    end)

    setAntiLagVisual = mkToggle(otherPage, "Anti Lag", function(on)
        if on then enableAntiLag() else disableAntiLag() end
    end)

    setNeonWeatherVisual = mkToggle(otherPage, "Neon Weather", function(on)
        toggleNeonWeather(on)
    end)

    mkSect(otherPage, "| Animation")
    ...
    do
        ...
        local leftBtn = Instance.new("TextButton", row)
        leftBtn.Size = UDim2.new(0, 28, 0, 26)
        leftBtn.Position = UDim2.new(1, -120, 0.5, -13)
        leftBtn.BackgroundColor3 = INP
        leftBtn.BackgroundTransparency = 0.7
        leftBtn.BorderSizePixel = 0
        leftBtn.Text = "<"
        ...
        animSelectorLabel = Instance.new("TextLabel", row)
        animSelectorLabel.Size = UDim2.new(0, 80, 0, 26)
        animSelectorLabel.Position = UDim2.new(1, -95, 0.5, -13)
    ...
    setEditModeVisual = mkToggle(configPage, "Edit Button", function(on)
        toggleEditMode(on)
    ...
    setLockUIVisual = mkToggle(configPage, "Lock UI", function(on)
        toggleLockUI(on)
    ...
    mkSect(configPage, "| Stretch")

    local stretchToggleSetter
    stretchToggleSetter = mkToggle(configPage, "Stretch", function(on)
        stretchEnabled = on
    ...
    -- ============================================================
    --  PESTAÑA KEYBINDS (SIN INSTA RESET)
    -- ============================================================
    setActivePage(keybindsPage)
    mkSect(keybindsPage, "Keybinds")

    addKeybindRow(keybindsPage, "Carry Mode", KB.CarryToggle)
    addKeybindRow(keybindsPage, "Lagger Mode", KB.LaggerMode)
    addKeybindRow(keybindsPage, "Auto Left", KB.AutoLeft)
    addKeybindRow(keybindsPage, "Auto Right", KB.AutoRight)
    addKeybindRow(keybindsPage, "Auto Bat", KB.AutoBat)
    addKeybindRow(keybindsPage, "Anti-Desync", KB.TPLock)
    addKeybindRow(keybindsPage, "Bat V2", KB.BatV2)
    addKeybindRow(keybindsPage, "TP Down", KB.TPFloor)
    addKeybindRow(keybindsPage, "Drop Brainrot", KB.DropBrainrot)
    addKeybindRow(keybindsPage, "Hide GUI", KB.GuiHide)
...
function createMobilePanel()
    ...
    local ACTIVE_BG = Color3.fromRGB(0, 150, 255)
        ...
        elseif name == "Carry" then
            callback = function(setActive)
                if not speedMode then
                    speedMode = true; laggerToggled = false; laggerLevel = 1
                    setActive(true)
        ...
        elseif name == "Lagger1" then
            callback = function(setActive)
                ...
                if not laggerToggled or laggerLevel ~= 1 then
                    laggerToggled = true; laggerLevel = 1
                    setActive(true)
                    if buttons.Lagger2 and buttons.Lagger2.setActive then
                        buttons.Lagger2.setActive(false)
                    end
                else
                    laggerToggled = false; laggerLevel = 1
                    setActive(false)
                end
                refreshSpeedModeLabel()
            end
        elseif name == "Lagger2" then
            callback = function(setActive)
                ...
                if not laggerToggled or laggerLevel ~= 2 then
                    laggerToggled = true; laggerLevel = 2
                    setActive(true)
                    if buttons.Lagger1 and buttons.Lagger1.setActive then
                        buttons.Lagger1.setActive(false)
                    end
...
-- ============================================================
--  BOTONES FLOTANTES (SIN INSTA RESET)
-- ============================================================
function createTpBatFloatingButton()
    ...
    btnFrame.BackgroundColor3 = _G.AceAntiDesyncAimbotOn and Color3.fromRGB(0, 150, 255) or Color3.fromRGB(0,0,0)
    ...
    local function setActive(state)
        if state then
            btnFrame.BackgroundColor3 = Color3.fromRGB(0, 150, 255)
            label.TextColor3 = Color3.fromRGB(0,0,0)
        else
            btnFrame.BackgroundColor3 = Color3.fromRGB(0,0,0)
...
function createBatV2FloatingButton()
    ...
    btnFrame.BackgroundColor3 = autoBatV2Enabled and Color3.fromRGB(0, 150, 255) or Color3.fromRGB(0,0,0)
    btnFrame.BackgroundTransparency = 0
    btnFrame.BorderSizePixel = 0
    btnFrame.ZIndex = 20
    Instance.new("UICorner", btnFrame).CornerRadius = UDim.new(0, 18)
    ...
    if kbMatch(KB.TPLock, kc) then
        ...
        if tpBatFloatingButton then
            ...
            if btnFrame then
                ...
                local label = btnFrame:FindFirstChild("TextLabel")
                if label then
                    label.TextColor3 = _G.AceAntiDesyncAimbotOn and Color3.fromRGB(0,0,0) or Color3.fromRGB(180, 180, 190)
                end
            end
        end
        return
...
print("✅ SURE HUB v2 cargado correctamente.")
print("✅ Animaciones: Mago (antes Tryhard), Zombie, Adidas Aura, Vampire, Amazon Unboxed.")
print("✅ Menú con pestañas: SPEED, OTHER, CONFIG, KEYBINDS.")
print("✅ Color azul estilo Surehub.")
print("✅ Insta Reset eliminado completamente.")