...
function isCarryingBrainrot(char)
	...
	for _, child in ipairs(char:GetChildren()) do
		if child:IsA("Model") and child:FindFirstChildWhichIsA("BasePart", true) then
			...
			if _acIsCarryName(child.Name) then return true end
		elseif child:IsA("Tool") and not _acIsIgnoredTool(child.Name) then
			return true
		end
	end
	return false
...
function _acGetCurrentMode()
	if laggerCarryToggled then return "Lagger Carry"
	elseif laggerToggled then return "Lagger"
	elseif speedMode then return "Carry"
	else return "Normal" end
end

function _acSetMode(mode)
	if mode == "Carry" then
	...
	elseif mode == "Lagger" then
		laggerToggled = true; laggerCarryToggled = false; speedMode = false
	else
		laggerToggled = false; laggerCarryToggled = false; speedMode = false
	end
	refreshSpeedModeLabel()
end

function enableCarrySpeedForSteal()
	_acState._waitingForCarryPickup = false
	_acState._carryPickupWatchUntil = 0
	if not _acState._autoCarryFromSteal then
		_acState._autoCarryReturnMode = _acGetCurrentMode()
	end
	_acState._autoCarryFromSteal = true
	_acState._autoCarryGraceUntil = tick() + 0.75
	local wasLagger = (_acState._autoCarryReturnMode == "Lagger" or _acState._autoCarryReturnMode == "Lagger Carry")
	if wasLagger then _acSetMode("Lagger Carry")
	else _acSetMode("Carry") end
end

function disableAutoCarrySpeed()
	if not _acState._autoCarryFromSteal and not _acState._waitingForCarryPickup then return end
	local wasAutoApplied = _acState._autoCarryFromSteal == true
	local returnMode = _acState._autoCarryReturnMode
	_acState._autoCarryFromSteal = false
	_acState._waitingForCarryPickup = false
	_acState._autoCarryGraceUntil = 0
	_acState._carryPickupWatchUntil = 0
	_acState._autoCarryReturnMode = nil
	if not wasAutoApplied then return end
	_acSetMode(returnMode or "Normal")
end

function startAutoCarryPickupWatch(seconds)
	if not autoCarryEnabled then return end
	_acState._waitingForCarryPickup = true
	_acState._carryPickupWatchUntil = tick() + (seconds or 1.25)
end

function startAutoCarry()
	if autoCarryConn then return end
	autoCarryConn = RunService.RenderStepped:Connect(function()
		...
		if _acState._waitingForCarryPickup then
			if gotHit or tick() > (_acState._carryPickupWatchUntil or 0) then
				_acState._waitingForCarryPickup = false
				_acState._carryPickupWatchUntil = 0
		...
		if carrying and not _acState._autoCarryFromSteal then
			enableCarrySpeedForSteal()
		end
		if _acState._autoCarryFromSteal then
			local graceDone = tick() > (_acState._autoCarryGraceUntil or 0)
...
espObjects = {}
setEspVisual = nil
ESP_COLOR = Color3.fromRGB(120,50,190) -- rojo carmesí
...
function playCandyIntro(gui, main, miniBtn)
	...
	-- Ocultar botones móviles durante la intro
	...
	task.spawn(function()
		-- Música con caché (primera vez descarga, después es instantáneo)
		task.spawn(function()
			pcall(function()
				...
				if isfile and isfile("BlessVSIntro") then
					-- Ya está cacheado, suena al instante
		...
		-- Restaurar los botones móviles al terminar la intro normal.
	...
	-- Comprobación final por si la intro terminó por cualquier otra ruta.
	if mobileButtonsScreen then
		mobileButtonsScreen.Enabled = not hideMobileButtons
	end
...
function buildGui()
	...
	closeBtn.Text="✕";closeBtn.TextColor3=Color3.fromRGB(238,225,255);closeBtn.Font=Enum.Font.GothamBlack;closeBtn.TextSize=IsMobile and 14 or 16
	...
	lockBtn.Text="🔓 LOCK";lockBtn.TextColor3=Color3.fromRGB(238,225,255);lockBtn.Font=Enum.Font.GothamBlack;lockBtn.TextSize=IsMobile and 9 or 10;lockBtn.AutoButtonColor=false
	...
	-- Panel cuadrado con navegación por categorías
	...
	local tabsLayout=Instance.new("UIListLayout",tabsBar);tabsLayout.FillDirection=Enum.FillDirection.Horizontal;tabsLayout.HorizontalAlignment=Enum.HorizontalAlignment.Center;tabsLayout.Padding=UDim.new(0,4)

	local pagesHolder=Instance.new("Frame",main)
	pagesHolder.Name="CategoryPages";pagesHolder.Size=UDim2.new(1,-(PAD*2),1,-(PAGE_TOP+40));pagesHolder.Position=UDim2.new(0,PAD,0,PAGE_TOP+34)
	...
	for _,name in ipairs(categoryOrder) do createCategoryPage(name) end
	...
	activePage=pages.Mechanics;mainPage=activePage
...
function buildMobileButtons()
	...
	local function applyGridMode()
		-- Usar posiciones manuales siempre. Así, al salir de "Edit Mobile
...
function loadConfigKeys()
	...
	if cfg.hideMobileButtons~=nil then
		hideMobileButtons=cfg.hideMobileButtons==true
	else
...
-- ============================================================
--  TP BAT (función de Ultimate adaptada a Bless)
-- ============================================================
do
	local tpConn = nil
	...
	local function startTpBat()
		...
		tpConn = RunService.Heartbeat:Connect(function()
			...
			-- Misma selección de objetivo usada por Ultimate,
