...
local function showDiscordInProgressBar()
	...
	progressPct.Text = "🌀 Cryon.vs  ·  mogs"
...
verLbl.Text = "ㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤCryon.vs  ·  Cryon mogs"
...
do
	...
	local function makeProfileButton(label)
		...
		stroke.Color = Color3.fromRGB(16, 12, 100)
		stroke.Thickness = 1
	...
	local function refreshProfileVisual()
		...
		TweenService:Create(normalProfileBtn, TweenInfo.new(0.12), {
			BackgroundColor3 = normalActive and Color3.fromRGB(8, 14, 150) or CARD_BG,
		...
		TweenService:Create(laggerProfileBtn, TweenInfo.new(0.12), {
			BackgroundColor3 = not normalActive and Color3.fromRGB(8, 14, 150) or CARD_BG,
			TextColor3 = not normalActive and WHITE or DIM,
			BackgroundTransparency = not normalActive and 0.02 or 0.12
		}):Play()
		normalProfileStroke.Color = normalActive and Color3.fromRGB(18, 45, 255) or Color3.fromRGB(16, 12, 100)
		laggerProfileStroke.Color = not normalActive and Color3.fromRGB(18, 45, 255) or Color3.fromRGB(16, 12, 100)
	end
...
State._setHitboxFollower = rowToggle(
    "Performance",
    "Hitbox Follower",
    "Sigue con la mirada la hitbox más cercana",
    false,
    function(on)
...
State.buildBackgroundPage = function()
	...
	for index, assetId in ipairs(State.backgroundAssetIds) do
		...
		thumb.BackgroundColor3 = Color3.fromRGB(0, 0, 13)
		...
		badge.BackgroundColor3 = Color3.fromRGB(0, 0, 13)
...
do
	...
	MobilePanel.BackgroundColor3 = Color3.fromRGB(0, 0, 235)
	...
	local Q_ON       = Color3.fromRGB(0, 0, 235)
	local Q_TEXT_OFF = Color3.fromRGB(13, 35, 235)

	State._purpleAnimatedButtons = State._purpleAnimatedButtons or {}
	State._purpleAnimationPeriod = 5.5

	local purpleTextPalette = {
		Color3.fromRGB(11, 30, 255),
		Color3.fromRGB(0, 0, 205),
		Color3.fromRGB(0, 0, 100),
		Color3.fromRGB(0, 0, 8),
		Color3.fromRGB(0, 0, 145),
		Color3.fromRGB(15, 15, 255),
	}

	local function paletteColor(palette, progress)
		local count = #palette
		if count == 0 then return Color3.fromRGB(21, 55, 255) end
				...
				for button, visual in pairs(State._purpleAnimatedButtons) do
					if button and button.Parent then
						...
						if active or flash then
							...
							targetText = Color3.fromRGB(30, 135, 255)
						else
	...
	local function attachBlueTextShine(button)
		...
		shineGradient.Color = ColorSequence.new({
			ColorSequenceKeypoint.new(0.00, Color3.fromRGB(8, 45, 220)),
			ColorSequenceKeypoint.new(0.38, Color3.fromRGB(11, 75, 245)),
			ColorSequenceKeypoint.new(0.50, Color3.fromRGB(70, 235, 255)),
			ColorSequenceKeypoint.new(0.62, Color3.fromRGB(17, 125, 255)),
			ColorSequenceKeypoint.new(1.00, Color3.fromRGB(8, 45, 220)),
		})
		...
		-- Actualiza únicamente cuando cambia una propiedad; no revisa cada frame.
	...
	-- Un solo brillo activo a la vez. Evita múltiples tweens infinitos simultáneos.
	...
	local function createMobileButton(name, displayText, col, row, isToggle, onAction)
		...
		mobileStroke.Color = Color3.fromRGB(13, 120, 255)
	...
	batV2Stroke.Color = Color3.fromRGB(13, 120, 255)
	...
	instaResetStroke.Color = Color3.fromRGB(13, 120, 255)
...
local function CreateESP(player)
    ...
    Line.Color = Color3.fromRGB(0, 0, 205) -- Se mantiene la línea azul
...
local function createRadiusPart()
	...
	p.Color = Color3.fromRGB(0, 0, 120)
...
frame.BackgroundColor3 = Color3.fromRGB(0, 0, 9)
...
frameGradient.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(0, 0, 25)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(0, 0, 6))
})
...
sep.BackgroundColor3 = Color3.fromRGB(0, 0, 120)
...
radTag.Text = "RADIUS"
radTag.TextColor3 = Color3.fromRGB(42, 110, 255)
...
radBox.BackgroundColor3 = Color3.fromRGB(0, 0, 13)
radBox.BorderSizePixel = 0
radBox.Text = tostring(CONFIG.STEAL_RANGE)
radBox.TextColor3 = Color3.fromRGB(21, 55, 255)
...
radStroke.Color = Color3.fromRGB(0, 0, 255)
...
barBg.BackgroundColor3 = Color3.fromRGB(0, 0, 32)
...
toggleBtn.BackgroundColor3 = Color3.fromRGB(0, 0, 225)
toggleBtn.BorderSizePixel = 0
toggleBtn.Text = "STOP"
toggleBtn.TextColor3 = Color3.fromRGB(21, 55, 255)
toggleBtn.Font = Enum.Font.GothamBold
toggleBtn.TextSize = 10
Instance.new("UICorner", toggleBtn).CornerRadius = UDim.new(0, 8)
local toggleStroke = Instance.new("UIStroke", toggleBtn)
toggleStroke.Color = Color3.fromRGB(25, 65, 255)
...
toggleBtn.MouseButton1Click:Connect(function()
	...
	if CONFIG.AUTO_STEAL_ENABLED then
		...
		toggleBtn.BackgroundColor3 = Color3.fromRGB(0, 0, 8)
		toggleStroke.Color = Color3.fromRGB(0, 0, 130)
		stopAutoSteal()
...
local function updateStealBar(dt)
	...
	local targetPct = 0
	local targetColor = Color3.fromRGB(0, 0, 220)
	...
	if StealState.active then
		...
		if StealState.phase == "waitingRange" then
			status = "WAITING"
			targetColor = Color3.fromRGB(0, 0, 150)
		else
			status = "STEALING"
	...
	elseif recent then
		...
		targetColor = success and Color3.fromRGB(21, 55, 255) or Color3.fromRGB(0, 0, 125)
	elseif CONFIG.AUTO_STEAL_ENABLED then
		local scan = math.sin(tick() * 2.2) * 0.5 + 0.5
		targetPct = scan * 0.75
		status = "SCAN"
		targetColor = Color3.fromRGB(0, 0, 215)
	end
...
print("✅ Auto Steal ACTIVADO - Robando automáticamente!")
print("📌 Presiona el botón en la barra para pausar/reanudar")
...
loadConfig = function()
    ...
    local applyOk = pcall(function()
        ...
        if cfg.autoStealEnabled ~= nil then
            ...
            if toggleBtn then
                toggleBtn.Text = autoStealOn and "STOP" or "START"
                toggleBtn.BackgroundColor3 = autoStealOn
                    and Color3.fromRGB(0, 0, 235)
                    or Color3.fromRGB(0, 0, 235)
            end
            if toggleStroke then
                toggleStroke.Color = autoStealOn
                    and Color3.fromRGB(12, 12, 255)
                    or Color3.fromRGB(12, 12, 255)
            end
            if statusLabel then statusLabel.Text = autoStealOn and "READY" or "IDLE" end
...
State._attachOtherSpeedBillboard = function(player, character)
    ...
    task.spawn(function()
        ...
        label.BackgroundTransparency = 1
        label.Text = "0.0"
        label.TextColor3 = Color3.fromRGB(0, 0, 235)
        label.Font = Enum.Font.GothamBlack
...
local function setupChar(char)
    ...
    if head then
        ...
        speedLbl.Text="0.0"; speedLbl.TextColor3=Color3.fromRGB(0, 0, 235)
