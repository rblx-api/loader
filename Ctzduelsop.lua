if _G.Ctz_duelsRunning then
	return
end
_G.Ctz_duelsRunning = true
...
local InfJumpState = { enabled = true, mode = "hold" }
...
UIS.JumpRequest:Connect(function()
	if not InfJumpState.enabled then
		return
	end
...
UIS.InputBegan:Connect(function(inp)
	if inp.KeyCode ~= Enum.KeyCode.ButtonA or not isGamepad(inp) then
		return
	end
...
UIS.TouchStarted:Connect(function(touch)
	if not InfJumpState.enabled or InfJumpState.mode ~= "hold" then
		return
	end
...
UIS.TouchEnded:Connect(function(touch)
	...
	if _lastTouchStart == touch then
		_lastTouchStart = nil
	end
	if _holdTouch == touch then
		_holdTouch = nil
	end
end)
...
local function createInfJumpPanel()
	return nil, nil
end
...
local function executeStealAsync(prompt, animalData)
	...
	task.spawn(function()
		...
		while true do
			...
			if elapsed > CONFIG.HOLD_MAX then
				break
			end
...
autoResetStates = { BALLOON = false, JAIL = false, TINY = false, RAGDOLL = false, ROCKET = false }
...
autoBatSetVisual, autoLeftSetVisual, autoRightSetVisual, setBatCounterVisual, setMedusaVisual, setMedusaAutoResetVisual =
	nil, nil, nil, nil, nil, nil
setAntiRagVisual, setJumpVisual, setUnwalkVisual, setAntiLagVisual, setLockUIVisual, setInstaGrab =
	nil, nil, nil, nil, nil, nil
setEditModeVisual = nil
setESPVIsual = nil
mobSetAutoBat, mobSetAutoLeft, mobSetAutoRight, mobSetDropBR, mobSetTpDown, mobSetCarry, mobSetLagger1, mobSetLagger2 =
...
local AntiRagdollV2 = { Enabled = false, Connection = nil, ResetCooldown = 0 }
local function startAntiRagdoll()
	...
	AntiRagdollV2.Connection = RunService.Heartbeat:Connect(function()
		if not AntiRagdollV2.Enabled then
			return
		end
		local char = LP.Character
		if not char then
...
function setupSpeedIndicator(char)
	...
	local oldBB = head:FindFirstChild("Ctz_duelsSpeedIndicator")
	if oldBB then
		oldBB:Destroy()
	end
	local bb = Instance.new("BillboardGui", head)
	bb.Name = "Ctz_duelsSpeedIndicator"
...
function resetMovementState()
	...
	if mobSetLagger1 then
		mobSetLagger1(laggerToggled and laggerLevel == 1)
	end
	if mobSetLagger2 then
		mobSetLagger2(laggerToggled and laggerLevel == 2)
	end
end
			...
			obj.OnClientEvent:Connect(function(...)
				for _, arg in ipairs({ ... }) do
					if type(arg) == "string" then
						local msg = arg:lower()
						if msg:find("jump higher") and autoResetStates.BALLOON then
							instareset("balloon")
						elseif msg:find("jail") and autoResetStates.JAIL then
							instareset("jail")
						elseif msg:find("tiny") and autoResetStates.TINY then
							instareset("tiny")
						elseif msg:find("ragdoll") and autoResetStates.RAGDOLL then
							instareset("ragdoll")
						elseif msg:find("rocket") and autoResetStates.ROCKET then
							instareset("rocket")
						end
					end
				end
...
function instareset(resetType)
	...
	task.spawn(function()
		...
		while attempts < 50 do
			pcall(function()
				resetRemote:FireServer(GUID, LP, resetType or "balloon")
			end)
			task.wait(0.05)
			attempts = attempts + 1
			if LP.Character ~= oldChar then
				break
			end
		end
	end)
...
local function tryHitBatV2()
	if autoBatV2HitCooldown or not autoBatV2SwingEnabled then
		return
	end
...
function enableBatV2()
	...
	if batV2FloatingButton then
		...
		if btnFrame then
			btnFrame.BackgroundColor3 = Color3.fromRGB(180, 180, 190)
...
local function trySwingAntiDesync()
	if _G.AceAntiDesync.hittingCooldown then
		return
	end
...
function startAntiDesyncAimbot()
	...
	if LP.Character then
		pcall(function()
			setupCharAntiDesync(LP.Character)
		end)
	end
	_G.AceAntiDesync.conn = RunService.Heartbeat:Connect(function()
		if not (_G.AceAntiDesyncAimbotOn and _G.AceAntiDesync.h and _G.AceAntiDesync.hrp) then
			return
		end
	...
	if tpLockSetVisual then
		tpLockSetVisual(true)
	end
...
function setupMedusa(char)
	...
	Conns.anchor = {}
	if not char or not medusaCounterEnabled then
		return
	end
...
function setupMedusaAutoReset(char)
	for _, c in pairs(medusaResetConns) do
		pcall(function()
			c:Disconnect()
		end)
	end
	medusaResetConns = {}
	if not char or not medusaAutoResetEnabled then
		return
	end
	for _, part in ipairs(char:GetDescendants()) do
		if part:IsA("BasePart") then
...
function setMedusaCounterState(state)
	...
	if state then
		if medusaAutoResetEnabled then
			medusaAutoResetEnabled = false
			if setMedusaAutoResetVisual then
				setMedusaAutoResetVisual(false)
			end
			stopMedusaAutoReset()
		end
		if LP.Character then
...
function setMedusaAutoResetState(state)
	...
	if state then
		...
		if LP.Character then
			setupMedusaAutoReset(LP.Character)
		else
			stopMedusaAutoReset()
		end
	else
	...
	if setMedusaAutoResetVisual then
		setMedusaAutoResetVisual(state)
	end
end
...
function runDropBrainrot()
	...
	if dropMode == 1 then
		...
		if tick() - lastDropTime < cooldown then
			return
		end
		lastDropTime = tick()
		dropActive = true
	...
	for _, e in ipairs(Lighting:GetChildren()) do
		...
		elseif e:IsA("BloomEffect") then
			_originalLighting.Bloom =
				{ Enabled = e.Enabled, Intensity = e.Intensity, Size = e.Size, Threshold = e.Threshold }
		end
	end
end
...
function setupMovementAndIndicators(char)
	...
	movementLoop = RunService.RenderStepped:Connect(function()
		...
		if speedLabel then
			...
			speedLabel.Text = "Spd: " .. string.format("%.1f", flatSpeed)
		end
	end)
...
function loadAllSettings()
	...
	if data.mobilePanelPos then
		savedMobilePanelPos = data.mobilePanelPos
	end
	if data.instaResetFloatingPos then
		instaResetFloatingPos = data.instaResetFloatingPos
	end
	if data.tpBatFloatingPos then
		tpBatFloatingPos = data.tpBatFloatingPos
	end
	if data.batV2FloatingPos then
		batV2FloatingPos = data.batV2FloatingPos
	end
	...
	if data.backgroundIndex then
		currentBgIndex = data.backgroundIndex
		if bgDecalRef then
			bgDecalRef.Image = BackgroundDecals[currentBgIndex] or ""
		end
		if bgPreviewRef then
			bgPreviewRef.Image = BackgroundDecals[currentBgIndex] or ""
		end
	else
		currentBgIndex = 1
	end
...
function forceResetUI()
	...
	if laggerBox then
		laggerBox.Text = tostring(LAGGER_SPEED_1)
	end
	if lagger2Box then
		lagger2Box.Text = tostring(LAGGER_SPEED_2)
	end
	if batSpeedBox then
	...
	currentBgIndex = 1
	if bgDecalRef then
		bgDecalRef.Image = BackgroundDecals[1] or ""
	end
	if bgPreviewRef then
		bgPreviewRef.Image = BackgroundDecals[1] or ""
	end
...
function resetToFactoryDefaults()
	...
	bodyLockRange = 20
	autoBatV2Enabled = false
	for k in pairs(autoResetStates) do
		autoResetStates[k] = false
	end
	currentAnimPack = "Off"
	stopAnimPack()
	savedStealRadius = CONFIG.STEAL_RANGE
...
function getDefaultButtonPosition(btnName)
	...
	local GAP = 8
	local orderMap =
		{ DropBR = 0, AutoLeft = 1, AutoBat = 2, AutoRight = 3, TpDown = 4, Carry = 5, Lagger1 = 6, Lagger2 = 7 }
...
local BackgroundDecals = {
	[1] = "rbxassetid://101894744159774",
	[2] = "rbxassetid://101894744159774",
	[3] = "rbxassetid://80423640819609",
	[4] = "rbxassetid://131644387510029",
	[5] = "rbxassetid://130908218899282",
}
currentBgIndex = currentBgIndex or 1
local bgDecalRef = nil
local bgPreviewRef = nil
local function updateBackground(index)
	currentBgIndex = index
	if bgDecalRef then
		bgDecalRef.Image = BackgroundDecals[index] or ""
	end
	if bgPreviewRef then
		bgPreviewRef.Image = BackgroundDecals[index] or ""
	end
...
function buildGui()
	...
	local ROW_BORDER = Color3.fromRGB(0, 255, 100)
	...
	local STROKE_COLOR = Color3.fromRGB(0, 255, 100)
	local GUI_W, GUI_H = 330, 480
	local old = game:GetService("CoreGui"):FindFirstChild("Ctz_duels")
	if old then
		old:Destroy()
	end
	local pg = LP:FindFirstChild("PlayerGui")
	if pg then
		local o = pg:FindFirstChild("Ctz_duels")
		if o then
			o:Destroy()
		end
	end
	...
	mainUIScale.Scale = uiScaleValue / 100
	bgDecalRef = Instance.new("ImageLabel", main)
	bgDecalRef.Size = UDim2.new(1, 0, 1, 0)
	bgDecalRef.Position = UDim2.new(0, 0, 0, 0)
	bgDecalRef.BackgroundTransparency = 1
	bgDecalRef.Image = BackgroundDecals[currentBgIndex] or ""
	bgDecalRef.ZIndex = 0
	bgDecalRef.ScaleType = Enum.ScaleType.Crop
	local decalCorner = Instance.new("UICorner", bgDecalRef)
	decalCorner.CornerRadius = UDim.new(0, 18)
	local titleLabel = Instance.new("TextLabel", main)
	titleLabel.Size = UDim2.new(1, -80, 0, 40)
	titleLabel.Position = UDim2.new(0, 12, 0, 4)
	titleLabel.BackgroundTransparency = 1
	titleLabel.Text = "Ctz_duels"
	...
	closeBtn.MouseLeave:Connect(function()
		TS:Create(closeBtn, TweenInfo.new(0.12), { TextColor3 = WHITE, BackgroundColor3 = Color3.fromRGB(30, 30, 35) })
			:Play()
	end)
	...
	miniBtn.Text = "Ctz_duels"
	miniBtn.TextColor3 = SILVER
	...
	tabBar.Size = UDim2.new(0, 80, 1, -96)
	...
	tabContent.Size = UDim2.new(1, -80 - 20, 1, -96)
	tabContent.Position = UDim2.new(0, 80 + 12, 0, 86)
	...
	for i, name in ipairs(tabs) do
		...
		btn.MouseButton1Click:Connect(function()
			for _, pg in pairs(contentPages) do
				pg.Visible = false
			end
			page.Visible = true
	...
	local function animPill(pill, dot, on)
		TS:Create(pill, TweenInfo.new(0.2, Enum.EasingStyle.Quad), { BackgroundColor3 = on and SILVER or OFF }):Play()
		...
		if stroke then
			TS
				:Create(
					stroke,
					TweenInfo.new(0.2),
					{ Color = on and SILVER or ROW_BORDER, Transparency = on and 0 or 0.6 }
				)
				:Play()
		end
	end
	...
	local function mkBox(parent, default, w, xOff, cb)
		...
		tb.Focused:Connect(function()
			TS:Create(bs, TweenInfo.new(0.12), { Color = SILVER, Transparency = 0 }):Play()
		end)
	...
	do
		...
		normalBox = mkBox(row, NS, 50, 56, function(v)
	...
	do
		...
		carryBox = mkBox(row, CS, 50, 56, function(v)
			if v > 0 and v <= 500 then
				CS = v
			end
		end)
	...
	do
		local row = mkRow(momentPage, 38)
		mkLabel(row, "Lagger 1 Speed")
		laggerBox = mkBox(row, LAGGER_SPEED_1, 50, 56, function(v)
			if v > 0 and v <= 500 then
				LAGGER_SPEED_1 = v
			end
		end)
	...
	do
		local row = mkRow(momentPage, 38)
		mkLabel(row, "Lagger 2 Speed")
		lagger2Box = mkBox(row, LAGGER_SPEED_2, 50, 56, function(v)
			if v > 0 and v <= 500 then
				LAGGER_SPEED_2 = v
			end
		end)
	...
	setUnwalkVisual = mkToggle(momentPage, "Unwalk", function(on)
		unwalkEnabled = on
		if on then
			startUnwalk()
		else
	...
	do
		...
		dropModeBtnRef = mkSelector(
			row,
			dropMode == 1 and "Fling" or "Jump Drop",
			{ "Fling", "Jump Drop" },
			function(dir, update)
				if dropActive then
					stopDropBrainrot()
	...
	mkSect(momentPage, "Auto Movement")
	...
	setInstaResetVisual = mkToggle(momentPage, "Insta Reset", function(on)
		if on then
			...
			if instaResetFloatingButton and instaResetFloatingButton:FindFirstChild("Frame") then
				...
				if btnFrame then
					btnFrame.BackgroundColor3 = SILVER
					if label then
						label.TextColor3 = Color3.fromRGB(0, 0, 0)
					end
	...
	do
		...
		batSpeedBox = mkBox(row, BAT_AIMBOT_SPEED, 50, 56, function(v)
			if v > 0 and v <= 200 then
				BAT_AIMBOT_SPEED = v
			end
		end)
	end
	tpLockSetVisual = mkToggle(combatPage, "Anti-Desync", function(on)
		if on then
			if not _G.AceAntiDesyncAimbotOn then
				startAntiDesyncAimbot()
			end
		else
			if _G.AceAntiDesyncAimbotOn then
	...
	if tpLockSetVisual then
		tpLockSetVisual(_G.AceAntiDesyncAimbotOn)
	end
	...
	setMedusaAutoResetVisual = mkToggle(combatPage, "Medusa Auto Reset", function(on)
	...
	do
		...
		for i, entry in ipairs(ANIM_PACK_ORDER) do
			if entry[2] == currentAnimPack then
				currentIndex = i
				break
			end
		end
		...
		local function updateAnimSelector(direction)
			...
			if newIdx < 1 then
				newIdx = #ANIM_PACK_ORDER
			end
			if newIdx > #ANIM_PACK_ORDER then
				newIdx = 1
			end
	...
	do
		...
		leftBtn.Size = UDim2.new(0, 28, 0, 22)
		leftBtn.Position = UDim2.new(1, -132, 0.5, -11)
		leftBtn.BackgroundColor3 = INP
		leftBtn.BackgroundTransparency = 0.7
		leftBtn.BorderSizePixel = 0
		leftBtn.Text = "<"
		...
		rightStroke.Thickness = 1
		leftBtn.MouseButton1Click:Connect(function()
			local newIndex = currentBgIndex - 1
			if newIndex < 1 then
				newIndex = 5
			end
			updateBackground(newIndex)
			preview.Image = BackgroundDecals[newIndex] or ""
		end)
		rightBtn.MouseButton1Click:Connect(function()
			local newIndex = currentBgIndex + 1
			if newIndex > 5 then
	...
	do
		...
		expandBtn.Position = UDim2.new(0.58, 0, 0.5, -11)
		expandBtn.BackgroundColor3 = INP
		expandBtn.BackgroundTransparency = 0.7
		expandBtn.BorderSizePixel = 0
		expandBtn.Text = "▶"
		expandBtn.TextColor3 = WHITE
		...
		local function toggleFov()
			fovContainer.Visible = not fovContainer.Visible
			expandBtn.Text = fovContainer.Visible and "▼" or "▶"
		end
		expandBtn.MouseButton1Click:Connect(toggleFov)
		fovToggleBtn = expandBtn
	end
	...
	setNeonWeatherVisual = mkToggle(mainPage, "�� Clima Neon", function(on)
	...
	addKeybindRow(keyPage, "Anti-Desync", KB.TPLock)
	...
	profileContainer.Size = UDim2.new(0, 70, 0, 85)
	profileContainer.Position = UDim2.new(0, 12, 1, -95)
	...
	profileImage.Position = UDim2.new(0.5, -28, 0, 0)
	...
	local circleCorner = Instance.new("UICorner", profileImage)
	circleCorner.CornerRadius = UDim.new(1, 0)
	local profileStroke = Instance.new("UIStroke", profileImage)
	profileStroke.Color = SILVER
	profileStroke.Thickness = 2
	profileStroke.Transparency = 0
	local onlineLabel = Instance.new("TextLabel", profileContainer)
	onlineLabel.Size = UDim2.new(1, 0, 0, 20)
	onlineLabel.Position = UDim2.new(0, 0, 0, 60)
	onlineLabel.BackgroundTransparency = 1
	onlineLabel.Text = "Online"
	onlineLabel.TextColor3 = SILVER
	onlineLabel.Font = Enum.Font.GothamBold
	onlineLabel.TextSize = 12
	onlineLabel.TextXAlignment = Enum.TextXAlignment.Center
	onlineLabel.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
	onlineLabel.TextStrokeTransparency = 0.2
	onlineLabel.ZIndex = 31
	task.spawn(function()
	...
	pbDiv.Position = UDim2.new(0, 180, 0.5, -7)
	...
	progressRadLbl.Size = UDim2.new(0, 92, 1, 0)
	progressRadLbl.Position = UDim2.new(0, 184, 0, 0)
	...
	task.spawn(function()
		...
		while task.wait() do
			...
			if StealState.active then
				local elapsed = tick() - StealState.startTime
				local duration = CONFIG.HOLD_MAX
			...
			if progressPct then
				progressPct.Text = math.floor(f * 100) .. "%"
			end
		end
	end)
	task.spawn(function()
...
function createMobilePanel()
	...
	panel.Name = "Ctz_duelsMobilePanel"
	panel.ResetOnSpawn = false
	panel.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	pcall(function()
		if syn and syn.protect_gui then
			syn.protect_gui(panel)
		end
	end)
	if not pcall(function()
	...
	if buttons.Lagger1 and buttons.Lagger1.setActive then
		buttons.Lagger1.setActive(laggerToggled and laggerLevel == 1)
	end
	if buttons.Lagger2 and buttons.Lagger2.setActive then
		buttons.Lagger2.setActive(laggerToggled and laggerLevel == 2)
	end
...
function createInstaResetFloatingButton()
	...
	if instaResetFloatingPos then
		btnFrame.Position = UDim2.new(
			...
			instaResetFloatingPos.XOffset or -120,
...
function createTpBatFloatingButton()
	...
	btnFrame.BackgroundColor3 = _G.AceAntiDesyncAimbotOn and SILVER or Color3.fromRGB(0, 0, 0)
	btnFrame.BackgroundTransparency = 0
	btnFrame.BorderSizePixel = 0
	btnFrame.ZIndex = 20
	Instance.new("UICorner", btnFrame).CornerRadius = UDim.new(0, 18)
	...
	label.BackgroundTransparency = 1
	label.Text = "ANTI\nDESYNC"
	label.TextColor3 = _G.AceAntiDesyncAimbotOn and Color3.fromRGB(0, 0, 0) or SILVER
	label.Font = Enum.Font.GothamBold
	label.TextSize = 10
	label.TextWrapped = true
	label.ZIndex = 21
...
function createBatV2FloatingButton()
	...
	btnFrame.BackgroundColor3 = autoBatV2Enabled and SILVER or Color3.fromRGB(0, 0, 0)
	btnFrame.BackgroundTransparency = 0
	btnFrame.BorderSizePixel = 0
	btnFrame.ZIndex = 20
	Instance.new("UICorner", btnFrame).CornerRadius = UDim.new(0, 18)
	...
	label.BackgroundTransparency = 1
	label.Text = "BAT\nV2"
	label.TextColor3 = autoBatV2Enabled and Color3.fromRGB(0, 0, 0) or SILVER
	label.Font = Enum.Font.GothamBold
	label.TextSize = 11
	label.TextWrapped = true
	label.ZIndex = 21
	...
	if _G.fovButtons then
		for _, btn in ipairs(_G.fovButtons) do
			if btn:IsA("TextButton") then
				...
				if val == stretchFOV then
					btn.BackgroundColor3 = Color3.fromRGB(180, 180, 190)
					btn.TextColor3 = Color3.fromRGB(0, 0, 0)
				else
...
if LP.Character then
	...
	while
		...
		or not LP.Character:FindFirstChildOfClass("Humanoid")
	do
		task.wait()
	end
...
UIS.InputBegan:Connect(function(input, gpe)
	...
	if kbMatch(KB.TPLock, kc) then
		...
		if tpBatFloatingButton then
			local btnFrame = tpBatFloatingButton:FindFirstChild("Frame")
			if btnFrame then
				btnFrame.BackgroundColor3 = _G.AceAntiDesyncAimbotOn and Color3.fromRGB(180, 180, 190)
					or Color3.fromRGB(0, 0, 0)
				local label = btnFrame:FindFirstChild("TextLabel")
				if label then
					label.TextColor3 = _G.AceAntiDesyncAimbotOn and Color3.fromRGB(0, 0, 0)
	...
	if kbMatch(KB.BatV2, kc) then
		...
		if batV2FloatingButton then
			local btnFrame = batV2FloatingButton:FindFirstChild("Frame")
			if btnFrame then
				btnFrame.BackgroundColor3 = autoBatV2Enabled and Color3.fromRGB(180, 180, 190)
					or Color3.fromRGB(0, 0, 0)
				local label = btnFrame:FindFirstChild("TextLabel")
				if label then
					label.TextColor3 = autoBatV2Enabled and Color3.fromRGB(0, 0, 0) or Color3.fromRGB(180, 180, 190)
				end
			end
		end
		return
