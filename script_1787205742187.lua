-- STL HUB v3 (fix registradores + protecao contra execucao dupla)
if _G.__STLHUB_ACTIVE and not _G.__STLHUB_FORCE_RELOAD then
    local function _find(n)
        local host = (gethui and gethui()) or game:GetService("CoreGui")
        local r
        pcall(function() r = host and host:FindFirstChild(n) end)
        if not r then
            pcall(function()
                local pg = game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui")
                r = pg and pg:FindFirstChild(n)
            end)
        end
        return r
    end
    local g = _find("SXEHub_V3")
    if g then
        pcall(function() g.Enabled = true end)
        pcall(function()
            local m = g:FindFirstChild("SXE_MasterFrame", true)
            if m then m.Visible = true end
        end)
    end
    warn("[STL HUB] ja esta rodando - reabrindo a UI existente (execucao dupla bloqueada).")
    return
end
_G.__STLHUB_FORCE_RELOAD = false
_G.__STLHUB_ACTIVE = true
local _SXENV = setmetatable({}, {__index = getfenv(1)})
setfenv(1, _SXENV)
print = print
do
    local _stlRealPrint = print
    local function _stlSilentLog(...) end
    _stlRealPrint("STL HUB FREE ON TOP")
    print = _stlSilentLog
    warn = _stlSilentLog
end
...
old = _G.SXEBuscaGui("SXEHub_V3"); if old then old:Destroy() end
gui_sg = Instance.new("ScreenGui"); gui_sg.Name = "SXEHub_V3"; gui_sg.ResetOnSpawn = false; gui_sg.IgnoreGuiInset = true; gui_sg.DisplayOrder = 9999999; gui_sg.Parent = ((gethui and gethui()) or game:GetService("CoreGui"))
...
Themes = {
    Light = {
        ...
        InputBg=Color3.fromRGB(255,255,255), SliderBg=Color3.fromRGB(196,210,230),
        BlacklistHover=Color3.fromRGB(240,206,214), BlacklistLeave=Color3.fromRGB(224,233,245),
    },
    Dark = {
        ...
        InputBg=Color3.fromRGB(24,27,33), SliderBg=Color3.fromRGB(52,60,72),
        BlacklistHover=Color3.fromRGB(96,44,54), BlacklistLeave=Color3.fromRGB(48,54,65),
    }
}
HERESY = { NOME="STL HUB", GRAD_A=Color3.fromRGB(24,110,225), GRAD_B=Color3.fromRGB(120,195,255) }
Theme = {}
...
Keybinds = {
    ...
    ["Ragdoll Self"]="R",["Drop Brainrot"]="G",["Insta Reset"]="X",Float="Z",["Auto Buy"]="K",
    ["Click to AP"]="NONE",
    ["Steal Nearest"]="NONE",
    ["Face Nearest"]="NONE"
}
...
actionConfig = {
    ["Ragdoll Self (R)"]=true,["Rejoin Job ID (J)"]=true,["Rejoin PS"]=true,
    ["Kick (Y)"]=true,["Kick To Private"]=true,
    ["Anti Ragdoll"]=false,["Infinite Jump"]=false,["Float"]=false,["Carpet Speed"]=false,
    ["Insta Reset"]=true,
}
CONFIG_FILE = "sxe_hub_v3_config.json"
PS_CODE_FILE = "sxe_hub_pscode.txt"
PrivateServerCode = ""
function loadPSCode()
...
Config = {
    ...
    AutoTPGen=false, AutoTPNearest=false, GenValueESP=false, GenValueESPMin="", AntiSwap=false,
    ...
    AutoDestroyTurrets=false, -- DISABLED (removed)
    ...
    TpSettings = {
        ...
        TpOnLoad=false, MinGenForTp="", MinGenForGrab="", DelayTp=0,
...
function getTpDelay()
    local v = tonumber(Config and Config.TpSettings and Config.TpSettings.DelayTp) or 0
    return math.max(0, math.min(v, 60))
end
function waitTpDelay()
    local d = getTpDelay()
...
lastActionUse = {}
function _readRealAdminTimer(_cmd)
    return nil
end
...
AP_REMOTE_NAMES = {"AdminPanel","AdminAction","AdminCommand","AdminPanelAction","AdminPanelCommand","UseAdminPanel","AdminPunish","AdminPanelService","AdminPanelUse"}
_apRemote = nil
function _apResolveRemote()
    if _apRemote and _apRemote.Parent then return _apRemote end
    _apRemote = nil
    if not _G.Net then return nil end
    for _, n in ipairs(AP_REMOTE_NAMES) do
        local ok, r = pcall(function()
            return (_G.Net.GetRemote and _G.Net:GetRemote(n))
                or (_G.Net.RemoteEvent and _G.Net:RemoteEvent(n))
        end)
        if ok and typeof(r) == "Instance" and r:IsA("RemoteEvent") then _apRemote = r; return r end
    end
    return nil
end
function _apGameGui()
    ...
    for _, g in ipairs(pg:GetChildren()) do
        if g:IsA("ScreenGui") and g.Name:lower():find("admin") then return g end
    end
    for _, g in ipairs(pg:GetDescendants()) do
        if (g:IsA("Frame") or g:IsA("ScrollingFrame")) and g.Name:lower():find("admin") then return g end
    end
    return nil
end
function _apMatchesPlayer(obj, plr)
    local txt = ((obj:IsA("TextButton") or obj:IsA("TextLabel")) and obj.Text or "") .. " " .. obj.Name
    txt = txt:lower()
    return txt:find(plr.Name:lower(), 1, true) ~= nil
        or txt:find((plr.DisplayName or plr.Name):lower(), 1, true) ~= nil
end
function _apClickCommandUI(targetPlayer, commandName)
    local gui = _apGameGui()
    if not gui then return false end
    pcall(function() if gui:IsA("ScreenGui") then gui.Enabled = true end end)
    local playerBtn
    for _, d in ipairs(gui:GetDescendants()) do
        if d:IsA("TextButton") and _apMatchesPlayer(d, targetPlayer) then playerBtn = d; break end
        if (d:IsA("TextLabel") or d:IsA("Frame")) and _apMatchesPlayer(d, targetPlayer) then
            local btn = d:FindFirstChildWhichIsA("TextButton", true)
                or (d.Parent and d.Parent:FindFirstChildWhichIsA("TextButton", true))
            if btn then playerBtn = btn; break end
        end
    end
    if playerBtn then fireClick(playerBtn); task.wait(0.08) end
    local cmd = commandName:lower()
    for _, d in ipairs(gui:GetDescendants()) do
        if d:IsA("TextButton") then
            local t = (d.Text or ""):lower() .. " " .. d.Name:lower()
            if t:find(cmd, 1, true) then
                fireClick(d)
                return true
            end
        end
...
function runAdminCommand(_targetPlayer, _commandName)
    if not _targetPlayer or not _commandName then return false end
    if _targetPlayer == LocalPlayer and _commandName ~= "ragdoll" then
        -- self commands other than ragdoll are not supported by the game panel
    end
    if apIsOnCooldown(_commandName) then return false end
    local fired = false
    local remote = _apResolveRemote()
    if remote then
        fired = select(1, pcall(function()
            remote:FireServer(_commandName, _targetPlayer)
        end))
        if not fired then
            pcall(function() remote:FireServer(_targetPlayer, _commandName) end)
            fired = true
        end
    end
    if not fired then
        local ok, res = pcall(_apClickCommandUI, _targetPlayer, _commandName)
        fired = ok and res == true
    end
    if fired then apStartCooldown(_commandName) end
    return fired
end
...
function ShowNotification(title, text)
    task.spawn(function()
        pcall(function()
            ...
            local sg = parent:FindFirstChild("XiNotifs")
            if not sg then
                sg = Instance.new("ScreenGui")
                sg.Name = "XiNotifs"; sg.ResetOnSpawn = false; sg.IgnoreGuiInset = true
                sg.DisplayOrder = 9999999; sg.Parent = parent
            ...
            t.Position = UDim2.fromOffset(10, 6); t.Size = UDim2.new(1, -20, 0, 16)
            t.TextXAlignment = Enum.TextXAlignment.Left
            t.Font = Enum.Font.GothamBold; t.TextSize = 13
            t.TextColor3 = Color3.fromRGB(235, 235, 245)
            t.Text = tostring(title or ""); t.Parent = f
            local d = t:Clone()
            d.Position = UDim2.fromOffset(10, 26); d.Size = UDim2.new(1, -20, 0, 18)
            d.Font = Enum.Font.Gotham; d.TextSize = 12
            d.TextColor3 = Color3.fromRGB(175, 175, 190)
            d.TextWrapped = true
            d.Text = tostring(text or ""); d.Parent = f
...
-- SEMON-style Insta Reset (logic only, no GUI) — high void TP until respawn
local SEMON_RESET_CF = CFrame.new(1000003.56, 999999.69, 8.17)
local function semonInstaReset()
    local lp = LocalPlayer
    ...
    task.delay(0.6, function()
        ...
        if hm then
            pcall(function()
                ...
                workspace.CurrentCamera.CameraSubject = hm
            end)
        end
    end)
    pcall(function() hrp.CFrame = SEMON_RESET_CF end)
    local con
    con = RunService.Heartbeat:Connect(function()
        ...
        if root then pcall(function() root.CFrame = SEMON_RESET_CF end) end
    end)
end

executeReset = function(isBalloon)
    -- use SEMON insta reset source (Actions only)
    return semonInstaReset()
end
local function __executeReset_DISABLED(isBalloon)
    if isBalloon then
...
pcall(function() Config.AutoDestroyTurrets = false end)
...
function startWalkFling()
    ...
    local co=coroutine.create(function()
        ...
        -- Pulse rate is capped so high-FPS (PC) behaves like mobile.
        -- Huge velocities (10000+) at 120+ FPS trigger the server anti-cheat kick.
        local PULSE_INTERVAL = 1/20
        local PULSE_POWER    = 240
        local MAX_PULSES     = 8
        local pulses = 0
        while _wfActive and pulses < MAX_PULSES do
            if not rr or not rr.Parent then break end
            local v = rr.AssemblyLinearVelocity
            rr.AssemblyLinearVelocity = Vector3.new(v.X, PULSE_POWER, v.Z)
            task.wait()
            if not rr or not rr.Parent then break end
            rr.AssemblyLinearVelocity = v
            pulses = pulses + 1
            task.wait(PULSE_INTERVAL)
        end
        if rr and rr.Parent then rr.AssemblyLinearVelocity = Vector3.new(0,0,0) end
        _wfActive = false
...
function runDropBrainrot()
    ...
    startWalkFling()
    task.delay(0.75, stopWalkFling)
end
_G.runDropBrainrot = runDropBrainrot
do
        ...
        task.spawn(function()
            while animPlaying do
                pcall(function()
                    ...
                    if item then _pingSec = math.clamp((item:GetValue() or 0) / 1000, 0, 0.6) end
                end)
                task.wait(1)
            end
        end)
            ...
            if character and character:FindFirstChild("Humanoid") and character.Humanoid.Health > 0 and oldRoot then
                ...
                if root then
                    ...
                    elseif lastSetPosition and ghostEnabled then
                        ...
                        if jumpDist > 6 and not _G.RecoveryInProgress then
                            lastSetPosition = nil; createServerGhost(currentPos)
                            -- lagback: reconstroi o invis em vez de deixar o boneco voltar ao normal
                            if _G.AutoRecoverLagback ~= false and _G._forceInvisToggle then
                                _G.RecoveryInProgress = true
                                task.spawn(function()
                                    pcall(_G._forceInvisToggle); task.wait(0.45)
                                    if not _G.invisibleStealEnabled then pcall(_G._forceInvisToggle) end
                    ...
                    if oldRoot and oldRoot.Parent then
                        ...
                        -- compensacao de ping: joga o corpo do servidor pra frente do movimento
                        -- pra ele nao ficar arrastado atras do personagem real
                        local vel = root.AssemblyLinearVelocity
                        local lead = math.clamp((_G.InvisLead or (_pingSec * 0.85)) + (dt or 0), 0, 0.4)
                        local predicted = root.Position + Vector3.new(vel.X, 0, vel.Z) * lead
                        local cf = CFrame.new(predicted - Vector3.new(0, sa, 0)) * (root.CFrame - root.Position)
                        oldRoot.CFrame = cf * CFrame.Angles(math.rad(_G.InvisStealAngle or 225), 0, 0)
                        oldRoot.AssemblyLinearVelocity = vel; oldRoot.CanCollide = false
...
-- watchdog: se o lagback/respawn quebrar o clone, reconstroi o invis
-- em vez de deixar o boneco voltar ao normal
task.spawn(function()
    while true do
        task.wait(0.3)
        if animPlaying and not _G.RecoveryInProgress then
            local character = player.Character
            local humanoid = character and character:FindFirstChildOfClass("Humanoid")
            if character and humanoid and humanoid.Health > 0 then
                local broken = (not oldRoot) or (not oldRoot.Parent)
                    or (not clone) or (not clone.Parent)
                    or (character.PrimaryPart ~= clone)
                if broken then
                    _G.RecoveryInProgress = true
                    pcall(invisTurnOff)
                    task.wait(0.2)
                    ...
                    if c2 and h2 and h2.Health > 0 then pcall(invisTurnOn) end
                    _G.RecoveryInProgress = false
                end
            end
        end
...
InfJumpState={enabled=false,conn=nil}
local function setInfiniteJump(en)
    InfJumpState.enabled=en
    Config.InfiniteJump=en
    if Config.TpSettings then Config.TpSettings.InfiniteJump=en end
    setToggle("Infinite Jump",en)
    saveConfig()
    if InfJumpState.conn then InfJumpState.conn:Disconnect(); InfJumpState.conn=nil end
    if not en then return end
    -- JumpRequest system (cleaner than Space+Heartbeat)
    InfJumpState.conn = UIS.JumpRequest:Connect(function()
        if not InfJumpState.enabled then return end
        local c = player.Character or LocalPlayer.Character
...
local _ownPlotCache = setmetatable({}, {__mode = "k"})
local _ownPlotCacheT = setmetatable({}, {__mode = "k"})
local function isOwnPlotPrompt(prompt)
    local now = os.clock()
    local t = _ownPlotCacheT[prompt]
    if t and (now - t) < 5 then return _ownPlotCache[prompt] end
    local mine = false
    local plots = workspace:FindFirstChild("Plots")
    if plots then
            ...
            if sign then
                ...
                if label then
                    ...
                    if txt:find(game.Players.LocalPlayer.Name:lower(), 1, true)
                        or txt:find(game.Players.LocalPlayer.DisplayName:lower(), 1, true) then mine = true end
                end
            end
        end
    end
    _ownPlotCache[prompt] = mine
    _ownPlotCacheT[prompt] = now
    return mine
end
local function isPromptAvailable(prompt, hrpPos)
    ...
    if isOwnPlotPrompt(prompt) then return false end
    ...
    while task.wait(_G.getSafePollRate()) do
        ...
        if autoStealEnabled and instantStealEnabled then
            ...
            for prompt in pairs(trackedPrompts) do
                ...
                elseif isPromptAvailable(prompt, myPos) then
                    local pos = getPromptPosition(prompt)
                    n = n + 1
                    _availBuf[n] = { p = prompt, d = pos and (pos - myPos).Magnitude or math.huge }
                end
            end
            for i = #_availBuf, n + 1, -1 do _availBuf[i] = nil end
            -- mais perto primeiro: evita gastar burst num alvo longe e perder o da frente
            table.sort(_availBuf, function(a, b) return a.d < b.d end)
            CONFIG.AUTO_STEAL = n > 0
            -- dispara ja neste ciclo (antes so disparava no ciclo seguinte = atraso)
            for i = 1, n do
                firePrompt(_availBuf[i].p, FIRE_BURST, FIRE_DEBOUNCE)
            end
        else
...
function runAutoSnipe()
    ...
    local ok, err = pcall(function()
        ...
        local signPart = getClosestBaseSign(targetPart)
            ...
            task.wait(0.05)
            -- Altura adaptativa: usa o piso real do alvo, não um valor fixo de dois andares.
            -- Mantém uma margem acima do piso/sinal para o Carpet registrar o destino.
            local riseTargetY
            if f2FromF1 then
                riseTargetY = math.max(signPart.Position.Y + 3, exactPos.Y + 1)
            else
                riseTargetY = math.max(signPart.Position.Y + 3, exactPos.Y + 1)
            end
...
function tpToBrainrot()
    ...
    -- Posicao viva: o alvo pode se mover (esteira / dono girando o pod),
    -- entao releamos a posicao em vez de usar um snapshot antigo.
    local function livePos(fallback)
        if targetPart and targetPart.Parent then
            local okP, pos = pcall(function()
                if targetPart:IsA("Attachment") then return targetPart.WorldPosition end
                if targetPart:IsA("BasePart") then return targetPart.Position end
                return targetPart:GetPivot().Position
            end)
            if okP and typeof(pos) == "Vector3" then return pos end
        end
        return fallback
    end
    local hadFpsCap, prevFpsCap = pcall(function() return getfpscap() end)
    if hadFpsCap and type(prevFpsCap) == "number" and prevFpsCap > 0 then
        pcall(setfpscap, 200)
    else
        hadFpsCap = false
    end
    ...
    local ok, err = pcall(function()
        ...
        local targetPos = livePos(targetPart.Position)
        -- Ja estamos em cima do alvo: nao precisa reposicionar (evita lagback a toa).
        ...
        if verticalDiff > 2 or targetPos.Y > 25 then
            ...
            task.wait(0.03)
            targetPos = livePos(targetPos)
            freeze()
            hrp.CFrame = CFrame.new(targetPos.X, targetPos.Y - 8, targetPos.Z)
            task.wait(0.05)
            freeze()
            local keep = plat
            plat = nil
            ...
            while tick() < holdEnd do
                ...
                targetPos = livePos(targetPos)
                freeze()
                -- Reaplica o CFrame apenas quando o servidor nos empurrou
                -- (lagback), em vez de forcar todo frame.
                if (hrp.Position - targetPos).Magnitude > 0.75 then
                    corrections = corrections + 1
                    if corrections > 25 then break end
    ...
    if hadFpsCap then pcall(setfpscap, prevFpsCap) end
    _G._tpBusy = false
    if not ok then
...
local function refreshBrainrotESP()
    ...
    if Config.GenValueESP then
        local raw=tostring(Config.GenValueESPMin or "")
        local manualMin=parseMinGen(raw)
        if raw=="" or raw=="0" then threshold=-1
        elseif manualMin and manualMin>0 then threshold=manualMin
        else
            local maxGV=0; local hasTop=false
            for _,d in ipairs(cache) do local v=tonumber(d.genValue or d.mpsValue) or 0; if v>maxGV then maxGV=v end; if v>=10000000 then hasTop=true end end
            threshold=hasTop and 10000000 or maxGV*0.5
        end
    end
    local seen={}
    for _,data in ipairs(cache) do if (tonumber(data.genValue or data.mpsValue) or 0)>=threshold then seen[data.uid]=true
...
local function startCarpetLock()
    ...
    local _lastCarpetCheck = 0
    carpetLockConn = RunService.Heartbeat:Connect(function()
        if not autoBuyActive then return end
        local now = os.clock()
        if (now - _lastCarpetCheck) < 0.4 then return end
        _lastCarpetCheck = now
        ensureCarpet()
    end)
end
...
RunService.Heartbeat:Connect(function()
    ...
    -- prediz o movimento da esteira pra nao ficar sempre atras do bicho
    local vel = lockedPart.AssemblyLinearVelocity
    local lead = math.clamp(_G.AutoBuyLead or 0.12, 0, 0.5)
    local above = lockedPart.Position + Vector3.new(vel.X, 0, vel.Z) * lead + Vector3.new(0, HOVER_HEIGHT, 0)
...
-- ============ AUTO DESTROY (rewrite: cache + hit real no bat) ============
local _autoDestroyBusy = false
local _sentryCache, _sentryLastScan = {}, 0
local function _isSentry(inst)
    return inst and inst.Name and inst.Name:match("^Sentry_") ~= nil
end
local function _sentryRoot(inst)
...
local function _hasExclamation(target)
    for _, d in ipairs(target:GetDescendants()) do
        if d:IsA("TextLabel") or d:IsA("TextButton") then
            local t = d.Text
            if t and t:find("!", 1, true) then return true end
        end
    end
    return false
end
-- cache incremental: scan pesado no maximo 1x a cada 3s, novos sentries entram via evento
local function _rescanSentries()
    local list = {}
    for _, inst in ipairs(Workspace:GetDescendants()) do
        if _isSentry(inst) and (inst:IsA("Model") or inst:IsA("BasePart")) then
            list[#list + 1] = inst
        end
    end
    _sentryCache = list
    _sentryLastScan = os.clock()
end
task.spawn(function()
    pcall(_rescanSentries)
    Workspace.DescendantAdded:Connect(function(inst)
        if _isSentry(inst) and (inst:IsA("Model") or inst:IsA("BasePart")) then
            _sentryCache[#_sentryCache + 1] = inst
        end
    end)
end)
local function _getBat(char)
    local bp = LocalPlayer:FindFirstChildOfClass("Backpack")
    local function pick(parent)
        if not parent then return nil end
        for _, t in ipairs(parent:GetChildren()) do
            if t:IsA("Tool") and (t.Name == "Bat" or t.Name:lower():find("bat")) then return t end
        end
        return nil
    end
    return pick(char) or pick(bp)
end
task.spawn(function()
    while true do
        task.wait(0.05)
        if false and Config.AutoDestroyTurrets and not _autoDestroyBusy and LocalPlayer:GetAttribute("Stealing") ~= true then
            local char = LocalPlayer.Character
            local hrp = char and char:FindFirstChild("HumanoidRootPart")
            local hum = char and char:FindFirstChildOfClass("Humanoid")
            if hrp and hum then
                if (os.clock() - _sentryLastScan) >= 3 then pcall(_rescanSentries) end
                local hp = hrp.Position
                local closest, closestRoot, bestDist = nil, nil, math.huge
                local alive = {}
                for _, inst in ipairs(_sentryCache) do
                    if inst.Parent then
                        alive[#alive + 1] = inst
                        local root = _sentryRoot(inst)
                        if root then
                            local dist = (hp - root.Position).Magnitude
                            if dist <= 32 and dist < bestDist and _hasExclamation(inst) then
                                bestDist, closest, closestRoot = dist, inst, root
                            end
                        end
                    end
                end
                _sentryCache = alive
                if closest then
                    _autoDestroyBusy = true
                    task.spawn(function()
                        local bat = _getBat(char)
                        if bat then
                            if bat.Parent ~= char then pcall(function() hum:EquipTool(bat) end) end
                        end
                        -- puxa a torreta pra frente e martela ate sumir (ou timeout)
                        local t0 = os.clock()
                        while closest.Parent and Config.AutoDestroyTurrets and (os.clock() - t0) < 2.5 do
                            pcall(function()
                                for _, d in ipairs(closest:GetDescendants()) do
                                    if d:IsA("BasePart") then d.CanCollide = false end
                                end
                                if closestRoot then closestRoot.CanCollide = false end
                                local myHrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
                                if myHrp then
                                    local target = CFrame.new(myHrp.Position + myHrp.CFrame.LookVector * 3.5, myHrp.Position)
                                    if closest:IsA("Model") then closest:PivotTo(target) else closest.CFrame = target end
                                end
                                local b = _getBat(LocalPlayer.Character)
                                if b then
                                    if b.Parent ~= LocalPlayer.Character then hum:EquipTool(b) end
                                    b:Activate()
                                end
                            end)
                            task.wait(0.12)
...
function makeHeader(f,t,isMain)
    ...
    local function marca(txt)
        ...
        return (txt:gsub("SXE HUB PRIVAT",(HERESY and HERESY.NOME) or "HERESY"))
    end
    if isMain then
...
function rebuildActions()
    ...
    makeQuickButton(panels["ActionsBody"],"TP no Brainrot",function()
        task.spawn(function()
            pcall(waitTpDelay)
            if _G.SXECicloTP then _G.SXECicloTP()
            elseif _G.SXE_ExecuteManualTP then _G.SXE_ExecuteManualTP()
            elseif _G.SXEStartSideTP then _G.SXEStartSideTP()
            else runAutoSnipe() end
        end)
    end,Theme.SoftAccent)
    makeQuickButton(panels["ActionsBody"],"Clone",function() task.spawn(function() local ok=instantClone(); if ShowNotification then ShowNotification("CLONE",ok and "Ativado" or "Quantum Cloner nao encontrado") end end) end,Theme.SoftAccent)
    ...
    if actionConfig["Rejoin PS"] then makeQuickButton(panels["ActionsBody"],"Rejoin PS",function() pcall(function() TeleportService:Teleport(game.PlaceId,player) end) end) end
    makeQuickButton(panels["ActionsBody"],"Server Hop",function() pcall(function() TeleportService:Teleport(game.PlaceId) end) end)
    makeQuickButton(panels["ActionsBody"],"Drop Brainrot",function() pcall(runDropBrainrot) end,Theme.SoftAccent)
    ...
    if actionConfig["Insta Reset"] ~= false then makeQuickButton(panels["ActionsBody"],"Insta Reset (G)",function() pcall(function() if executeReset then executeReset() elseif _G.executeReset then _G.executeReset() end end) end) end
...
-- MENU AI: atalho fixo no canto esquerdo para abrir as configurações de controle no PC.
do
    local aiGui = Instance.new("ScreenGui")
    aiGui.Name = "STL_MENU_AI"
    aiGui.ResetOnSpawn = false
    aiGui.IgnoreGuiInset = true
    aiGui.DisplayOrder = 100000
    aiGui.Parent = playerGui
    local aiRoot = registerScreenGui(aiGui)
    local aiButton = Instance.new("TextButton")
    aiButton.Name = "MenuAIButton"
    aiButton.Size = UDim2.fromOffset(112, 36)
    aiButton.Position = UDim2.new(0, 12, 0.5, -18)
    aiButton.BackgroundColor3 = Theme.SoftAccent
    aiButton.BackgroundTransparency = 0.08
    aiButton.BorderSizePixel = 0
    aiButton.Text = "MENU AI"
    aiButton.TextColor3 = Theme.Text
    aiButton.Font = Enum.Font.GothamBold
    aiButton.TextSize = 11
    aiButton.AutoButtonColor = false
    aiButton.Parent = aiRoot
    corner(aiButton, 7)
    stroke(aiButton, Theme.Stroke, 1, 0.35)
    aiButton.MouseEnter:Connect(function() tw(aiButton, {BackgroundColor3 = Theme.Accent}, 0.12) end)
    aiButton.MouseLeave:Connect(function() tw(aiButton, {BackgroundColor3 = Theme.SoftAccent}, 0.12) end)
    local function toggleMainSettings()
        ...
        if main.Visible then
            ...
            openAnim(main)
            task.defer(function()
                if loadTab then loadTab("Auto TP") end
            end)
        end
    end
    aiButton.MouseButton1Click:Connect(toggleMainSettings)
    _G.ToggleAIMenu = toggleMainSettings
end
do
...
-- Auto Destroy Turrets removed
        -- makeSyncStateRow(panels["StealBody"],"Auto Destroy:","Auto Destroy",function(on) Config.AutoDestroyTurrets=on; saveConfig() end)
do
...
LazyInit("Admin Panel UI", function()
    ...
    local adminTabs=Instance.new("Frame"); adminTabs.Name="AdminTabs"; adminTabs.BackgroundTransparency=1; adminTabs.Size=UDim2.new(1,-8,0,26); adminTabs.Position=UDim2.new(0,4,0,20); adminTabs.ZIndex=20; adminTabs.Parent=apOuter
    local adminPlayersTab=Instance.new("TextButton"); adminPlayersTab.Name="PlayersTab"; adminPlayersTab.Size=UDim2.new(0,100,1,0); adminPlayersTab.Text="PLAYERS"; adminPlayersTab.Font=Enum.Font.GothamBlack; adminPlayersTab.TextSize=11; adminPlayersTab.TextColor3=Color3.new(1,1,1); adminPlayersTab.BackgroundColor3=Theme.Accent; adminPlayersTab.AutoButtonColor=false; adminPlayersTab.ZIndex=21; adminPlayersTab.Parent=adminTabs; corner(adminPlayersTab,6)
    local adminBlacklistTab=Instance.new("TextButton"); adminBlacklistTab.Name="BlacklistTab"; adminBlacklistTab.Size=UDim2.new(0,100,1,0); adminBlacklistTab.Position=UDim2.new(0,106,0,0); adminBlacklistTab.Text="BLACKLIST"; adminBlacklistTab.Font=Enum.Font.GothamBlack; adminBlacklistTab.TextSize=11; adminBlacklistTab.TextColor3=Theme.Text; adminBlacklistTab.BackgroundColor3=Theme.SoftButton; adminBlacklistTab.AutoButtonColor=false; adminBlacklistTab.ZIndex=21; adminBlacklistTab.Parent=adminTabs; corner(adminBlacklistTab,6)
    apList.Position=UDim2.new(0,0,0,50)
    local blFrame=Instance.new("ScrollingFrame"); blFrame.Name="BlacklistFrame"; blFrame.BackgroundTransparency=1; blFrame.BorderSizePixel=0; blFrame.Position=UDim2.new(0,4,0,50); blFrame.Size=UDim2.new(1,-8,0,260); blFrame.AutomaticCanvasSize=Enum.AutomaticSize.Y; blFrame.CanvasSize=UDim2.new(0,0,0,0); blFrame.ScrollBarThickness=3; blFrame.Visible=false; blFrame.ZIndex=12; blFrame.Parent=apOuter
    local blLayout=Instance.new("UIListLayout"); blLayout.Padding=UDim.new(0,3); blLayout.SortOrder=Enum.SortOrder.LayoutOrder; blLayout.Parent=blFrame
    local function refreshAdminBlacklist()
        for _,child in ipairs(blFrame:GetChildren()) do if child:IsA("GuiObject") and child ~= blLayout then child:Destroy() end end
        local found=0
        for _,plr in ipairs(Players:GetPlayers()) do
            if plr ~= player and isPlayerBlacklisted(plr) then
                found=found+1
                local row=Instance.new("Frame"); row.Size=UDim2.new(1,-4,0,34); row.BackgroundColor3=Theme.Panel; row.BackgroundTransparency=0.15; row.ZIndex=13; row.Parent=blFrame; corner(row,6)
                local label=Instance.new("TextLabel"); label.Size=UDim2.new(1,-75,1,0); label.Position=UDim2.new(0,8,0,0); label.BackgroundTransparency=1; label.Text=plr.DisplayName.." (@"..plr.Name..")"; label.TextColor3=Theme.Text; label.Font=Enum.Font.GothamSemibold; label.TextSize=11; label.TextXAlignment=Enum.TextXAlignment.Left; label.ZIndex=14; label.Parent=row
                local unban=Instance.new("TextButton"); unban.Size=UDim2.new(0,58,0,24); unban.Position=UDim2.new(1,-64,0.5,-12); unban.Text="REMOVE"; unban.Font=Enum.Font.GothamBlack; unban.TextSize=9; unban.TextColor3=Color3.new(1,1,1); unban.BackgroundColor3=Theme.Red; unban.AutoButtonColor=false; unban.ZIndex=14; unban.Parent=row; corner(unban,5)
                unban.MouseButton1Click:Connect(function() _G.apBlacklist[tostring(plr.UserId)]=nil; Config.apBlacklist=_G.apBlacklist; saveConfig(); refreshAdminBlacklist(); if _G.refreshAdminPanelRows then _G.refreshAdminPanelRows() end end)
            end
        end
        if found==0 then local empty=Instance.new("TextLabel"); empty.Size=UDim2.new(1,-4,0,30); empty.BackgroundTransparency=1; empty.Text="Blacklist vazia"; empty.TextColor3=Theme.Dim; empty.Font=Enum.Font.GothamMedium; empty.TextSize=11; empty.ZIndex=13; empty.Parent=blFrame end
    end
    _G.refreshAdminBlacklistTab = refreshAdminBlacklist
    local function setAdminTab(tab)
        local players=tab=="Players"; apList.Visible=players; blFrame.Visible=not players
        adminPlayersTab.BackgroundColor3=players and Theme.Accent or Theme.SoftButton; adminPlayersTab.TextColor3=players and Color3.new(1,1,1) or Theme.Text
        adminBlacklistTab.BackgroundColor3=players and Theme.SoftButton or Theme.Accent; adminBlacklistTab.TextColor3=players and Theme.Text or Color3.new(1,1,1)
        if not players then refreshAdminBlacklist() end
    end
    adminPlayersTab.MouseButton1Click:Connect(function() setAdminTab("Players") end)
    adminBlacklistTab.MouseButton1Click:Connect(function() setAdminTab("Blacklist") end)
...
function loadTab(tabName)
    ...
    if tabName=="Keybinds" then
        for _,name in ipairs({"Kick","Rejoin Job ID","Clone","Manual TP","Invisible Steal","Job ID","Proximity","Carpet Boost","Open Menu","Ragdoll Self","Drop Brainrot","Float","Auto Buy","Click to AP","Steal Nearest","Face Nearest"}) do makeKeybindRow(mainBody,name) end
    elseif tabName=="Auto TP" then
        makeMainToggle(mainBody,"Auto TP Gen",Config.AutoTPGen,function(on)
            Config.AutoTPGen=on; if on then Config.AutoTPNearest=false; Config.AutoTPPriority=false end; saveConfig()
        end)
        makeMainToggle(mainBody,"Auto TP Nearest",Config.AutoTPNearest,function(on)
            Config.AutoTPNearest=on; if on then Config.AutoTPGen=false; Config.AutoTPPriority=false end; saveConfig()
        end)
        makeMainToggle(mainBody,"Auto TP Priority Mode",Config.AutoTPPriority,function(on)
            ...
            if on then
                ...
                Config.AutoTPGen = false
                Config.AutoTPNearest = false
            end
            saveConfig()
        end)
        ...
        makeMainTextBox(mainBody,"Delay TP (seconds)",tostring(Config.TpSettings.DelayTp or 0),"0 = instant",function(v)
            local n=tonumber(v) or 0
            Config.TpSettings.DelayTp=math.max(0,math.min(n,60))
            saveConfig()
        end)
    elseif tabName=="ESP" then
        ...
        makeMainToggle(mainBody,"Gen Value ESP",Config.GenValueESP,function(on) Config.GenValueESP=on; brainrotESPEnabled=on or brainrotESPEnabled; Config.BrainrotESP=brainrotESPEnabled; saveConfig(); if brainrotESPEnabled then task.spawn(function() pcall(refreshBrainrotESP) end) else clearBrainrotESP() end end)
        makeMainTextBox(mainBody,"Gen ESP Min Value",Config.GenValueESPMin,"50m",function(v)
            Config.GenValueESPMin=v or ""
            saveConfig()
            pcall(refreshBrainrotESP)
        end)
        makeMainToggle(mainBody,"Line To Base Rainbow",Config.LineToBase,function(on)
    ...
    elseif tabName=="UI" then
        if _G.HeresyExtras then
            ...
            makeSyncMainToggle(mainBody,"Anti Swap","Anti Swap",function(on) Config.AntiSwap=on; saveConfig() end)
        end
        for _,name in ipairs({"Invisible Steal Panel","Admin Command Panel","Actions","Command Cooldowns","Steal Panel","Steal Target"}) do
...
UIS.InputBegan:Connect(function(input,gp) if gp then return end
    ...
    local kn=_kn
    local actions={
        ["Drop Brainrot"]=runDropBrainrot, ["Insta Reset"]=function() pcall(function() if executeReset then executeReset() end end) end, ["Clone"]=instantClone,
    ...
    local function _pickByMode(pets)
        ...
        if C.AutoTPGen then
            ...
            for _, p in ipairs(pets) do
                local v = tonumber(p.mpsValue or p.mps or p.generation or 0) or 0
                if not bv or v > bv then bv, best = v, p end
            end
            if best then return best end
        end
        if C.AutoTPNearest then
            local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            if hrp then
                local best, bd
                for _, p in ipairs(pets) do
...
do
    ...
    local function construirBotaoPs()
        ...
        UIS.InputEnded:Connect(function(i)
            ...
            or i.UserInputType == Enum.UserInputType.Touch then
                if arrastando and moveu then
                    pcall(function() rememberPosition("STL HUB Ps Button", psBtn) end)
                end
                arrastando = false
            end
        end)
        ...
        pcall(function() applySavedPosition("STL HUB Ps Button", psBtn) end)
    ...
    local function construirPainel()
        ...
        painel, corpo = makeQuickPanel("STL HUB\nPanel Face Away",
        ...
        pcall(function() applySavedPosition("STL HUB\nPanel Face Away", painel) end)
    end
    local ultimoPaint = 0
