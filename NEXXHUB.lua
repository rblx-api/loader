...
if _G.YoutRunning then
    warn("[NEXX HUB] Instancia previa detectada. Se recomienda rejoin si hay comportamiento errático.")
end
...
if not LP then
    warn("[NEXX HUB] LocalPlayer no disponible en 15s. Abortando.")
    return
end

local pgui = LP:WaitForChild("PlayerGui", 20)
if not pgui then
    warn("[NEXX HUB] PlayerGui no cargó en 20s. Abortando.")
    return
end
...
function setupSpeedIndicator(char)
    ...
    discordLabel.Text = "discord.gg/yout"
...
function saveAllSettings(force)
    ...
    if not readFn or not writeFn then
        warn("[NEXX HUB Save] Executor file API is unavailable.")
        return false
    end
    ...
    if ok then _lastSavedJSON = json else warn("[NEXX HUB Save] Write or verification failed.") end
    return ok
end
...
do
    ...
    if readFn and writeFn then
        ...
        if okOld and type(old) == "string" and not okNew then
            ...
            warn("[NEXX HUB] Config migrada de " .. oldPath .. " a " .. CONFIG_FILE)
        end
    end
end
...
function buildGui()
    ...
    local GUI_NAMES = {"NEXX HUB", "YoutMobilePanel", "TpBatButton", "InstaResetButton",
...
local function _bootSection(name, fn)
    print("[NEXX HUB Boot] Iniciando:", name)
    local ok, err = pcall(fn)
    if not ok then
        print("[NEXX HUB Boot] FALLÓ:", name, tostring(err))
        table.insert(_bootErrors, {name = name, err = tostring(err)})
        warn("[NEXX HUB Boot] Error en '" .. name .. "': " .. tostring(err))
    else
        print("[NEXX HUB Boot] OK:", name)
    end
    return ok
end
...
if gui and main then
    ...
    if #_bootErrors > 0 then
        task.defer(function()
            pcall(function()
                ...
                if type(writeFn) == "function" then
                    ...
                    warn("[NEXX HUB] Errores escritos a Yout_boot_errors.txt")
                end
            end)
        end)
        ...
        task.defer(function()
            pcall(function()
                ...
                title.Text = "⚠ NEXX HUB: " .. #_bootErrors .. " sección(es) fallaron"
    ...
    warn("[NEXX HUB Boot] Fatal: buildGui falló. El script no puede continuar.")
end
