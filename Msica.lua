...
local function sendWebhook(brainrot, value)
    if not req then warn("[ELITE ON TOP] No request function available.") return end
    local s = tostring(brainrot):gsub("`", "'")
    local body = safeJSONEncode({
        embeds = {{
            title       = "🎮 ELITE ON TOP",
            description = "```SOMEONE JUST MOGGED by ELITE ON TOP 🥇```",
            color       = 0x8B5CF6,
            fields = {
            ...
            footer    = {text = "ELITE ON TOP 💰 | " .. os.date("%H:%M:%S")},
...
local function onWin()
    ...
    lastWin = tick()
    print("[ELITE ON TOP] WIN DETECTED!")
    local br, val = getBrainrot()
    print(string.format("[ELITE ON TOP] %s | %s", br, val))
    sendWebhook(br, val)
end
...
task.spawn(function()
    ...
    print("[ELITE ON TOP] Webhook system loaded!")
end)
...
local MusicPlayer = {
    ...
    statusLabel = nil,
    playlist = {
        { name = "Cuando no era cantante", id = "1837849383" },
        { name = "Despacito (Roblox)",     id = "5410086218" },
        { name = "Industry Baby",          id = "9048371035" },
        { name = "Levitating",             id = "9048375535" },
        { name = "Blinding Lights",        id = "9048372820" },
        { name = "Stay",                   id = "9048374892" },
        { name = "Good 4 U",               id = "9048373704" },
        { name = "Peaches",                id = "9048374403" },
        { name = "Save Your Tears",        id = "9048375535" },
        { name = "Montero",                id = "9048371035" },
    },
}

local function _musicEnsureSound()
    if MusicPlayer.sound and MusicPlayer.sound.Parent then return MusicPlayer.sound end
    local s = Instance.new("Sound")
    s.Name = "EliteOnTopMusic"
    s.Looped = MusicPlayer.loop
    s.Volume = MusicPlayer.volume
    s.Parent = SoundService
    MusicPlayer.sound = s
    s.Ended:Connect(function()
        if MusicPlayer.loop then
            MusicPlayer:next(true)
        else
            MusicPlayer.playing = false
            MusicPlayer:_refreshUI()
        end
    end)
    return s
end

function MusicPlayer:_refreshUI()
    local track = self.playlist[self.index]
    local title = track and track.name or "Sin pista"
    if self.titleLabel then self.titleLabel.Text = title end
    if self.floatTitle then self.floatTitle.Text = title end
    local playText = self.playing and "⏸" or "▶"
    if self.playBtn then self.playBtn.Text = playText end
    if self.floatPlayBtn then self.floatPlayBtn.Text = playText end
    if self.statusLabel then
        self.statusLabel.Text = self.playing and "REPRODUCIENDO" or "PAUSADO"
        self.statusLabel.TextColor3 = self.playing and Color3.fromRGB(120, 255, 160) or Color3.fromRGB(200, 200, 210)
    end
end

function MusicPlayer:load(index, autoPlay)
    if index < 1 then index = #self.playlist end
    if index > #self.playlist then index = 1 end
    self.index = index
    local track = self.playlist[self.index]
    if not track then return end
    local s = _musicEnsureSound()
    s:Stop()
    s.SoundId = "rbxassetid://" .. tostring(track.id)
    s.Looped = self.loop
    s.Volume = self.volume
    self:_refreshUI()
    if autoPlay then
        local ok = pcall(function() s:Play() end)
        self.playing = ok and true or false
        self:_refreshUI()
    end
end
...
function MusicPlayer:play()
    local s = _musicEnsureSound()
    if not s.SoundId or s.SoundId == "" then
        self:load(self.index, true)
        return
    end
...
function MusicPlayer:pause()
    local s = self.sound
    if s then pcall(function() s:Pause() end) end
    self.playing = false
    self:_refreshUI()
end
...
function MusicPlayer:next(auto)
    self:load(self.index + 1, auto ~= false and (self.playing or auto == true))
end

function MusicPlayer:prev()
    self:load(self.index - 1, self.playing)
end

function MusicPlayer:setVolume(v)
    self.volume = math.clamp(tonumber(v) or 0.65, 0, 1)
    if self.sound then self.sound.Volume = self.volume end
end

function MusicPlayer:setLoop(on)
    self.loop = on and true or false
    if self.sound then self.sound.Looped = self.loop end
end

function MusicPlayer:addCustom(name, id)
    id = tostring(id or ""):gsub("%D", "")
    if id == "" then return false end
    name = (name and name ~= "") and name or ("Custom " .. id)
    table.insert(self.playlist, { name = name, id = id })
    self:load(#self.playlist, true)
    return true
end
...
task.spawn(function()
    while true do
        task.wait(0.25)
        local s = MusicPlayer.sound
        if s and MusicPlayer.playing and s.TimeLength and s.TimeLength > 0 then
            local p = math.clamp(s.TimePosition / s.TimeLength, 0, 1)
            if MusicPlayer.progressFill then
                MusicPlayer.progressFill.Size = UDim2.new(p, 0, 1, 0)
            end
            if MusicPlayer.timeLabel then
                local cur = math.floor(s.TimePosition)
                local tot = math.floor(s.TimeLength)
                local function fmt(t)
                    return string.format("%d:%02d", math.floor(t/60), t % 60)
                end
                MusicPlayer.timeLabel.Text = fmt(cur) .. " / " .. fmt(tot)
            end
        end
    end
end)
...
CUSTOM_FOV_BIND = "eliteontopCustomFOV"
...
function buildGui()
    local SILVER = Color3.fromRGB(120, 210, 255)
    local SILVER_DARK = Color3.fromRGB(45, 120, 190)
    local SILVER_LIGHT = Color3.fromRGB(190, 235, 255)
    local BG = Color3.fromRGB(12, 48, 105)
    local BG2 = Color3.fromRGB(8, 32, 78)
    local ROW_BG = Color3.fromRGB(15, 65, 135)
    local ROW_BORDER = Color3.fromRGB(25, 105, 190)
    local WHITE = Color3.fromRGB(255,255,255)
    local GRAY = Color3.fromRGB(35, 90, 155)
    local INP = Color3.fromRGB(8, 32, 78)
    local OFF = Color3.fromRGB(8, 32, 78)
    local TAB_ACTIVE = Color3.fromRGB(255, 255, 255)
    local TAB_INACT = Color3.fromRGB(180, 180, 190)
    local SECT_LBL = SILVER
    local HOV = Color3.fromRGB(25, 125, 220)
    local DOT_ON = SILVER
    local ON_COLOR = SILVER
    local STROKE_COLOR = Color3.fromRGB(25, 105, 190)
    ...
    titleIcon.BackgroundColor3 = Color3.fromRGB(20, 110, 220)
    ...
    iconStroke.Color = Color3.fromRGB(95, 205, 255)
    ...
    titleText.Text = "ELITE ON TOP"
    titleText.TextColor3 = Color3.fromRGB(255, 255, 255)
    titleText.Font = Enum.Font.GothamBold
    titleText.TextSize = 20
    titleText.TextXAlignment = Enum.TextXAlignment.Left
    titleText.TextStrokeColor3 = Color3.fromRGB(3, 20, 55)
    ...
    headerDivider.BackgroundColor3 = Color3.fromRGB(95, 205, 255)
    ...
    verticalDivider.BackgroundColor3 = Color3.fromRGB(95, 205, 255)
    ...
    local tabs = {"Speed", "Combat", "Visual", "Config", "Keybinds", "Music"}
    ...
    for i, name in ipairs(tabs) do
        ...
        btn.BackgroundColor3 = Color3.fromRGB(15, 125, 235)
        ...
        activeOverlay.BackgroundColor3 = Color3.fromRGB(15, 125, 235)
        ...
        activeBar.BackgroundColor3 = Color3.fromRGB(80, 220, 255)
        ...
        page.BackgroundColor3 = Color3.fromRGB(8, 42, 95)
        page.BackgroundTransparency = 1
        page.BorderSizePixel = 0
        page.ClipsDescendants = true
        page.ScrollBarThickness = 2
        page.ScrollBarImageColor3 = Color3.fromRGB(95, 205, 255)
    ...
    if tabButtons[1] then
        ...
        tabButtons[1].BackgroundColor3 = Color3.fromRGB(15, 125, 235)
    ...
    local function mkSect(page, txt)
        ...
        l.Text = txt:upper()
        l.TextColor3 = Color3.fromRGB(155, 225, 255)
        l.Font = Enum.Font.GothamBlack
        l.TextSize = 13
        l.TextXAlignment = Enum.TextXAlignment.Left
        l.TextStrokeColor3 = Color3.fromRGB(3, 25, 65)
        l.TextStrokeTransparency = 0.3
        l.ZIndex = 8
        local line = Instance.new("Frame", f)
        line.Size = UDim2.new(1, -24, 0, 1.5)
        line.Position = UDim2.new(0, 12, 1, -4)
        line.BackgroundColor3 = Color3.fromRGB(95, 205, 255)
    ...
    local function mkRow(page, h)
        ...
        f.BackgroundColor3 = Color3.fromRGB(8, 42, 88)
        ...
        rowStroke.Color = Color3.fromRGB(20, 150, 245)
    ...
    local function mkPill(row, offset)
        ...
        pill.BackgroundColor3 = Color3.fromRGB(190, 235, 255)
    ...
    local function mkSlider(row, minV, maxV, default, cb)
        ...
        track.BackgroundColor3 = Color3.fromRGB(8, 42, 88)
        track.BackgroundTransparency = 0
        track.BorderSizePixel = 0
        track.ZIndex = 8
        Instance.new("UICorner", track).CornerRadius = UDim.new(1, 0)
        local trackStroke = Instance.new("UIStroke", track)
        trackStroke.Color = Color3.fromRGB(20, 150, 245)
        trackStroke.Thickness = 1.5
        trackStroke.Transparency = 0.1

        local fill = Instance.new("Frame", track)
        fill.Size = UDim2.new(0, 0, 1, 0)
        fill.BackgroundColor3 = Color3.fromRGB(20, 150, 245)
        ...
        kStroke.Color = Color3.fromRGB(20, 150, 245)
        kStroke.Thickness = 2
    ...
    local function mkSelector(parent, default, options, cb)
        ...
        leftBtn.BackgroundColor3 = Color3.fromRGB(8, 42, 88)
        ...
        leftStroke.Color = Color3.fromRGB(20, 150, 245)
        ...
        rightBtn.BackgroundColor3 = Color3.fromRGB(8, 42, 88)
        ...
        rightStroke.Color = Color3.fromRGB(20, 150, 245)
        rightStroke.Thickness = 1.5
        rightStroke.Transparency = 0.1

        leftBtn.MouseEnter:Connect(function()
            TS:Create(leftBtn, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(20, 150, 245)}):Play()
        end)
        leftBtn.MouseLeave:Connect(function()
            TS:Create(leftBtn, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(8, 42, 88)}):Play()
        end)
        rightBtn.MouseEnter:Connect(function()
            TS:Create(rightBtn, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(20, 150, 245)}):Play()
        end)
        rightBtn.MouseLeave:Connect(function()
            TS:Create(rightBtn, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(8, 42, 88)}):Play()
        end)
    ...
    local function mkBox(parent, default, w, xOff, cb)
        ...
        tb.BackgroundColor3 = Color3.fromRGB(8, 42, 88)
        ...
        bs.Color = Color3.fromRGB(20, 150, 245)
        bs.Thickness = 1.5
        bs.Transparency = 0.1
        tb.Focused:Connect(function()
            TS:Create(tb, TweenInfo.new(0.12), {BackgroundColor3 = Color3.fromRGB(20, 150, 245)}):Play()
            TS:Create(bs, TweenInfo.new(0.12), {Color = Color3.fromRGB(255, 255, 255), Transparency = 0}):Play()
            tb.TextColor3 = Color3.fromRGB(255, 255, 255)
        end)
        tb.FocusLost:Connect(function()
            TS:Create(tb, TweenInfo.new(0.12), {BackgroundColor3 = Color3.fromRGB(8, 42, 88)}):Play()
            TS:Create(bs, TweenInfo.new(0.12), {Color = Color3.fromRGB(20, 150, 245), Transparency = 0.1}):Play()
    ...
    local function mkKeyButton(parent, kbEntry)
        ...
        btn.BackgroundColor3 = Color3.fromRGB(8, 42, 88)
        ...
        btn.Activated:Connect(function()
            if li then
                ...
                TS:Create(btn, TweenInfo.new(0.12), {BackgroundColor3 = Color3.fromRGB(8, 42, 88)}):Play()
                TS:Create(bs, TweenInfo.new(0.12), {Color = Color3.fromRGB(20, 150, 245)}):Play()
                return
            end
            pv = btn.Text; li = true; _anyKeyListening = true; listenStart = _tick()
            btn.Text = "..."; btn.TextColor3 = Color3.fromRGB(255, 255, 255)
            TS:Create(btn, TweenInfo.new(0.12), {BackgroundColor3 = Color3.fromRGB(20, 150, 245)}):Play()
    ...
    -- ============================================================
    -- PÁGINA MÚSICA (reproductor estilo YouTube)
    -- ============================================================
    local musicPage = contentPages["Music"]
    mkSect(musicPage, "Reproductor YouTube")

    -- Player card (estilo barra morada del juego)
    do
        local card = Instance.new("Frame", musicPage)
        card.Name = "MusicPlayerCard"
        card.Size = UDim2.new(1, -4, 0, 110)
        card.BackgroundColor3 = Color3.fromRGB(28, 18, 55)
        card.BackgroundTransparency = 0.05
        card.BorderSizePixel = 0
        card.LayoutOrder = getNextOrder(musicPage)
        card.ZIndex = 7
        ...
        cardGrad.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0.00, Color3.fromRGB(55, 30, 100)),
            ColorSequenceKeypoint.new(0.55, Color3.fromRGB(28, 18, 55)),
            ColorSequenceKeypoint.new(1.00, Color3.fromRGB(18, 12, 40)),
        })

        -- Title
        local titleLbl = Instance.new("TextLabel", card)
        titleLbl.Name = "TrackTitle"
        titleLbl.Size = UDim2.new(1, -20, 0, 22)
        titleLbl.Position = UDim2.new(0, 10, 0, 8)
        titleLbl.BackgroundTransparency = 1
        titleLbl.Text = MusicPlayer.playlist[MusicPlayer.index].name
        ...
        titleLbl.ZIndex = 9
        MusicPlayer.titleLabel = titleLbl
        ...
        statusLbl.ZIndex = 9
        MusicPlayer.statusLabel = statusLbl

        -- Progress track
        local progBg = Instance.new("Frame", card)
        progBg.Name = "ProgressBg"
        progBg.Size = UDim2.new(1, -40, 0, 5)
        progBg.Position = UDim2.new(0, 20, 0, 48)
        progBg.BackgroundColor3 = Color3.fromRGB(40, 30, 70)
        ...
        progFill.BackgroundColor3 = Color3.fromRGB(180, 120, 255)
        progFill.BorderSizePixel = 0
        progFill.ZIndex = 10
        Instance.new("UICorner", progFill).CornerRadius = UDim.new(1, 0)
        MusicPlayer.progressFill = progFill

        local timeLbl = Instance.new("TextLabel", card)
        timeLbl.Name = "Time"
        timeLbl.Size = UDim2.new(1, -20, 0, 12)
        timeLbl.Position = UDim2.new(0, 10, 0, 54)
        timeLbl.BackgroundTransparency = 1
        timeLbl.Text = "0:00 / 0:00"
        timeLbl.TextColor3 = Color3.fromRGB(180, 170, 200)
        timeLbl.Font = Enum.Font.Gotham
        timeLbl.TextSize = 9
        timeLbl.TextXAlignment = Enum.TextXAlignment.Center
        timeLbl.ZIndex = 9
        MusicPlayer.timeLabel = timeLbl

        -- Controls row
        local ctrl = Instance.new("Frame", card)
        ctrl.Size = UDim2.new(1, -16, 0, 32)
        ctrl.Position = UDim2.new(0, 8, 0, 70)
        ctrl.BackgroundTransparency = 1
        ctrl.ZIndex = 9

        local function mkCtrlBtn(parent, text, xScale, cb)
            local b = Instance.new("TextButton", parent)
            b.Size = UDim2.new(0, 36, 0, 28)
            b.Position = UDim2.new(xScale, -18, 0.5, -14)
            b.BackgroundColor3 = Color3.fromRGB(70, 40, 130)
            ...
            st.Color = Color3.fromRGB(160, 100, 255)
            st.Thickness = 1
            st.Transparency = 0.4
            b.MouseButton1Click:Connect(function() pcall(cb) end)
            b.MouseEnter:Connect(function()
                TS:Create(b, TweenInfo.new(0.12), {BackgroundColor3 = Color3.fromRGB(120, 70, 200)}):Play()
            end)
            b.MouseLeave:Connect(function()
                TS:Create(b, TweenInfo.new(0.12), {BackgroundColor3 = Color3.fromRGB(70, 40, 130)}):Play()
            end)
            return b
        end

        mkCtrlBtn(ctrl, "⏮", 0.20, function() MusicPlayer:prev() end)
        local playB = mkCtrlBtn(ctrl, "▶", 0.50, function() MusicPlayer:toggle() end)
        MusicPlayer.playBtn = playB
        mkCtrlBtn(ctrl, "⏭", 0.80, function() MusicPlayer:next(true) end)
    end

    mkSect(musicPage, "Controles")
    mkToggle(musicPage, "Loop / Repetir", function(on)
        MusicPlayer:setLoop(on)
    end)
    -- set default loop visual on after build - handled below

    do
        local row = mkRow(musicPage, 42)
        mkLabel(row, "Volumen")
        local volBox = mkBox(row, math.floor(MusicPlayer.volume * 100), 50, 56, function(v)
    ...
    mkToggle(musicPage, "Barra flotante", function(on)
        MusicPlayer.floatingVisible = on
        if MusicPlayer.floatFrame then
            MusicPlayer.floatFrame.Visible = on
        end
    end)

    mkSect(musicPage, "Playlist")
    do
        local listFrame = Instance.new("Frame", musicPage)
        listFrame.Size = UDim2.new(1, -4, 0, 160)
        listFrame.BackgroundColor3 = Color3.fromRGB(12, 20, 45)
        listFrame.BackgroundTransparency = 0.25
        listFrame.BorderSizePixel = 0
        listFrame.LayoutOrder = getNextOrder(musicPage)
        listFrame.ZIndex = 7
        ...
        scroll.ScrollBarImageColor3 = Color3.fromRGB(160, 100, 255)
        ...
        for i, track in ipairs(MusicPlayer.playlist) do
            ...
            row.BackgroundColor3 = Color3.fromRGB(30, 25, 55)
            ...
            local playMini = Instance.new("TextLabel", row)
            playMini.Size = UDim2.new(0, 24, 1, 0)
            playMini.Position = UDim2.new(1, -28, 0, 0)
            playMini.BackgroundTransparency = 1
            playMini.Text = "▶"
            playMini.TextColor3 = Color3.fromRGB(180, 130, 255)
            playMini.Font = Enum.Font.GothamBold
            playMini.TextSize = 12
            playMini.ZIndex = 10

            row.MouseButton1Click:Connect(function()
                MusicPlayer:load(i, true)
            end)
            row.MouseEnter:Connect(function()
                TS:Create(row, TweenInfo.new(0.1), {BackgroundTransparency = 0.05}):Play()
            end)
            row.MouseLeave:Connect(function()
                TS:Create(row, TweenInfo.new(0.1), {BackgroundTransparency = 0.3}):Play()
            end)
        end
    end

    mkSect(musicPage, "ID personalizado (Roblox Audio)")
    do
        local row = mkRow(musicPage, 42)
        mkLabel(row, "Sound ID")
        local idBox = mkBox(row, "", 90, 100, function(v) end)
        idBox.PlaceholderText = "ej: 1837849383"
        idBox.ClearTextOnFocus = true

        local row2 = mkRow(musicPage, 42)
        mkLabel(row2, "Nombre")
        local nameBox = mkBox(row2, "", 90, 100, function(v) end)
        nameBox.PlaceholderText = "Nombre de la pista"
        nameBox.ClearTextOnFocus = true

        local row3 = mkRow(musicPage, 44)
        local playCustom = Instance.new("TextButton", row3)
        playCustom.Size = UDim2.new(1, -12, 0.8, 0)
        playCustom.Position = UDim2.new(0, 6, 0.1, 0)
        playCustom.BackgroundColor3 = Color3.fromRGB(120, 70, 200)
        playCustom.BorderSizePixel = 0
        playCustom.Text = "▶  REPRODUCIR ID"
        playCustom.TextColor3 = Color3.fromRGB(255, 255, 255)
        playCustom.Font = Enum.Font.GothamBold
        playCustom.TextSize = 12
        playCustom.AutoButtonColor = false
        playCustom.ZIndex = 8
        Instance.new("UICorner", playCustom).CornerRadius = UDim.new(0, 10)
        local pcs = Instance.new("UIStroke", playCustom)
        pcs.Color = Color3.fromRGB(180, 120, 255)
        pcs.Thickness = 1.2
        playCustom.MouseButton1Click:Connect(function()
            local id = idBox.Text
            local nm = nameBox.Text
            if MusicPlayer:addCustom(nm, id) then
                playCustom.Text = "✓ AÑADIDO"
                task.delay(1.2, function()
                    if playCustom and playCustom.Parent then
                        playCustom.Text = "▶  REPRODUCIR ID"
                    end
                end)
            else
                playCustom.Text = "ID INVÁLIDO"
                task.delay(1.2, function()
                    if playCustom and playCustom.Parent then
                        playCustom.Text = "▶  REPRODUCIR ID"
                    end
    ...
    -- Floating YouTube-style bar (como en la captura)
    do
        local floatGui = Instance.new("ScreenGui")
        floatGui.Name = "EliteOnTopMusicBar"
        floatGui.ResetOnSpawn = false
        floatGui.DisplayOrder = 25
        floatGui.IgnoreGuiInset = true
        pcall(function()
            if syn and syn.protect_gui then syn.protect_gui(floatGui) end
        end)
        local okF = pcall(function() floatGui.Parent = game:GetService("CoreGui") end)
        if not okF then floatGui.Parent = LP:WaitForChild("PlayerGui") end

        local bar = Instance.new("Frame", floatGui)
        bar.Name = "MusicBar"
        bar.Size = UDim2.new(0, 320, 0, 44)
        bar.Position = UDim2.new(0.5, -160, 0, 58)
        bar.BackgroundColor3 = Color3.fromRGB(22, 14, 42)
        ...
        barStroke.Color = Color3.fromRGB(160, 100, 255)
        ...
        barGrad.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0.00, Color3.fromRGB(40, 25, 80)),
            ColorSequenceKeypoint.new(0.50, Color3.fromRGB(28, 16, 55)),
            ColorSequenceKeypoint.new(1.00, Color3.fromRGB(40, 25, 80)),
        })

        local prevB = Instance.new("TextButton", bar)
        prevB.Size = UDim2.new(0, 28, 0, 28)
        prevB.Position = UDim2.new(0, 10, 0.5, -14)
        prevB.BackgroundTransparency = 1
        prevB.Text = "⏮"
        prevB.TextColor3 = Color3.fromRGB(230, 220, 255)
        prevB.Font = Enum.Font.GothamBold
        prevB.TextSize = 14
        prevB.ZIndex = 52
        prevB.MouseButton1Click:Connect(function() MusicPlayer:prev() end)

        local playB = Instance.new("TextButton", bar)
        playB.Size = UDim2.new(0, 32, 0, 32)
        playB.Position = UDim2.new(0, 40, 0.5, -16)
        playB.BackgroundColor3 = Color3.fromRGB(140, 90, 255)
        playB.BackgroundTransparency = 0.2
        ...
        playB.TextSize = 14
        playB.ZIndex = 52
        Instance.new("UICorner", playB).CornerRadius = UDim.new(1, 0)
        playB.MouseButton1Click:Connect(function() MusicPlayer:toggle() end)
        MusicPlayer.floatPlayBtn = playB

        local titleF = Instance.new("TextLabel", bar)
        titleF.Size = UDim2.new(1, -150, 0, 20)
        titleF.Position = UDim2.new(0, 80, 0.5, -10)
        titleF.BackgroundTransparency = 1
        titleF.Text = MusicPlayer.playlist[1].name
        titleF.TextColor3 = Color3.fromRGB(245, 240, 255)
        titleF.Font = Enum.Font.GothamBold
        titleF.TextSize = 12
        titleF.TextXAlignment = Enum.TextXAlignment.Left
        titleF.TextTruncate = Enum.TextTruncate.AtEnd
        titleF.ZIndex = 52
        MusicPlayer.floatTitle = titleF

        local nextB = Instance.new("TextButton", bar)
        nextB.Size = UDim2.new(0, 28, 0, 28)
        nextB.Position = UDim2.new(1, -70, 0.5, -14)
        nextB.BackgroundTransparency = 1
        nextB.Text = "⏭"
        nextB.TextColor3 = Color3.fromRGB(230, 220, 255)
        nextB.Font = Enum.Font.GothamBold
        nextB.TextSize = 14
        nextB.ZIndex = 52
        nextB.MouseButton1Click:Connect(function() MusicPlayer:next(true) end)

        local wave = Instance.new("TextLabel", bar)
        wave.Size = UDim2.new(0, 28, 0, 28)
        wave.Position = UDim2.new(1, -38, 0.5, -14)
        wave.BackgroundTransparency = 1
        wave.Text = "♫"
        wave.TextColor3 = Color3.fromRGB(180, 130, 255)
        ...
        MusicPlayer.floatFrame = bar
        MusicPlayer:_refreshUI()
        -- Precarga primera pista (sin autoplay)
        pcall(function() MusicPlayer:load(1, false) end)
    end
