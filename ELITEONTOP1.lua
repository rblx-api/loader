...
local ELITEONTOP1_LANGUAGE = "en"

local ELITEONTOP1_TRANSLATIONS = {
...
local function localizeEliteOnTop1Interface(root)
    local dictionary = ELITEONTOP1_TRANSLATIONS[ELITEONTOP1_LANGUAGE]
...
function setupSpeedIndicator(char)
    ...
    discordLabel.Text = "discord.gg/eliteontop1"
...
-- [ELITEONTOP1] Marcador azul de ultima posicion
do
    local ELITEONTOP1_MARKER_DISTANCE   = 3
    local ELITEONTOP1_MIN_X, ELITEONTOP1_MAX_X = -536.2, -422
    local ELITEONTOP1_MIN_Y, ELITEONTOP1_MAX_Y = -10,    75
    local ELITEONTOP1_MIN_Z, ELITEONTOP1_MAX_Z = -71.8, 192.9
    local ELITEONTOP1_GROUND_Y          = -7
    local ELITEONTOP1_LOOKBACK          = 0.2
    local ELITEONTOP1_SAMPLE_INTERVAL   = 1 / 60
    ...
    local function isInsideAllowedArea(pos)
        ...
        return pos.X >= ELITEONTOP1_MIN_X and pos.X <= ELITEONTOP1_MAX_X
           and pos.Y >= ELITEONTOP1_MIN_Y and pos.Y <= ELITEONTOP1_MAX_Y
           and pos.Z >= ELITEONTOP1_MIN_Z and pos.Z <= ELITEONTOP1_MAX_Z
    end
    ...
    local function getLastMarkerCFrame()
        ...
        local teleportPos = marker.Position + away.Unit * ELITEONTOP1_MARKER_DISTANCE
    ...
    local function snapshotBeforeExit(history, now)
        ...
        local cutoff = now - ELITEONTOP1_LOOKBACK
        local selected
            ...
            if root and hum and hum.Health > 0 and finiteVector(root.Position) then
                if isInsideAllowedArea(root.Position) then
                    ...
                    while sample.history[1]
                        and now - sample.history[1].time > (ELITEONTOP1_LOOKBACK + 0.25) do
    ...
    local monitorConn = RunService.Heartbeat:Connect(function(dt)
        ...
        if markerState.accumulator < ELITEONTOP1_SAMPLE_INTERVAL then return end
        markerState.accumulator = markerState.accumulator % ELITEONTOP1_SAMPLE_INTERVAL
        ...
        task.defer(function()
            pcall(function()
                ...
                if type(writeFn) == "function" then
                    ...
                    pcall(writeFn, "EliteOnTop1_boot_errors.txt", log)
                    warn("[ELITE ON TOP 1] Errores escritos a EliteOnTop1_boot_errors.txt")
                end
            end)
        end)
