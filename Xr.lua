...
local function setupSpeedIndicator(char)
    ...
    local oldBB = head:FindFirstChild("namiSpeedIndicator")
    if oldBB then oldBB:Destroy() end
    local bb = Instance.new("BillboardGui", head)
    bb.Name = "namiSpeedIndicator"
...
-- ====== CONFIGURACIÓN ======
local CONFIG_FILE = "Nami_config.json"
...
local function deleteAllSettings()
    ...
    if isfile and isfile("Nami_PanelPos.txt") then
        pcall(delfile, "Nami_PanelPos.txt")
    end
    resetToDefaults()
    if pbFrame then
...
local INP     = Color3.fromRGB(0, 0, 0)
    ...
    local old=game:GetService("CoreGui"):FindFirstChild("Nami_hub")
    if old then old:Destroy() end
    local pg=LP:FindFirstChild("PlayerGui")
    if pg then
        local o=pg:FindFirstChild("Nami_hub")
...
local STEAL_COOLDOWN = 0.1
local PROMPT_CACHE_REFRESH = 0.15
local STEAL_STATE_FILE = "Nami_StealState.json"
...
do
    ...
    if type(loadAllSettings) == "function" then
        ...
        local ok, data = pcall(function()
            if isfile and isfile("Nami_config.json") then
                return game:GetService("HttpService"):JSONDecode(readfile("Nami_config.json"))
            end
        end)
