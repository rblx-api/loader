-- Elite.VS.duels (Recoded with Burst Reset, Amazon Unboxed Animation, and New Song)
...
-- ===== BURST RESET (v4gg.xyz 0.1s) =====
_G._AceResetting = false
_G.AceBurstReset = function()
    if _G._AceResetting then return end
    _G._AceResetting = true
    ...
    mt.__index = newcclosure(function(s, k)
        if k == "WalkSpeed" and _G._AceResetting then return 16 end
        return oldIdx(s, k)
    end)
    setreadonly(mt, true)
...
function cursedInstaReset() return _G.AceBurstReset() end

-- Remove old GUIs
for _, name in ipairs({"EliteVSDuelsAdaptReconstruct", "EliteVSDuelsAdaptReconstruct", "AdaptHubPolished", "CyberHub"}) do
    local old = PlayerGui:FindFirstChild(name)
    if old then old:Destroy() end
end

-- (All original speed/aimbot/steal/etc. variables remain unchanged – they are the "ace logics")
local NS = 59.5
local CS = 28.8
...
-- Separate visual setter variables for Normal and Anti Bypass
local setNormalAimbotVisual = nil
local setAntiBypassAimbotVisual = nil

local DEFAULT_SPEED_KEYBINDS = {
    SpeedToggle = Enum.KeyCode.Q,
    LaggerToggle = Enum.KeyCode.R,
    DropBrainrot = Enum.KeyCode.X,
    Aimbot = Enum.KeyCode.E,                -- Normal Bat Aimbot
    BatAimbot = Enum.KeyCode.G,             -- Anti Bypass Bat V2
...
local speedKeybinds = {
    ...
    BatAimbot = DEFAULT_SPEED_KEYBINDS.BatAimbot,
...
function _G.AceStopNormalInfJumpHoldState()
    ...
    S.holdPressed = false; S.holdActive = false; S.controllerActive = false; S.mobilePressed = false; S.mobileActive = false
end
...
UserInputService.JumpRequest:Connect(function() _G.AceApplyNormalInfJumpBoost(50) end)
...
UserInputService.InputEnded:Connect(function(input)
    local S = _G.AceNormalInfJump
    if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == Enum.KeyCode.Space then
        S.holdPressed = false; S.holdActive = false
    end
...
for _, obj in ipairs(PlayerGui:GetDescendants()) do _G.AceHookNormalInfMobileJumpButton(obj) end
PlayerGui.DescendantAdded:Connect(function(obj) task.defer(_G.AceHookNormalInfMobileJumpButton, obj) end)
...
setInfJumpInternal = function(on)
    ...
    if not infJumpEnabled then _G.AceStopNormalInfJumpHoldState() end
end

local currentBackground = 0
local aceGuiScaleValue = 0.52
local aceProgressBarScaleValue = 0.83
CONFIG_FILE = "EliteVSDuels_MainGUI_Config.json"
KEYBINDS_CONFIG_FILE = "EliteVSDuels_Keybinds.json"
...
INTRO_MUSIC_OPTIONS = {
    {name="Song 1", url="https://files.catbox.moe/zn48ws.mp3", file="AceDuelSong_477474.mp3"},
    {name="Song 2", url="https://files.catbox.moe/j5mj01.mp3", file="AceDuelSong_73737373.mp3"},
    {name="Song 3", url="https://files.catbox.moe/qs56vd.mp3", file="AceDuelSong_526789.mp3"},
    {name="Song 4", url="https://files.catbox.moe/3dg0k2.mp3", file="AceDuelSong_6738.mp3"},
    {name="Song 5", url="https://files.catbox.moe/rkq06s.mp3", file="AceDuelSong_466464.mp3"},
    {name="Song 6", url="https://files.catbox.moe/6eigoh.mp3", file="AceDuelSong_477745.mp3"},
    {name="Song 7", url="https://files.catbox.moe/dvjtjk.mp3", file="AceDuelSong_32789.mp3"},
    {name="Song 8", url="https://files.catbox.moe/iknfuh.mp3", file="AceDuelSong_7484848.mp3"},
    {name="Song 9", url="https://files.catbox.moe/mzvrir.mp3", file="AceDuelSong_7383828.mp3"},
    {name="Song 10", url="https://files.catbox.moe/rcgr9f.mp3", file="AceDuelSong_378392.mp3"},
    -- NEW SONG
    {name="Song 11", url="https://files.catbox.moe/tjpo3s.mp3", file="AceDuelSong_2528448.mp3"},
}
...
function cacheIntroSong(option, allowDownload)
    ...
    local fileName = option.file or ("EliteVSDuelSong_" .. tostring(option.name or "song") .. ".mp3")
...
function makeIntroSoundFromId(soundId, name, parent)
    ...
    sound.Name = name or "EliteVSDuelsIntroMusic"
    sound.Volume = 0.65
    sound.Looped = false
...
function previewIntroMusic(index)
    ...
    task.spawn(function()
        ...
        local sound = createIntroSound(option, "EliteVSDuelsIntroPlayback_" .. tostring(token), SoundService, true)
        if token ~= introPlaybackToken then if sound then sound:Destroy() end; return end
...
function playIntroMusic()
    stopIntroPreview(); stopIntroPlayback()
    ...
    introPlaybackToken = token
    task.spawn(function()
        local sound = createIntroSound(option, "EliteVSDuelsIntroMusic_" .. tostring(token), SoundService, true)
...
function collectAceMobileButtonPositions()
    ...
    if next(out) == nil and type(_G.AceMobileButtonPositions) == "table" then return _G.AceMobileButtonPositions end
...
function keybindsToTable()
    local out = {}
    for keyId in pairs(DEFAULT_SPEED_KEYBINDS) do out[keyId] = keyToString(speedKeybinds[keyId]) end
    for keyId, key in pairs(speedKeybinds) do out[keyId] = keyToString(key) end
    return out
end
...
function applySavedKeybinds(t)
    ...
    for keyId in pairs(speedKeybinds) do
        if t[keyId] ~= nil then speedKeybinds[keyId] = stringToKeyCode(t[keyId]) end
    end
end
function applyDefaultAceKeybinds()
    for keyId, key in pairs(DEFAULT_SPEED_KEYBINDS) do speedKeybinds[keyId] = key end
...
function collectAceConfig()
    return {
        ...
        NS = NS, CS = CS, LAGGER_SPEED = LAGGER_SPEED, LAGGER_CARRY_SPEED = LAGGER_CARRY_SPEED,
...
function loadAceConfig()
    ...
    local ok, data = pcall(function() return HttpService:JSONDecode(_ace_readfile(CONFIG_FILE)) end)
    ...
    if keybindData.tpDownKeybind ~= nil then
        if tostring(keybindData.tpDownKeybind) == "None" then tpDownKeybind = nil
        else tpDownKeybind = stringToKeyCode(keybindData.tpDownKeybind) or DEFAULT_TP_DOWN_KEYBIND end
    end
    for keyId, defaultKey in pairs(DEFAULT_SPEED_KEYBINDS) do
        local savedKeys = keybindData and keybindData.keybinds
        if (not savedKeys or savedKeys[keyId] == nil) and speedKeybinds[keyId] == nil then speedKeybinds[keyId] = defaultKey end
    end
...
local function syncAnimationPackIndex()
    ...
    selectedAnimationPack = "OFF"; AnimationPackIndex = 1
end
...
-- Auto Reset on Med (UPDATED: uses burst reset)
...
function _G.AceAutoResetOnAnchorChanged(part)
    return part:GetPropertyChangedSignal("Anchored"):Connect(function() _G.AceAutoResetFireOnce(part) end)
end
function _G.AceStopAutoResetOnMed()
    local state = _G.AceAutoResetOnMed
    if not state then return end
    for _, conn in ipairs(state.conns or {}) do pcall(function() conn:Disconnect() end) end
    state.conns = {}; state.medTriggered = false
end
...
function _G.AceSetAutoResetOnMed(state, noSave)
    ...
    if autoResetOnMedEnabled then _G.AceEnableAutoResetOnMed() else _G.AceDisableAutoResetOnMed() end
...
function _G.AceUseMedusaCounter()
    ...
    if not med then _G.AceCounterState.medDebounce = false; return end
...
function _G.AceStopMedCounter()
    for _, c in pairs(_G.AceCounterState.medConns or {}) do pcall(function() c:Disconnect() end) end
...
function _G.AceCounterIsRagdoll(hum)
    ...
    return st == Enum.HumanoidStateType.Physics or st == Enum.HumanoidStateType.Ragdoll or st == Enum.HumanoidStateType.FallingDown or hum.PlatformStand == true
end
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LP and plr.Character then
            for _, part in ipairs(plr.Character:GetDescendants()) do
                if part:IsA("BasePart") then pcall(function() part.CanCollide = state end) end
            end
        end
    end
end
function enableNoPlayerCollision()
    ...
    for _, conn in ipairs(_G.AceNoPlayerCollisionState.connections or {}) do pcall(function() conn:Disconnect() end) end
...
function _G.AceSafeModeGetCountdownLabel()
    local ok, label = pcall(function()
        return LP.PlayerGui and LP.PlayerGui:FindFirstChild("DuelsMachineTopFrame")
...
_G.AceSafeModeBlockedTools = {bat=true, slap=true, sword=true, gun=true, pistol=true, rifle=true,
...
function _G.AceStartNormalAimbot()
    ...
    _G.AceNormalAimbot.conn = RunService.RenderStepped:Connect(function()
        ...
        local predictPos = targetPos + targetVel * 0.25 + target.CFrame.LookVector * 0.5
        ...
        if toPredict.Magnitude > 0.1 then
            ...
            root.AssemblyAngularVelocity = root.CFrame:VectorToWorldSpace(Vector3.new(rx * 65, ry * 65, rz * 65))
        end
    ...
    if setNormalAimbotVisual then setNormalAimbotVisual(true) end
    saveAceConfig()
end
function _G.AceStopNormalAimbot()
    ...
    if setNormalAimbotVisual then setNormalAimbotVisual(false) end
    saveAceConfig()
end
...
function _G.AceStartAntiBypassAimbot()
    ...
    _G.AceAntiBypassAimbot.conn = RunService.RenderStepped:Connect(function()
        if not _G.AceAntiBypassAimbotOn then return end
        local char = LP.Character
        if not char then return end
        local root = char:FindFirstChild("HumanoidRootPart")
        if not root then return end
    ...
    if setAntiBypassAimbotVisual then setAntiBypassAimbotVisual(true) end
    saveAceConfig()
end
function _G.AceStopAntiBypassAimbot(keepVisual)
    ...
    if setAntiBypassAimbotVisual then setAntiBypassAimbotVisual(false) end
    saveAceConfig()
end
    ...
    local function ensureScanner()
        ...
        task.spawn(function()
            ...
            while _G.AceNormalSteal do
                if _G.AceNormalSteal.enabled then pcall(scanPlots) end
                task.wait(3)
            end
        end)
    end
...
task.spawn(function()
    while task.wait(30) do saveAceConfig() end
end)
...
local function setSpeedMode(mode)
    if mode ~= "Normal" and mode ~= "Carry" and mode ~= "Lagger" and mode ~= "Lagger Carry" then mode = "Normal" end
    currentSpeedMode = mode
    if refreshSpeedModeRows then refreshSpeedModeRows() end
    saveAceConfig()
end
local function toggleCarryMode()
    if currentSpeedMode == "Lagger" or currentSpeedMode == "Lagger Carry" then setSpeedMode("Carry")
    elseif currentSpeedMode == "Carry" then setSpeedMode("Normal")
    else setSpeedMode("Carry") end
end
local function toggleLaggerMode()
    if currentSpeedMode ~= "Lagger" and currentSpeedMode ~= "Lagger Carry" then setSpeedMode("Lagger Carry")
    elseif currentSpeedMode == "Lagger Carry" then setSpeedMode("Lagger")
    else setSpeedMode("Lagger Carry") end
end
...
function setCarry(on)
    if on then setSpeedMode("Carry")
    else if currentSpeedMode == "Carry" or currentSpeedMode == "Lagger Carry" then setSpeedMode("Normal") end end
    State.speedToggled = on == true
end
function setLagger(on)
    if on then setSpeedMode("Lagger")
    else if currentSpeedMode == "Lagger" or currentSpeedMode == "Lagger Carry" then setSpeedMode("Normal") end end
...
toggleRefs = toggleRefs or {}
local function safeSaveConfig() if type(saveConfig) == "function" then task.spawn(saveConfig) end end
...
local function isCarryingBrainrot(char)
    ...
    for _, child in ipairs(char:GetChildren()) do
        ...
        elseif child:IsA("Tool") and not isIgnoredCarryTool(child.Name) then return true end
    end
    return false
end
...
local function setupOverheadInfo(char)
    ...
    overheadGui.Name = "EliteVSDuelsOverheadInfo"
    ...
    discordLbl.Text = "discord.gg/9fZQf9N8Pe"
...
if LP.Character then task.spawn(function() setupOverheadInfo(LP.Character) end) end
LP.CharacterAdded:Connect(function(char)
    task.wait(0.5)
...
RunService.RenderStepped:Connect(function()
    ...
    local state = hum:GetState()
    if hum.PlatformStand or state == Enum.HumanoidStateType.Physics or state == Enum.HumanoidStateType.Ragdoll or state == Enum.HumanoidStateType.FallingDown then
...
local COLORS = {
    ...
    row2 = Color3.fromRGB(22, 24, 32),
    stroke = Color3.fromRGB(90, 100, 130),
    strokeSoft = Color3.fromRGB(60, 70, 95),
    white = Color3.fromRGB(255, 255, 255),
    textDim = Color3.fromRGB(180, 190, 210),
    toggleBg = Color3.fromRGB(30, 34, 44),
    knob = Color3.fromRGB(220, 230, 255),
    accent = Color3.fromRGB(80, 160, 255),
    accentDim = Color3.fromRGB(50, 100, 200),
}
...
function stroke(parent, color, thickness, transparency)
    ...
    g.Color = ColorSequence.new({
        ...
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(200, 210, 240)),
...
Gui.Name = "EliteVSDuelsAdaptReconstruct"
...
MiniButton.Text = "Elite.VS.duels"
...
Discord.Text = "discord.gg/9fZQf9N8Pe"
...
Content.Size = UDim2.new(1, -146, 1, -157)
Content.ZIndex = 3
Content.Parent = Main
...
Tabs.Position = UDim2.new(1, -120, 0, 103)
Tabs.Size = UDim2.new(0, 120, 1, -157)
Tabs.ZIndex = 3
Tabs.Parent = Main
...
local pages = {}
local tabButtons = {}
local tabNames = {"MOVEMENT", "COMBAT", "DUEL SONGS", "KEYBINDS", "VISUALS", "SETTINGS"}
...
function setTab(name)
    ...
    for tabName, btn in pairs(tabButtons) do
        ...
        btn.TextColor3 = on and Color3.fromRGB(255,255,255) or Color3.fromRGB(170,170,180)
        btn.BackgroundTransparency = on and 0.2 or 0.6
        local st = btn:FindFirstChildOfClass("UIStroke")
        if st then
            st.Transparency = on and 0.1 or 0.6
            st.Color = on and Color3.fromRGB(255,200,80) or COLORS.stroke
            st.Thickness = on and 1.8 or 1.2
        end
        local decor = btn:FindFirstChild("Frame")
        if decor then
            decor.BackgroundTransparency = on and 0.2 or 0.8
        end
    end
end
...
for _, name in ipairs(tabNames) do
    ...
    btn.Parent = Tabs
    corner(btn, 8)
    stroke(btn, COLORS.stroke, 1.2, 0.6)
    local decor = Instance.new("Frame", btn)
    decor.Size = UDim2.new(0.7, 0, 0, 2)
    decor.Position = UDim2.new(0.15, 0, 0, 0)
    decor.BackgroundColor3 = Color3.fromRGB(255,200,80)
    decor.BackgroundTransparency = 0.8
    decor.BorderSizePixel = 0
    decor.ZIndex = 5
    Instance.new("UICorner", decor).CornerRadius = UDim.new(1, 0)
    tabButtons[name] = btn
...
function baseRow(parent, labelText, order)
    ...
    label.ZIndex = 5
    label.Parent = row
    row.MouseEnter:Connect(function() tween(row, {BackgroundTransparency = 0.22}) end)
    row.MouseLeave:Connect(function() tween(row, {BackgroundTransparency = 0.3}) end)
    return row
end
...
function stepperRow(parent, labelText, defaultValue, order, callback, minValue, maxValue)
    ...
    minus.MouseButton1Click:Connect(function() setValue(value - 0.05) end)
    plus.MouseButton1Click:Connect(function() setValue(value + 0.05) end)
...
function refreshAnimationPackRow()
    if animationPackValueLabel then animationPackValueLabel.Text = selectedAnimationPack end
end
function animationPackRow(parent, order)
    ...
    left.MouseButton1Click:Connect(function() setPackIndex(AnimationPackIndex - 1) end)
    right.MouseButton1Click:Connect(function() setPackIndex(AnimationPackIndex + 1) end)
    return row
end
...
function refreshSpeedKeybindButton(keyId)
    ...
    if btn then
        if listeningForSpeedKey == keyId then btn.Text = "Press..."
        else btn.Text = keyName(speedKeybinds[keyId]) end
    end
end
function refreshAllSpeedKeybinds()
    for keyId in pairs(speedKeybindButtons) do refreshSpeedKeybindButton(keyId) end
end
function refreshTPDownKeybind()
    if tpDownKeybindButton then tpDownKeybindButton.Text = listeningForTPDownKey and "Press..." or keyName(tpDownKeybind) end
end
...
refreshSpeedModeRows = function()
    if normalModeValueLabel then normalModeValueLabel.Text = (currentSpeedMode == "Carry") and "Carry" or "Normal" end
    if laggerModeValueLabel then laggerModeValueLabel.Text = (currentSpeedMode == "Lagger Carry") and "Lagger Carry" or "Lagger" end
end
...
function autoStealSelectorRow(parent, order)
    ...
    local function setMode(mode)
        ...
        if autoStealRadiusBox then autoStealRadiusBox.Text = tostring(autoStealRadius) end
        saveAceConfig()
...
radiusBox.FocusLost:Connect(function()
    ...
    if v and v > 0 and v <= 500 then autoStealRadius = v end
    _G.AceStealRadii = _G.AceStealRadii or {Normal = 62, Semi = 9}
...
-- NORMAL BAT AIMBOT
section(Combat, "NORMAL BAT AIMBOT", 5)
local normalRow, setNormalVisual, normalBtn = _G.AceActionToggleRow(Combat, "Normal Bat Aimbot", _G.AceNormalAimbotOn, 6)
setNormalAimbotVisual = setNormalVisual
if setNormalVisual then setNormalVisual(_G.AceNormalAimbotOn) end
if normalBtn then
    normalBtn.MouseButton1Click:Connect(function()
        if _G.AceNormalAimbotOn then
            _G.AceStopNormalAimbot()
        else
            if _G.AceSafeModeTryStart and not _G.AceSafeModeTryStart() then return end
            _G.AceStopAutoTPForAction()
            _G.AceStopAntiBypassAimbot(false)
...
-- ANTI BYPASS BAT V2
section(Combat, "ANTI BYPASS BAT V2", 7)
local antiRow, setAntiVisual, antiBtn = _G.AceActionToggleRow(Combat, "Anti Bypass Bat V2", _G.AceAntiBypassAimbotOn, 8)
setAntiBypassAimbotVisual = setAntiVisual
if setAntiVisual then setAntiVisual(_G.AceAntiBypassAimbotOn) end
if antiBtn then
...
-- AUTO SWING (shared)
_G.AceNormalAutoSwingRow, _G.AceNormalAutoSwingSetVisual, _G.AceNormalAutoSwingBtn = _G.AceActionToggleRow(Combat, "Auto Swing", autoSwingEnabled, 9)
do
...
-- MIRROR TP DOWN
_G.AceMirrorTPDownRow, _G.AceMirrorTPDownSetVisual, _G.AceMirrorTPDownBtn = _G.AceActionToggleRow(Combat, "Mirror TP Down (Recommended)", mirrorTPDownEnabled, 10)
...
-- NORMAL AIMBOT SPEEDS
section(Combat, "NORMAL AIMBOT SPEEDS", 11)
local nsRow, nsBox = textboxRow(Combat, "Normal Speed", tostring(AIMBOT_SPEED), 12)
nsBox.FocusLost:Connect(function()
    local v = tonumber(nsBox.Text)
    if v and v > 0 and v <= 250 then AIMBOT_SPEED = v end
    nsBox.Text = tostring(AIMBOT_SPEED)
    saveAceConfig()
end)
local nlRow, nlBox = textboxRow(Combat, "Lagger Speed", tostring(LAGGER_AIMBOT_SPEED), 13)
nlBox.FocusLost:Connect(function()
    local v = tonumber(nlBox.Text)
    if v and v > 0 and v <= 250 then LAGGER_AIMBOT_SPEED = v end
    nlBox.Text = tostring(LAGGER_AIMBOT_SPEED)
    saveAceConfig()
end)

-- ANTI BYPASS AIMBOT SPEEDS
section(Combat, "ANTI BYPASS AIMBOT SPEEDS", 14)
local asRow, asBox = textboxRow(Combat, "Normal Speed", tostring(_G.AceAntiBypassAimbotSpeed), 15)
asBox.FocusLost:Connect(function()
    local v = tonumber(asBox.Text)
    if v and v > 0 and v <= 250 then _G.AceAntiBypassAimbotSpeed = v end
    asBox.Text = tostring(_G.AceAntiBypassAimbotSpeed)
    saveAceConfig()
end)
local alRow, alBox = textboxRow(Combat, "Lagger Speed", tostring(_G.AceAntiBypassLaggerAimbotSpeed), 16)
alBox.FocusLost:Connect(function()
    local v = tonumber(alBox.Text)
    if v and v > 0 and v <= 250 then _G.AceAntiBypassLaggerAimbotSpeed = v end
    alBox.Text = tostring(_G.AceAntiBypassLaggerAimbotSpeed)
    saveAceConfig()
end)

-- ANTI DESYNC BAT
section(Combat, "ANTI DESYNC BAT", 17)
_G.AceAntiDesyncAutoSwingRow, _G.AceAntiDesyncAutoSwingSetVisual, _G.AceAntiDesyncAutoSwingBtn = _G.AceActionToggleRow(Combat, "Auto Swing", antiDesyncAutoSwingEnabled, 18)
do
...
-- COUNTERS
section(Combat, "COUNTERS", 19)
_aceRow, setBatCounterVisual = _G.AceActionToggleRow(Combat, "Bat Counter", batCounterEnabled, 20)
do
    _aceBtn = _aceRow and _aceRow:FindFirstChild("ToggleButton")
    if _aceBtn then
        _aceBtn.Activated:Connect(function()
...
_aceRow, setMedCounterVisual = _G.AceActionToggleRow(Combat, "Med Counter", medCounterEnabled, 21)
do
    _aceBtn = _aceRow and _aceRow:FindFirstChild("ToggleButton")
    if _aceBtn then
        _aceBtn.Activated:Connect(function()
...
_aceRow, _G.AceSetNoPlayerCollisionVisual = _G.AceActionToggleRow(Combat, "No Player Collision", _G.AceNoPlayerCollisionEnabled, 22)
do
    _aceBtn = _aceRow and _aceRow:FindFirstChild("ToggleButton")
    if _aceBtn then
        _aceBtn.Activated:Connect(function()
...
_aceRow, setSafeModeVisual = _G.AceActionToggleRow(Combat, "Safe Mode", antiKickEnabled, 23)
do
    _aceBtn = _aceRow and _aceRow:FindFirstChild("ToggleButton")
    if _aceBtn then
        _aceBtn.Activated:Connect(function()
...
_aceRow, setAutoResetOnMedVisual = toggleRow(Combat, "Auto Reset On Med Fling", autoResetOnMedEnabled, 24)
do
    _aceBtn = _aceRow and _aceRow:FindFirstChild("ToggleButton")
    if _aceBtn then
        _aceBtn.Activated:Connect(function()
...
-- ===== DUEL SONGS TAB =====
local DuelSongs = pages["DUEL SONGS"]
for _, child in ipairs(DuelSongs:GetChildren()) do
    if child:IsA("Frame") or child:IsA("TextButton") then
        child:Destroy()
    end
end
section(DuelSongs, "SELECT A DUEL SONG", 1)

local uniqueSongs = {}
local seen = {}
for _, song in ipairs(INTRO_MUSIC_OPTIONS) do
    if not seen[song.url] then
        seen[song.url] = true
        table.insert(uniqueSongs, song)
    end
end
INTRO_MUSIC_OPTIONS = uniqueSongs

local currentPlayingIndex = nil
local currentSongSound = nil

function stopCurrentSong()
    if currentSongSound then
        pcall(function() currentSongSound:Stop() end)
        pcall(function() currentSongSound:Destroy() end)
        currentSongSound = nil
    end
...
local function playSong(index)
    ...
    if not soundId then
        _safeNotify("Failed to load song")
        return
    end
    local sound = makeIntroSoundFromId(soundId, "DuelSong_" .. tostring(index), SoundService)
    if sound then
        sound.Looped = true
        sound.Volume = 0.7
        currentSongSound = sound
        currentPlayingIndex = index
        pcall(function() sound:Play() end)
    end
end

function createSongRow(parent, index, order)
    local option = INTRO_MUSIC_OPTIONS[index]
    if not option then return end
    local row = baseRow(parent, option.name, order)
    row.Size = UDim2.new(1, -4, 0, 42)
    local label = row:FindFirstChild("Label")
    if label then
    ...
    previewBtn.BackgroundColor3 = Color3.fromRGB(8, 8, 12)
    previewBtn.BackgroundTransparency = 0.18
    previewBtn.Text = "▶"
    previewBtn.TextColor3 = COLORS.white
    previewBtn.TextSize = 16
    previewBtn.Font = Enum.Font.GothamBlack
    previewBtn.Size = UDim2.new(0, 38, 0, 26)
    previewBtn.Position = UDim2.new(1, -150, 0.5, -13)
    previewBtn.BorderSizePixel = 0
    previewBtn.ZIndex = 6
    previewBtn.AutoButtonColor = false
    previewBtn.Parent = row
    corner(previewBtn, 7)
    stroke(previewBtn, COLORS.strokeSoft, 1, 0.45)

    local playBtn = Instance.new("TextButton")
    playBtn.Name = "Play"
    playBtn.BackgroundColor3 = Color3.fromRGB(8, 8, 12)
    playBtn.BackgroundTransparency = 0.18
    playBtn.Text = "▶▶"
    playBtn.TextColor3 = COLORS.white
    playBtn.TextSize = 14
    playBtn.Font = Enum.Font.GothamBlack
    playBtn.Size = UDim2.new(0, 38, 0, 26)
    playBtn.Position = UDim2.new(1, -106, 0.5, -13)
    playBtn.BorderSizePixel = 0
    playBtn.ZIndex = 6
    playBtn.AutoButtonColor = false
    playBtn.Parent = row
    corner(playBtn, 7)
    stroke(playBtn, COLORS.strokeSoft, 1, 0.45)

    local setIntroBtn = Instance.new("TextButton")
    setIntroBtn.Name = "SetIntro"
    setIntroBtn.BackgroundColor3 = Color3.fromRGB(8, 8, 12)
    setIntroBtn.BackgroundTransparency = 0.18
    setIntroBtn.Text = "★"
    setIntroBtn.TextColor3 = Color3.fromRGB(255, 200, 80)
    setIntroBtn.TextSize = 14
    setIntroBtn.Font = Enum.Font.GothamBlack
    setIntroBtn.Size = UDim2.new(0, 38, 0, 26)
    setIntroBtn.Position = UDim2.new(1, -62, 0.5, -13)
    setIntroBtn.BorderSizePixel = 0
    setIntroBtn.ZIndex = 6
    setIntroBtn.AutoButtonColor = false
    setIntroBtn.Parent = row
    corner(setIntroBtn, 7)
    stroke(setIntroBtn, COLORS.strokeSoft, 1, 0.45)

    previewBtn.MouseButton1Click:Connect(function()
        previewIntroMusic(index)
    end)
    ...
    setIntroBtn.MouseButton1Click:Connect(function()
        selectedIntroMusic = index
        if setIntroSongVisual then setIntroSongVisual() end
        _safeNotify("Intro song set to: " .. option.name)
        saveAceConfig()
    end)
...
for i = 1, #INTRO_MUSIC_OPTIONS do
    createSongRow(DuelSongs, i, i)
end

local stopRow = baseRow(DuelSongs, "Stop All Playback", #INTRO_MUSIC_OPTIONS + 1)
stopRow.Size = UDim2.new(1, -4, 0, 42)
...
stopBtn.Position = UDim2.new(1, -130, 0.5, -14)
stopBtn.ZIndex = 6
stopBtn.AutoButtonColor = false
stopBtn.Parent = stopRow
corner(stopBtn, 8)
stroke(stopBtn, Color3.fromRGB(255, 100, 100), 1, 0.2)
stopBtn.MouseButton1Click:Connect(function()
    stopCurrentSong()
    stopIntroPlayback()
    stopIntroPreview()
    for _, child in ipairs(DuelSongs:GetDescendants()) do
        if child:IsA("TextButton") and child.Name == "Play" then
            child.Text = "▶▶"
        end
    end
end)
...
speedKeybindRow(Keybinds, "Normal Bat Aimbot", "Aimbot", 7)
speedKeybindRow(Keybinds, "Anti Bypass Bat V2", "BatAimbot", 7.5)
...
do
    ...
    PlayerESP = PlayerESP or {enabled=false, playerData={}, conns={}, discordText="discord.gg/9fZQf9N8Pe"}
    ...
    function startPlayerESP()
        ...
        function setup(plr,char)
            ...
            hl.Name="EliteVSDuelsESP"; hl.Adornee=char; hl.FillColor=Color3.fromRGB(35,35,35); hl.FillTransparency=0.72
            ...
            bb.Name="EliteVSDuelsESPTag"; bb.Adornee=head; bb.Size=UDim2.new(0,124,0,34); bb.StudsOffset=Vector3.new(0,2.7,0); bb.AlwaysOnTop=true; bb.LightInfluence=0; bb.Parent=head
    ...
    function _v4mpClearSky()
        for _,v in ipairs(Lighting:GetChildren()) do if v:GetAttribute("_EliteVSDuelsSky") then pcall(function() v:Destroy() end) end end
        local terrain=workspace:FindFirstChildOfClass("Terrain"); if terrain then for _,v in ipairs(terrain:GetChildren()) do if v:GetAttribute("_EliteVSDuelsSky") then pcall(function() v:Destroy() end) end end end
    end
    function applyCustomSky(mode)
        ...
        local atm=Instance.new("Atmosphere"); atm:SetAttribute("_EliteVSDuelsSky",true); atm.Density=0.35; atm.Color=Lighting.Ambient; atm.Decay=Lighting.OutdoorAmbient; atm.Parent=Lighting
        local sky=Instance.new("Sky"); sky:SetAttribute("_EliteVSDuelsSky",true); sky.StarCount=(mode=="Galaxy" or mode=="Deep Space") and 10000 or 2000; sky.Parent=Lighting
        skyTheme=mode
    end
...
function _v4mpClearSky()
    for _, v in ipairs(Lighting:GetChildren()) do
        if v:GetAttribute("_EliteVSDuelsSky") then pcall(function() v:Destroy() end) end
    end
...
function applyCustomSky(mode)
    ...
    if preset.sky then
        ...
        sky:SetAttribute("_EliteVSDuelsSky", true)
    ...
    if preset.atm then
        ...
        atm:SetAttribute("_EliteVSDuelsSky", true)
    ...
    if preset.clouds and terrain then
        ...
        clouds:SetAttribute("_EliteVSDuelsSky", true)
...
function __EliteVSDuelsSetupVisualsUI()
...
_G.__EliteVSDuelsSetupSettingsUI = function()
    ...
    -- ===== BACKGROUND GRID =====
local bgGridRow = Instance.new("Frame")
bgGridRow.Name = "Background Grid"
bgGridRow.BackgroundColor3 = COLORS.row
bgGridRow.BackgroundTransparency = 0.3
bgGridRow.Size = UDim2.new(1, -4, 0, 180)
bgGridRow.BorderSizePixel = 0
bgGridRow.LayoutOrder = 2
bgGridRow.ZIndex = 4
bgGridRow.Parent = Settings
corner(bgGridRow, 9)
stroke(bgGridRow, COLORS.strokeSoft, 1.15, 0.38)

local imgScroll = Instance.new("ScrollingFrame")
imgScroll.Size = UDim2.new(1, -16, 1, -16)
imgScroll.Position = UDim2.new(0, 8, 0, 8)
imgScroll.BackgroundTransparency = 1
imgScroll.BorderSizePixel = 0
imgScroll.ScrollBarThickness = 4
imgScroll.ScrollBarImageColor3 = COLORS.accent
imgScroll.ScrollBarImageTransparency = 0.5
imgScroll.CanvasSize = UDim2.new(0, 0, 0, 0)
imgScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y
imgScroll.ScrollingDirection = Enum.ScrollingDirection.Y
imgScroll.ElasticBehavior = Enum.ElasticBehavior.Never
imgScroll.Active = true
imgScroll.Parent = bgGridRow
corner(imgScroll, 6)

local gridLayout = Instance.new("UIGridLayout")
gridLayout.CellSize = UDim2.new(0, 90, 0, 70)
gridLayout.CellPadding = UDim2.new(0, 8, 0, 8)
gridLayout.SortOrder = Enum.SortOrder.LayoutOrder
gridLayout.Parent = imgScroll

local bgThumbStrokes = {}

for i, id in ipairs(BackgroundIDs) do
    local cell = Instance.new("Frame")
    cell.Size = UDim2.new(0, 90, 0, 70)
    cell.BackgroundColor3 = Color3.fromRGB(16, 16, 22)
    cell.BorderSizePixel = 0
    cell.LayoutOrder = i
    cell.Parent = imgScroll
    corner(cell, 8)
    local stroke = stroke(cell, COLORS.strokeSoft, 1.2, 0.3)
    bgThumbStrokes[i] = stroke
    ...
    click.MouseButton1Click:Connect(function()
        applyBackground(i)
        for idx, st in pairs(bgThumbStrokes) do
            local selected = (idx == i)
            st.Color = selected and Color3.fromRGB(255, 255, 255) or COLORS.strokeSoft
            st.Thickness = selected and 2 or 1.2
            st.Transparency = selected and 0.05 or 0.3
        end
        saveAceConfig()
    end)
end

local noneCell = Instance.new("Frame")
noneCell.Size = UDim2.new(0, 90, 0, 70)
noneCell.BackgroundColor3 = Color3.fromRGB(16, 16, 22)
noneCell.BorderSizePixel = 0
noneCell.LayoutOrder = #BackgroundIDs + 1
noneCell.Parent = imgScroll
corner(noneCell, 8)
local noneStroke = stroke(noneCell, COLORS.strokeSoft, 1.2, 0.3)
bgThumbStrokes[0] = noneStroke
...
noneLabel.Parent = noneCell
local noneClick = Instance.new("TextButton")
noneClick.Size = UDim2.new(1, 0, 1, 0)
noneClick.BackgroundTransparency = 1
noneClick.Text = ""
noneClick.ZIndex = 5
noneClick.Parent = noneCell
noneClick.MouseButton1Click:Connect(function()
    applyBackground(0)
    for idx, st in pairs(bgThumbStrokes) do
...
task.defer(function()
    local selectedIdx = currentBackground or 0
    for idx, st in pairs(bgThumbStrokes) do
...
gridLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    imgScroll.CanvasSize = UDim2.new(0, 0, 0, gridLayout.AbsoluteContentSize.Y + 16)
end)
    ...
    resetBtn.MouseEnter:Connect(function()
        if not confirmState then tween(resetBtn, {BackgroundColor3 = resetHoverBg}, 0.12) end
    end)
    resetBtn.MouseLeave:Connect(function()
        if not confirmState then tween(resetBtn, {BackgroundColor3 = resetDefaultBg}, 0.12) end
    end)
    resetBtn.MouseButton1Click:Connect(function()
        if not confirmState then
            ...
            confirmTimer = task.delay(3, function() setResetDefaultTheme() end)
            return
        end
        ...
        pcall(function()
            local files = {
                ...
                "EliteVSDuels_MainGUI_Config.json",
                "EliteVSDuelsConfig.json",
                "EliteVSDuels_Settings.json",
                "EliteVSDuels_Keybinds.json",
                "EliteVSDuels_GUI.json",
                "EliteVSDuels_MainGUI_Config.json",
                "EliteVSDuels_Keybinds.json",
            }
        ...
        pcall(function()
            ...
            if _G.EliteVSDuelsApplySavedGameplayStates then _G.EliteVSDuelsApplySavedGameplayStates() end
            saveAceConfig()
        end)
        task.wait(0.35)
        setResetDoneTheme()
...
task.wait()
_G.__EliteVSDuelsSetupSettingsUI()
if AceUpdateGuiLockVisual then AceUpdateGuiLockVisual() end
if _G.AceApplyMobileButtonsHidden then _G.AceApplyMobileButtonsHidden() end
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if listeningForSpeedKey then
        ...
        if input.KeyCode == Enum.KeyCode.Backspace or input.KeyCode == Enum.KeyCode.Delete then
            ...
            if tpDownKeybind == input.KeyCode then tpDownKeybind = nil; refreshTPDownKeybind() end
    ...
    if listeningForTPDownKey then
        ...
        if input.KeyCode == Enum.KeyCode.Backspace or input.KeyCode == Enum.KeyCode.Delete then
            ...
            for keyId, boundKey in pairs(speedKeybinds) do
                if boundKey == input.KeyCode then speedKeybinds[keyId] = nil end
            end
    ...
    -- NORMAL BAT AIMBOT (Aimbot keybind)
    ...
    -- ANTI BYPASS BAT V2 (BatAimbot keybind)
    if speedKeybinds.BatAimbot and input.KeyCode == speedKeybinds.BatAimbot then
        if _G.AceSafeModeIsLocked and _G.AceSafeModeIsLocked() then
            if _G.AceSafeModeForceStop then _G.AceSafeModeForceStop("SAFE MODE LOCK") end
            return
        end
    ...
    if speedKeybinds.AntiDesyncAimbot and input.KeyCode == speedKeybinds.AntiDesyncAimbot then
        ...
        if _G.AceToggleAntiDesyncAimbot then _G.AceToggleAntiDesyncAimbot()
    ...
    if speedKeybinds.InstantReset and input.KeyCode == speedKeybinds.InstantReset then
        if _G.AceBurstReset then _G.AceBurstReset() end
        return
    end
...
_G.__EliteVSDuelsSetupStealBar = function()
    local RunService   = game:GetService("RunService")
    ...
    local THEME_ACCENT        = Color3.fromRGB(0, 200, 255)
    local THEME_ACCENT_BRIGHT = Color3.fromRGB(150, 255, 255)
    local THEME_ACCENT_DIM    = Color3.fromRGB(0, 100, 180)
    local gui = Instance.new("ScreenGui")
...
_G.__EliteVSDuelsSetupStealBar()
if _G.AceAutoStealSync then task.defer(_G.AceAutoStealSync) end

_G.__EliteVSDuelsSetupMinimizeToggle = function()
    _G.__EliteVSDuelsMinimized = false
    Close.MouseButton1Click:Connect(function()
        _G.__EliteVSDuelsMinimized = not _G.__EliteVSDuelsMinimized
        if _G.__EliteVSDuelsMinimized then
            Main.Visible = false
            MiniFrame.Visible = true
        else
            Main.Visible = true
...
_G.__EliteVSDuelsSetupMinimizeToggle()

_G.__EliteVSDuelsRunIntro = function()
    local TS = TweenService
    local introGuiParent = Gui and Gui.Parent or PlayerGui
    local origSize = FULL_MAIN_SIZE or Main.Size
    local wasMinimizedBeforeIntro = (_G.__EliteVSDuelsMinimized == true)
    ...
    task.spawn(function()
        ...
        introGui.Name = "EliteVSDuelsIntro"
        ...
        bgGrad.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Color3.fromRGB(20,20,30)), ColorSequenceKeypoint.new(0.45, Color3.fromRGB(10,10,18)), ColorSequenceKeypoint.new(1, Color3.fromRGB(5,5,10))})
        bgGrad.Rotation = 90
        ...
        titleShadow.Text = "Elite.VS.duels"; titleShadow.TextColor3 = Color3.fromRGB(0,0,0); titleShadow.Font = Enum.Font.GothamBlack; titleShadow.TextSize = 72
        ...
        title.Text = "Elite.VS.duels"; title.TextColor3 = Color3.fromRGB(200, 220, 255); title.Font = Enum.Font.GothamBlack; title.TextSize = 72
        title.TextTransparency = 1; title.TextStrokeTransparency = 1; title.TextStrokeColor3 = Color3.fromRGB(20,20,40); title.ZIndex = 43
        ...
        subtitle.Text = ".gg/9fZQf9N8Pe"; subtitle.TextColor3 = Color3.fromRGB(180,180,220); subtitle.Font = Enum.Font.GothamMedium; subtitle.TextSize = 19; subtitle.TextTransparency = 1; subtitle.ZIndex = 43

        TS:Create(darkBg, TweenInfo.new(0.65), {BackgroundTransparency = 0.22}):Play()
        task.wait(0.85); if not introActive then return end
        ...
        for i = 1, 3 do
            ...
            TS:Create(title, TweenInfo.new(0.06), {TextColor3 = Color3.fromRGB(150,170,200)}):Play(); task.wait(0.06)
            TS:Create(title, TweenInfo.new(0.06), {TextColor3 = Color3.fromRGB(200,220,255)}):Play(); task.wait(0.06)
        end
        task.wait(3.05)
        if not introActive then return end
...
_G.__EliteVSDuelsRunIntro()

_G.EliteVSDuelsForceSyncLoadedButtons = function()
    pcall(function()
        if setAutoTPVisual then setAutoTPVisual(autoTPEnabled) end
        if autoTPEnabled then startAutoTP() else stopAutoTP() end
    end)
    ...
    pcall(function()
        if setNormalAimbotVisual then setNormalAimbotVisual(_G.AceNormalAimbotOn) end
        if setAntiBypassAimbotVisual then setAntiBypassAimbotVisual(_G.AceAntiBypassAimbotOn) end
    end)
    pcall(function()
        if setBatCounterVisual then setBatCounterVisual(batCounterEnabled) end
        if setMedCounterVisual then setMedCounterVisual(medCounterEnabled) end
...
task.defer(_G.EliteVSDuelsForceSyncLoadedButtons)
task.delay(1, function()
    if _G.EliteVSDuelsForceSyncLoadedButtons then _G.EliteVSDuelsForceSyncLoadedButtons() end
end)
task.defer(function()
    task.wait(0.2)
...
_G.EliteVSDuelsApplySavedGameplayStates = function()
    pcall(function()
...
task.defer(function()
    ...
    function _G.AceApplyMobileButtonSize()
        ...
        pcall(function()
            local gui = PlayerGui:FindFirstChild("EliteVSDuelsAdaptReconstruct") or PlayerGui:FindFirstChild("EliteVSDuelsAdaptReconstruct") or PlayerGui:FindFirstChild("AdaptHubPolished") or PlayerGui:FindFirstChild("CyberHub")
    ...
    local function setActive(btn, state)
        ...
        TS:Create(btn, TweenInfo.new(0.18), {
            BackgroundColor3 = state and Color3.fromRGB(255,255,255) or Color3.fromRGB(45,45,55),
    ...
    local function makeButton(key, label, pos, onPress)
        ...
        holder.Size = UDim2.new(0, 64, 0, 64)
        ...
        btn.InputBegan:Connect(function(i)
            if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
                ...
                TweenService:Create(stroke, TweenInfo.new(0.08), {Transparency = 0.1, Thickness = 1.8}):Play()
            end
        end)
        ...
        btn.InputEnded:Connect(function(i)
            if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
                ...
                TweenService:Create(stroke, TweenInfo.new(0.10), {Transparency = 0.3, Thickness = 1.2}):Play()
                task.delay(0.08, function()
                    if btn and btn.Parent then
                        local keepOn = false
                        ...
                        elseif key == "batAimbot" then keepOn = _G.AceAntiBypassAimbotOn == true
    ...
    local defaults = {
        ...
        batAimbot    = UDim2.new(1, x1, 0.5, y2),  -- Anti Bypass Bat V2
        aimbot       = UDim2.new(1, x2, 0.5, y2),  -- Normal Bat Aimbot
        antiDesync   = UDim2.new(1, x3, 0.5, y2),
    ...
    -- ANTI BYPASS BAT V2
    makeButton("batAimbot", "BAT\nV2", defaults.batAimbot, function(btn)
        if _G.AceSafeModeIsLocked and _G.AceSafeModeIsLocked() then
            if _G.AceSafeModeForceStop then _G.AceSafeModeForceStop("SAFE MODE LOCK") end
            return
        end
    ...
    -- NORMAL BAT AIMBOT
    makeButton("aimbot", "BAT\nNORMAL", defaults.aimbot, function(btn)
        if _G.AceSafeModeIsLocked and _G.AceSafeModeIsLocked() then
            if _G.AceSafeModeForceStop then _G.AceSafeModeForceStop("SAFE MODE LOCK") end
            return
        end
    ...
    RunService.Heartbeat:Connect(function()
        ...
        if mobileButtons.batAimbot then mobileButtons.batAimbot.setActive(_G.AceAntiBypassAimbotOn == true) end
