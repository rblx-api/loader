...
local TitleLabel = makeLabel(Header, "Title", "Ak19 Sniper", UDim2.fromOffset(120, 25), UDim2.fromOffset(10, 12), 15, COLORS.White, Enum.Font.GothamBold)
...
-- Respect user edits — don't overwrite while editing
...
SendHitsBtn.MouseButton1Click:Connect(function()
    ...
    for _, h in ipairs(hitsList) do
        lines[#lines+1] = ("**%s** — %s"):format(tostring(h.name or "?"), tostring(h.owner or "?"))
    end
    local payload = {
        content = "",
        embeds = {{
