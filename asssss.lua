...
pcall(function()
    for _, name in ipairs({"StickSemiTP_Main", "StickAPSpamGui", "AllowDisallow", "Stick_HelperV1", "Stick_SpeedBooster"}) do
...
local SpeedFrameRef
local CatBarRef
local switchCategoryRef
local SpeedPanelToggleBtnRef
...
do
    ...
    -- Filas de SELECT SLOT, ACTIVATE, EXECUTE
    ...
    local function createCategoryBtn(text, order)
        local btn = Instance.new("TextButton")
        btn.Size = UDim2.new(0, (panelWidth - 24) / 3, 1, 0)
        btn.BackgroundColor3 = Color3.fromRGB(40, 15, 18)
        btn.BorderSizePixel = 0
        btn.Text = text
    ...
    local pagePub = createCatPage("Pub Method")

    do
        local emptyFlash = Instance.new("TextLabel")
        emptyFlash.Size = UDim2.new(1, 0, 0, 40)
        emptyFlash.BackgroundTransparency = 1
        emptyFlash.Text = "Proximamente..."
        emptyFlash.TextColor3 = COLOR_TEXT_DARK
        emptyFlash.Font = MAIN_FONT
        emptyFlash.TextSize = 11
        emptyFlash.ZIndex = 5
        emptyFlash.Parent = pageFlash

        local emptyPub = Instance.new("TextLabel")
        emptyPub.Size = UDim2.new(1, 0, 0, 40)
        emptyPub.BackgroundTransparency = 1
        emptyPub.Text = "Proximamente..."
        emptyPub.TextColor3 = COLOR_TEXT_DARK
        emptyPub.Font = MAIN_FONT
        emptyPub.TextSize = 11
        emptyPub.ZIndex = 5
        emptyPub.Parent = pagePub
    end
    ...
    switchCategoryRef = switchCategory

    for i, name in ipairs({"Semi TP", "Flash TP Block", "Pub Method"}) do
    ...
    -- PageSemi: toggles originales
    local RowAutoActivate = createRowFrame(pageSemi, 28, 10)
    ...
    local Row2 = createRowFrame(pageSemi, 28, 11)
    ...
    local RowAutoWalk = createRowFrame(pageSemi, 28, 13)
    ...
    local RowAutoTPAllow = createRowFrame(pageSemi, 28, 17)
    ...
    local RowAPOnSteal = createRowFrame(pageSemi, 28, 14)
    ...
    local RowKickAfterSteal = createRowFrame(pageSemi, 28, 18)
    ...
    local RowGear = createRowFrame(pageSemi, 28, 20)
    ...
    local Row4 = createRowFrame(pageSemi, 28, 21)
    ...
    local RowAPDefense = createRowFrame(pageSemi, 28, 15)
    ...
    local RowLaserDefense = createRowFrame(pageSemi, 28, 16)
    ...
    local RowAimbot = createRowFrame(pageSemi, 28, 12)
    ...
    -- ===== PageFlash: show/close semi tp =====
    local RowFlashShow = createRowFrame(pageFlash, 30, 1)
    ...
    FlashShowBtn.MouseButton1Click:Connect(function()
        ...
        -- Reactiva el icono de flecha
        if FlashShowBtn then FlashShowBtn.Text = "Show Semi TP" end
    end)

    local RowFlashClose = createRowFrame(pageFlash, 30, 2)
    ...
    createHelperBtn("Kick", 1, Color3.fromRGB(180, 20, 30), COLOR_TEXT_LIGHT, function()
        pcall(function() player:Kick("Kicked by HELPER V1") end)
    end)

    createHelperBtn("Rejoin", 2, Color3.fromRGB(40, 15, 18), COLOR_TEXT_LIGHT, function()
        task.spawn(function()
    ...
    createHelperBtn("Reset", 3, COLOR_ACCENT, COLOR_BG_DARK, function()
