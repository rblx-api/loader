...
-- ============================================================
--  REEMPLAZAR LA FUNCIÓN PRINCIPAL buildGui CON ESTILO SURE HUB
...
function M.buildGui()
    ...
    -- ============================================================
    --  ESTILO SURE HUB (colores)
    -- ============================================================
    local SURE_ACCENT = Color3.fromRGB(0, 150, 255)
    local SURE_BG = Color3.fromRGB(0, 0, 0)
    local SURE_ROW = Color3.fromRGB(10, 10, 10)
    local SURE_ROW_HOVER = Color3.fromRGB(28, 28, 28)
    local SURE_TEXT = Color3.fromRGB(255, 255, 255)
    local SURE_TEXT_DIM = Color3.fromRGB(180, 180, 190)
    local SURE_INPUT = Color3.fromRGB(15, 15, 15)
    local SURE_OFF = Color3.fromRGB(25, 25, 30)
    local SURE_STROKE = Color3.fromRGB(50, 50, 50)
    local SURE_BORDER = Color3.fromRGB(0, 150, 255)
    local SURE_CORNER = 40
    ...
    Frame.BackgroundColor3 = SURE_BG
    ...
    frameCorner.CornerRadius = UDim.new(0, SURE_CORNER)
    ...
    titleLabel.Size = UDim2.new(0, 280, 0, 26)
    titleLabel.BackgroundTransparency = 1
    titleLabel.Text = "VYNX DUELS"
    titleLabel.TextColor3 = SURE_TEXT
    titleLabel.TextSize = 20
    titleLabel.Font = Enum.Font.GothamBlack
    titleLabel.TextXAlignment = Enum.TextXAlignment.Left
    titleLabel.TextStrokeColor3 = SURE_BG
    titleLabel.TextStrokeTransparency = 0.5
    titleLabel.Parent = Header
    M.titleLabel = titleLabel

    local madeByTag = Instance.new("TextLabel")
    madeByTag.ZIndex = 3
    madeByTag.Name = "MadeByTag"
    madeByTag.Position = UDim2.new(0, 16, 0, 32)
    madeByTag.Size = UDim2.new(0, 200, 0, 14)
    madeByTag.BackgroundTransparency = 1
    madeByTag.Text = "discord.gg/vynxduels"
    madeByTag.TextColor3 = SURE_TEXT_DIM
    madeByTag.TextSize = 11
    madeByTag.Font = Enum.Font.GothamBold
    madeByTag.TextXAlignment = Enum.TextXAlignment.Left
    madeByTag.TextStrokeColor3 = SURE_BG
    madeByTag.TextStrokeTransparency = 0.5
    madeByTag.Parent = Header
    ...
    MinBtn.BackgroundColor3 = SURE_OFF
    MinBtn.BorderSizePixel = 0
    MinBtn.Text = "-"
    MinBtn.TextColor3 = SURE_TEXT
    MinBtn.TextSize = 22
    MinBtn.Font = Enum.Font.GothamBold
    ...
    lockButton.BackgroundColor3 = SURE_ROW
    lockButton.BorderSizePixel = 0
    lockButton.Text = M.uiLocked and "LOCKED" or "UNLOCK"
    lockButton.TextColor3 = M.uiLocked and SURE_BORDER or SURE_TEXT_DIM
    ...
    -- Crear pestañas con estilo Sure Hub
    local function makeTab(parent, name, text, pos, active)
        local b = Instance.new("TextButton")
        b.Name = name or "Tab"
        b.Size = UDim2.new(1, -8, 0, 36)
        b.BackgroundColor3 = active and SURE_TEXT or SURE_OFF
        b.BackgroundTransparency = 0
        b.BorderSizePixel = 0
        b.Text = tostring(text or "")
        b.TextColor3 = active and SURE_BG or SURE_TEXT
        b.TextSize = 12
        b.Font = Enum.Font.GothamBold
        b.TextXAlignment = Enum.TextXAlignment.Center
        b.AutoButtonColor = false
        ...
        local stroke = Instance.new("UIStroke")
        stroke.Name = "TabStroke"
        stroke.Color = active and SURE_BORDER or SURE_STROKE
        stroke.Thickness = 1
        stroke.Transparency = active and 0.2 or 0.4
        stroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
        stroke.Parent = b
        b:SetAttribute("IsActiveTab", active and true or false)
        b.MouseEnter:Connect(function()
            if not b:GetAttribute("IsActiveTab") then
                TweenService:Create(b, UI_TWEEN_FAST, {BackgroundTransparency = 0.2, TextColor3 = SURE_TEXT}):Play()
            end
        end)
        b.MouseLeave:Connect(function()
            if not b:GetAttribute("IsActiveTab") then
                TweenService:Create(b, UI_TWEEN_FAST, {BackgroundTransparency = 0, TextColor3 = SURE_TEXT_DIM}):Play()
            end
        end)
        return b
    end

    local TSpeed = makeTab(TabBar, "Tab_SPEED", "Speed", nil, true)
    TSpeed.LayoutOrder = 1
    local TMech = makeTab(TabBar, "Tab_MECH", "Combat", nil, false)
    TMech.LayoutOrder = 2
    local TVis = makeTab(TabBar, "Tab_VIS", "Visuals", nil, false)
    TVis.LayoutOrder = 3

    -- Sidebar derecha (Utility + Keybinds)
    ...
    SideBarR.BackgroundColor3 = SURE_ROW
    ...
    local TUtil = makeTab(TabBarR, "Tab_UTIL", "Utility", nil, false)
    TUtil.LayoutOrder = 1
    local TKB = makeTab(TabBarR, "Tab_KB", "Keybinds", nil, false)
    TKB.LayoutOrder = 2
    ...
    -- Crear páginas con estilo Sure Hub
    local function makePage(parent, name, order, vis)
        ...
        p.BorderSizePixel = 0
        p.ScrollBarThickness = 8
        p.ScrollBarImageColor3 = SURE_BORDER
        p.ScrollBarImageTransparency = 0.15
        p.ScrollingEnabled = true
        p.ScrollingDirection = Enum.ScrollingDirection.Y
        p.ElasticBehavior = Enum.ElasticBehavior.Always
        ...
        local function upd()
            ...
            local padTop = (pad and pad.PaddingTop.Offset or 0)
            local h = lay.AbsoluteContentSize.Y + padBottom + padTop + 24
            p.CanvasSize = UDim2.new(0, 0, 0, math.max(h, 1))
        end
        l:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(upd)
        task.defer(upd)
        task.delay(0.15, upd)
        task.delay(0.5, upd)
        return p
    ...
    local PM = makePage(PagedContent, "Page_SPEED", 1, true)
    local PMech = makePage(PagedContent, "Page_MECHANICS", 2, false)
    local PVis = makePage(PagedContent, "Page_VISUALS", 3, false)
    local PUtil = makePage(PagedContent, "Page_UTILITY", 4, false)
    local PKB = makePage(PagedContent, "Page_KEYBINDS", 5, false)

    M.Pages = {SPEED = PM, MECHANICS = PMech, VISUALS = PVis, UTILITY = PUtil, KEYBINDS = PKB}
    M.Tabs = {SPEED = TSpeed, MECHANICS = TMech, VISUALS = TVis, UTILITY = TUtil, KEYBINDS = TKB}

    -- ============================================================
    --  CAMBIO DE PESTAÑA (estilo Sure Hub)
    -- ============================================================
    local curTab = "SPEED"
    M.selectTab = function(name)
        if curTab == name then return end
        curTab = name
        for k, p in pairs(M.Pages) do
            p.Visible = (k == name)
        end
        for k, b in pairs(M.Tabs) do
            local act = (k == name)
            b:SetAttribute("IsActiveTab", act)
            b.BackgroundColor3 = act and SURE_TEXT or SURE_OFF
            b.BackgroundTransparency = 0
            b.TextColor3 = act and SURE_BG or SURE_TEXT
            local st = b:FindFirstChild("TabStroke")
            if st then
                st.Color = act and SURE_BORDER or SURE_STROKE
                st.Transparency = act and 0.2 or 0.4
            end
        end
    end

    TSpeed.MouseButton1Click:Connect(function() M.selectTab("SPEED") end)
    TMech.MouseButton1Click:Connect(function() M.selectTab("MECHANICS") end)
    TVis.MouseButton1Click:Connect(function() M.selectTab("VISUALS") end)
    TUtil.MouseButton1Click:Connect(function() M.selectTab("UTILITY") end)
    TKB.MouseButton1Click:Connect(function() M.selectTab("KEYBINDS") end)

    -- Activar primera pestaña
    TSpeed.BackgroundColor3 = SURE_TEXT
    TSpeed.TextColor3 = SURE_BG
    local st = TSpeed:FindFirstChild("TabStroke")
    if st then
        st.Color = SURE_BORDER
        st.Transparency = 0.2
    end
    PM.Visible = true
    ...
    MinPill.BackgroundColor3 = SURE_OFF
    MinPill.BackgroundTransparency = 0
    MinPill.BorderSizePixel = 0
    MinPill.Parent = gui
    Instance.new("UICorner", MinPill).CornerRadius = UDim.new(1, 0)
    local pst = Instance.new("UIStroke", MinPill)
    pst.Color = SURE_BORDER
    pst.Thickness = 1.5
    pst.Transparency = 0.3

    local minLabel = Instance.new("TextLabel", MinPill)
    minLabel.Size = UDim2.new(1, 0, 1, 0)
    minLabel.BackgroundTransparency = 1
    minLabel.Text = "VYNX"
    minLabel.TextColor3 = SURE_TEXT
    minLabel.TextSize = 13
    minLabel.Font = Enum.Font.GothamBlack

    local minButton = Instance.new("TextButton", MinPill)
    minButton.ZIndex = 41
    ...
    --  Usando las funciones existentes de Vynx (uiSectionHeader, uiToggleRow, etc.)
    --  Pero con los colores de Sure Hub aplicados
    -- ============================================================

    -- El resto del contenido se construye exactamente igual que en el original
    -- usando las funciones M.uiSectionHeader, M.uiToggleRow, M.uiNumberRow, etc.
    -- que ya existen en el código original.

    -- ============================================================
    --  BARRA DE PROGRESO (Steal)
    -- ============================================================
    -- Esta parte se mantiene igual que en el original
    -- ============================================================

    -- ============================================================
    --  FINALIZAR
    -- ============================================================
    M.applyStealBarTheme(SURE_ACCENT)
    M.updateHeadTheme()
    M.applyFOV()
    M.CandyApplyCustomSky(M.currentSkyTheme)

    -- Si hay configuraciones guardadas, actualizar los toggles
    ...
    if M.setUltraModeVisual then M.setUltraModeVisual(M.ultraModeEnabled) end
    ...
    if M._modeLabel then
        M._modeLabel.Text = M.laggerModeEnabled and "Lagger" or (M.carrySpeedActive and "Carry" or "Normal")
    end

    -- Iniciar actualizaciones de velocidad
    M.startHeadSpeedUpdates()

    print("[VYNX] Menú reconstruido con estilo Sure Hub")
end

-- ============================================================
--  FINAL DEL ARCHIVO (resto del código original)
--  Las funciones M.startBatAimbot, M.stopBatAimbot, etc. ya existen arriba
-- ============================================================
