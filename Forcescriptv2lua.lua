...
function startAimbotAdapt()
    ...
    _aimbotConn = RunService.RenderStepped:Connect(function()
        ...
        local desiredVel = Vector3.new(
            flatDir.X * BAT_AIMBOT_SPEED,
            yVel,
            flatDir.Z * BAT_AIMBOT_SPEED
        )
...
function buildGui()
    ...
    local function animPill(pill, dot, on)
        TS:Create(pill, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {
            BackgroundColor3 = on and SILVER or OFF
        }):Play()
        ...
        if stroke then
            TS:Create(stroke, TweenInfo.new(0.2), {
                Color = on and SILVER or ROW_BORDER,
    ...
    do
        ...
        dropModeBtnRef = mkSelector(row, dropMode == 1 and "Fling" or "Jump Drop", { "Fling", "Jump Drop" },
            function(dir, update)
                if dropActive then
                    stopDropBrainrot()
                end
    ...
    mkSect(mainPage, "PersonalizaciÃ³n")
    ...
    do
        ...
        local function toggleFov()
            ...
            expandBtn.Text = fovContainer.Visible and "â–¼" or "â–¶"
        end
    ...
    setNeonWeatherVisual = mkToggle(mainPage, "ï¿½ï¿½ Clima Neon", function(on)
...
UIS.InputBegan:Connect(function(input, gpe)
    ...
    if kbMatch(KB.TPLock, kc) then
        ...
        if tpBatFloatingButton then
            local btnFrame = tpBatFloatingButton:FindFirstChild("Frame")
            if btnFrame then
                btnFrame.BackgroundColor3 = _G.AceAntiDesyncAimbotOn and Color3.fromRGB(180, 180, 190) or Color3.fromRGB(0, 0, 0)
                local label = btnFrame:FindFirstChild("TextLabel")
                if label then
    ...
    if kbMatch(KB.BatV2, kc) then
        ...
        if batV2FloatingButton then
            local btnFrame = batV2FloatingButton:FindFirstChild("Frame")
            if btnFrame then
                btnFrame.BackgroundColor3 = autoBatV2Enabled and Color3.fromRGB(180, 180, 190) or Color3.fromRGB(0, 0, 0)
                local label = btnFrame:FindFirstChild("TextLabel")
                if label then
