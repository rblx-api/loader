-- VYNX DUELS - Optimized (Removed: PING, LAGGER, ANIMATIONS, SKINS/ACCESSORIES)
...
do
    ...
    if g.__VYNX_UNLOAD then pcall(g.__VYNX_UNLOAD) end
...
local M = {}
M._allConns = {}
local function trackConn(conn) if conn then table.insert(M._allConns, conn) end; return conn end
...
M.uiScale = 0.8
if UIS.TouchEnabled and not UIS.KeyboardEnabled then M.uiScale = 0.7 end
...
function M.updateRagdollTimer(duration)
    ...
    if duration <= 0 then M.isRagdollActive = false; if M.headIndicator and M.headIndicator.ragdollTimer then M.headIndicator.ragdollTimer.Text = "" end; return end
    ...
    M.ragdollTimerThread = task.spawn(function()
        while M.isRagdollActive and M.ragdollTimerRemaining > 0 do
            ...
            if remaining <= 0 then M.isRagdollActive = false; if M.headIndicator and M.headIndicator.ragdollTimer then M.headIndicator.ragdollTimer.Text = "" end; break end
            task.wait(0.05)
        end
        M.ragdollTimerThread = nil
    end)
...
function M.onHumanoidStateChanged(old,new)
    ...
    if isRag and not hum.PlatformStand then M.updateRagdollTimer(2.6) end
end

function M.onMedusaStateChanged()
    local char=player.Character;if not char then return end
    local hum=char:FindFirstChildOfClass("Humanoid")
    if hum and hum.PlatformStand then M.updateRagdollTimer(4.5) end
end

function M.setupRagdollTriggers()
    local char=player.Character;if not char then return end
    local hum=char:FindFirstChildOfClass("Humanoid")
    if hum then hum.StateChanged:Connect(M.onHumanoidStateChanged); hum:GetPropertyChangedSignal("PlatformStand"):Connect(M.onMedusaStateChanged) end
end
...
function M.createPlayerSpeedGui(plr)
    ...
    M.addShimmerToLabel(label, Color3.fromRGB(255,255,255), Color3.fromRGB(255,255,255))
    local conn
    conn = char.AncestryChanged:Connect(function(_, parent)
        if not parent then M.removePlayerSpeedGui(plr); if conn then conn:Disconnect() end end
    end)
...
function M.togglePlayerSpeeds(on)
    ...
    if on then
        for _, plr in ipairs(Players:GetPlayers()) do if plr ~= player then M.createPlayerSpeedGui(plr) end end
...
function M.toggleESP(on)
    ...
    if on then
        for _, plr in ipairs(Players:GetPlayers()) do if plr ~= player then M.addESP(plr) end end
        if not M._espPlayerAdded then
            M._espPlayerAdded = Players.PlayerAdded:Connect(function(p)
                if p ~= player and M.playerESPEnabled then
                    p.CharacterAdded:Connect(function() task.wait(0.5); M.addESP(p) end)
        ...
        if not M._espPlayerRemoved then
            M._espPlayerRemoved = Players.PlayerRemoving:Connect(function(p) M.removeESP(p) end)
...
function M.getPlayerAvatarThumb(userId)
    local ok, content = pcall(function() return Players:GetUserThumbnailAsync(userId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size100x100) end)
...
function M.addAvatarESP(plr)
    ...
    if M.avatarESPList[plr] then
        ...
        if data.bb and head and data.bb.Adornee ~= head then data.bb.Adornee = head; data.bb.Parent = head end
        return
    end
    local char = plr.Character
    if not char then return end
...
function M.clearAvatarESP()
    for plr, _ in pairs(M.avatarESPList or {}) do M.removeAvatarESP(plr) end
end

function M.refreshAllAvatarESP()
    if not M.avatarESPEnabled then M.clearAvatarESP(); return end
    for _, plr in ipairs(Players:GetPlayers()) do if plr ~= player then pcall(function() M.addAvatarESP(plr) end) end end
end
    ...
    if M.avatarESPEnabled then
        ...
        if not M._avatarESPPlayerAdded then
            M._avatarESPPlayerAdded = Players.PlayerAdded:Connect(function(p)
                ...
                p.CharacterAdded:Connect(function() task.wait(0.4); if M.avatarESPEnabled then M.addAvatarESP(p) end end)
                if p.Character then task.defer(function() task.wait(0.3); if M.avatarESPEnabled then M.addAvatarESP(p) end end) end
            end)
            M.trackConn(M._avatarESPPlayerAdded)
        end
        if not M._avatarESPPlayerRemoving then
            M._avatarESPPlayerRemoving = Players.PlayerRemoving:Connect(function(p) M.removeAvatarESP(p) end)
...
function M.setupHeadIndicator(char)
    ...
    local accent = (M.Theme and M.Theme.Accent) or Color3.fromRGB(255,255,255)
...
function M.updateHeadTheme()
    ...
    local accent = (M.Theme and M.Theme.Accent) or M.UI_ACCENT or Color3.fromRGB(255,255,255)
    if name == "White" or name == "Default" then accent = Color3.fromRGB(255, 255, 255) end
    local STROKE = Color3.fromRGB(0, 0, 0)
...
function M.startHeadSpeedUpdates()
    ...
    speedUpdateConn = RunService.Heartbeat:Connect(function(dt)
        ...
        local displaySpeed
        if M.autoLeftEnabled or M.autoRightEnabled then displaySpeed = M.NS
        else displaySpeed = M.getActiveMoveSpeed() end
...
function M.buildStatusUI()
    ...
    local accent = (M.Theme and M.Theme.Accent) or M.UI_ACCENT or Color3.fromRGB(165, 75, 255)
    ...
    local ok = false
    if gethui then ok = pcall(function() gui.Parent = gethui() end)
    elseif syn and syn.protect_gui then ok = pcall(function() syn.protect_gui(gui); gui.Parent = game:GetService("CoreGui") end) end
    ...
    local function stopShimmer()
        if shimmerThread then task.cancel(shimmerThread); shimmerThread = nil end
    end
    startShimmer()
    ...
    do
        ...
        fpsConn = RunService.RenderStepped:Connect(function(delta)
            if not gui.Parent then if fpsConn then fpsConn:Disconnect() end; return end
            ...
            if timeAccum >= 0.5 then
                ...
                if infoLbl and infoLbl.Parent then infoLbl.Text = ("FPS %d · PING %dms"):format(fps, ping) end
            end
        end)
    end
    ...
    M.updateRadiusMarker = function()
        if M.statusRadiusLbl then
            local r = 62
            pcall(function() r = (M.getActiveStealRadius and M.getActiveStealRadius()) or (M.Steal and M.Steal.StealRadius) or 62 end)
    ...
    if M.applyStealBarTheme then pcall(function() M.applyStealBarTheme(accent) end) end
end
...
function M.updateStealProgress(progress, label)
    ...
    if M.statusBarPctLbl then M.statusBarPctLbl.Text = pct .. "%" end
    if M.statusSubLbl then
        if type(label) == "string" and label ~= "" then M.statusSubLbl.Text = string.upper(label)
        elseif progress > 0 then M.statusSubLbl.Text = "HOLDING"
        else local ready = M.Steal and M.Steal.AutoStealEnabled; M.statusSubLbl.Text = ready and "READY" or "OFF" end
    elseif M.statusPctLbl then
        if type(label) == "string" and label ~= "" then M.statusPctLbl.Text = string.upper(label)
        elseif progress > 0 then M.statusPctLbl.Text = "HOLDING " .. pct .. "%"
        else local ready = M.Steal and M.Steal.AutoStealEnabled; M.statusPctLbl.Text = ready and "READY" or "OFF" end
    end
    if M.statusModeLbl then
        local modeText = tostring(M.stealMode or "Normal")
        if modeText == "V1" then modeText = "Normal" end
...
function M.updateStatusRadius()
    ...
    if M.updateRadiusMarker then M.updateRadiusMarker() end
end

if not fireproximityprompt then
    fireproximityprompt = (getgenv and getgenv().fireproximityprompt) or (genv and genv().fireproximityprompt) or function(prompt) pcall(function() prompt:InputHoldBegin(); task.wait(0.05); prompt:InputHoldEnd() end) end
end
...
local function findPromptNormal(ad)
    ...
    local prompt = nil
    if att then
        for _, p in ipairs(att:GetChildren()) do if p:IsA("ProximityPrompt") then prompt = p; break end end
    end
    if not prompt then
        for _, obj in ipairs(spawn:GetDescendants()) do if obj:IsA("ProximityPrompt") then prompt = obj; break end end
    end
...
local function buildCallbacks(prompt)
    ...
    if ok1 and type(c1) == "table" then
        for _, conn in ipairs(c1) do if type(conn.Function) == "function" then table.insert(data.holdCallbacks, conn.Function) end end
    end
    local ok2, c2 = pcall(getconnections, prompt.Triggered)
    if ok2 and type(c2) == "table" then
        for _, conn in ipairs(c2) do if type(conn.Function) == "function" then table.insert(data.triggerCallbacks, conn.Function) end end
    end
    if #data.holdCallbacks > 0 or #data.triggerCallbacks > 0 then M.stealCache[prompt] = data end
end
...
local function execStealNormal(prompt, animalName)
    ...
    M.progressConn = RunService.Heartbeat:Connect(function()
        if not M.isStealing then M.progressConn:Disconnect(); M.progressConn = nil; return end
...
function M.startNormalSteal()
    ...
    M.stealConn = RunService.Heartbeat:Connect(function()
        ...
        if prompt then buildCallbacks(prompt); execStealNormal(prompt, target.name) end
    end)
end

function M.stopNormalSteal()
    if M.stealConn then M.stealConn:Disconnect(); M.stealConn = nil end
    M.isStealing = false
    if M.progressConn then M.progressConn:Disconnect(); M.progressConn = nil end
    M.updateStealProgress(0)
end
...
do
    ...
    local function barSet(p, label)
        ...
        local pct = math.floor(progress * 100 + 0.5)
        local text = nil
        if type(label) == "string" and label ~= "" then text = string.upper(label); if progress > 0 then text = text .. "  " .. tostring(pct) .. "%" end end
        M.updateStealProgress(progress, text)
    end
    local function barReset() M.updateStealProgress(0) end
    local function execStealV2(prompt)
        ...
        local function modeStillActive() return V2.enabled and M.stealMode == "V2" and V2.isStealing end
        ...
        local function restartFromZero()
            ...
            restarting = true
            V2.paused = false; V2.pauseStarted = nil; V2.pausedDuration = 0
        ...
        V2.progressConn = RunService.Heartbeat:Connect(function()
            if not V2.isStealing then if V2.progressConn then V2.progressConn:Disconnect(); V2.progressConn = nil end; return end
            if not modeStillActive() or not canStillGrab() then restartFromZero(); return end
            local elapsed = tick() - V2.stealStartTime - (V2.pausedDuration or 0)
            if V2.paused and V2.pauseStarted then elapsed = V2.pauseStarted - V2.stealStartTime - (V2.pausedDuration or 0) end
            barSet(math.clamp(elapsed / duration, 0, 1))
        end)
        task.spawn(function()
            for _, fn in ipairs(data.holdCallbacks) do task.spawn(fn) end
            if #data.holdCallbacks == 0 and #data.triggerCallbacks == 0 and fireproximityprompt then pcall(function() fireproximityprompt(prompt, duration) end) end
            ...
            while modeStillActive() do
                if not canStillGrab() then restartFromZero(); return end
                if isCloseEnoughToGrab() then wasAbleToGrab = true; break end
                if tick() - waitStart >= 0.95 then wasAbleToGrab = true; break end
                task.wait()
            end
            if not modeStillActive() or not canStillGrab() then restartFromZero(); return end
    ...
    function M.startV2Steal()
        ...
        V2.conn = RunService.Heartbeat:Connect(function()
            ...
            local prompt = M.promptCache[target.uid]
            if not prompt or not prompt.Parent then prompt = findPromptNormal(target) end
            if prompt then execStealV2(prompt) end
        end)
    end
...
do
    ...
    local function findPromptForAnimal(animalData)
        ...
        for _, prompt in ipairs(attach:GetChildren()) do
            if prompt:IsA("ProximityPrompt") then A.promptCache[animalData.uid] = prompt; return prompt end
        end
        return nil
    end
            ...
            if typeof(animalList) == "table" then
                for slot, animalData in pairs(animalList) do
                    if type(animalData) == "table" then
                        ...
                        if info then table.insert(newCache, {
    ...
    local function pickClosest()
        ...
        for _, animalData in ipairs(A.animals) do
            if not isMyBaseAnimal(animalData) then
                ...
                if dist <= (A.primeRange or 80) and dist < bestDist then best, bestDist = animalData, dist end
            end
        end
        return best
    end
    local function buildCallbacksSemi(prompt)
        ...
        if okHold and type(holds) == "table" then
            for _, conn in ipairs(holds) do if type(conn.Function) == "function" then table.insert(data.holdCallbacks, conn.Function) end end
        end
        local okTrigger, triggers = pcall(getconnections, prompt.Triggered)
        if okTrigger and type(triggers) == "table" then
            for _, conn in ipairs(triggers) do if type(conn.Function) == "function" then table.insert(data.triggerCallbacks, conn.Function) end end
        end
    ...
    local function executeSemi(prompt, animalData)
        ...
        task.spawn(function()
            ...
            if fired then barSet(1, "STOLE " .. tostring(A.state.label))
            else barSet(0, A.state.lastResult) end
...
local function v3HoldPrompt(prompt)
    ...
    local ok = pcall(function() if prompt.InputHoldBegin then prompt:InputHoldBegin() end end)
    if not ok then pcall(function() if fireproximityprompt then fireproximityprompt(prompt) end end) end
    buildCallbacks(prompt)
    local data = M.stealCache[prompt]
    if data then for _, fn in ipairs(data.holdCallbacks) do task.spawn(function() pcall(fn) end) end end
    return true
end
...
local function v3TriggerPrompt(prompt)
    ...
    buildCallbacks(prompt)
    local data = M.stealCache[prompt]
    if data then for _, fn in ipairs(data.triggerCallbacks) do task.spawn(function() pcall(fn) end) end end
    pcall(function() if prompt.InputHoldEnd then prompt:InputHoldEnd() end end)
...
function M.startV3Steal()
    ...
    M.V3.conn = RunService.Heartbeat:Connect(function(dt)
        ...
        if tick() < (M.V3.cooldownUntil or 0) then M.updateStealProgress(0); return end
        ...
        if inRange then
            ...
            if M.V3.progress > 0 then
                ...
                M.updateStealProgress(M.V3.progress)
                if M.V3.progress <= 0 then M.V3.currentUid = nil; M.isStealing = false; M.updateStealProgress(0)
                else M.isStealing = true end
            else M.isStealing = false end
        end
    end)
end
...
function M.stopV3Steal()
    M.V3.enabled = false
    if M.V3.holdPrompt then v3ReleasePrompt(M.V3.holdPrompt) end
    if M.V3.conn then pcall(function() M.V3.conn:Disconnect() end); M.V3.conn = nil end
    M.V3.progress = 0
    M.V3.currentUid = nil
    M.V3.holding = false
    M.V3.holdPrompt = nil
...
function M.startAutoSteal()
    ...
    if mode == "Semi" then M.startSemiSteal()
    elseif mode == "V2" then M.startV2Steal()
    elseif mode == "V3" then M.startV3Steal()
    else M.startNormalSteal() end
end
...
function M.findMedusa()
    ...
    local bp=player:FindFirstChild("Backpack");if bp then for _,t in ipairs(bp:GetChildren()) do if t:IsA("Tool") then local n=t.Name:lower();if n:find("medusa") or n:find("head") or n:find("stone") then return t end end end
    return nil
end
...
function M.getAutoBatTarget()
    ...
    if now - (M._aimbotLastScan or 0) <= 0.1 and M._aimbotTarget and M._aimbotTarget.Parent then
        ...
        if hum and hum.Health > 0 then return M._aimbotTarget end
    end
...
function M.getNormalAimbotSpeed()
    if M.laggerEnabled then return tonumber(M.laggerAimbotSpeed) or tonumber(M.aimbotSpeed) or 57 end
...
function M.startBatAimbot()
    ...
    M.aimbotConn = RunService.Heartbeat:Connect(function()
        ...
        if M.aimbotRotationEnabled ~= false then hum.AutoRotate = false
        ...
        if distFlat > 0.01 then flatDir = flatDir.Unit
        else flatDir = Vector3.new(0, 0, 1) end

        local chaseSpeed = M.getNormalAimbotSpeed()
        local hitRange = tonumber(M.aimbotHitRange) or 9.5

        if distFlat < hitRange * 0.9 then chaseSpeed = math.max(chaseSpeed * 0.85, chaseSpeed * 0.75) end
    ...
    RunService:BindToRenderStep("VynxAimbotRotCam", Enum.RenderPriority.Camera.Value + 1, function()
        ...
        if M.aimbotRotationEnabled ~= false then
            ...
            if alpha >= 0.99 then root.CFrame = goalOnlyRot
            else root.CFrame = root.CFrame:Lerp(goalOnlyRot, alpha) end
            root.AssemblyAngularVelocity = Vector3.zero
        else
            hum.AutoRotate = true
            local cam = workspace.CurrentCamera
...
function M.swingCurrentBatAimbot(char)
    ...
    local bat = M.findBatForAimbot()
    if bat then M._aimbotSwingBat(char or player.Character, bat) end
end
...
function M._bypassFindBat()
    ...
    if bp then
        ...
        if tool and tool:IsA("Tool") then tool.Parent = char; return tool end
        for _, t in ipairs(bp:GetChildren()) do
            if t:IsA("Tool") and (t.Name:lower():find("bat") or t.Name:lower():find("slap")) then t.Parent = char; return t end
        end
    end
    return nil
end
...
function M._bypassProtectCharacter(char)
    ...
    pcall(function()
        ...
        M._bypassGodStateConn = hum.StateChanged:Connect(function(_, new)
            if not M.bypassAimbotEnabled then return end
            if new == Enum.HumanoidStateType.Dead or new == Enum.HumanoidStateType.Ragdoll or new == Enum.HumanoidStateType.FallingDown or new == Enum.HumanoidStateType.Physics then
                pcall(function()
                    hum.Health = hum.MaxHealth or 100
                    hum.PlatformStand = false
                    hum.Sit = false
...
function M.startAntiDie()
    ...
    local function forceAlive(char)
        ...
        pcall(function()
            ...
            if st == Enum.HumanoidStateType.Dead or st == Enum.HumanoidStateType.FallingDown or st == Enum.HumanoidStateType.Ragdoll or st == Enum.HumanoidStateType.Physics or hum.Health <= 0 then
                hum.Health = hum.MaxHealth
                hum.PlatformStand = false
                hum.Sit = false
            ...
            if hrp and not M.dropActive and not M.bypassAimbotEnabled then
                ...
                if v ~= v then hrp.AssemblyLinearVelocity = Vector3.zero; hrp.AssemblyAngularVelocity = Vector3.zero
...
function M.stopAntiDie()
    ...
    M._adClearConns()
    if M._antiDieRenderConn then pcall(function() M._antiDieRenderConn:Disconnect() end); M._antiDieRenderConn = nil end
end
...
function M.startAntiFling()
    if M._antiFlingConn then pcall(function() M._antiFlingConn:Disconnect() end); M._antiFlingConn = nil end
    M.antiFlingEnabled = true
    M._antiFlingConn = RunService.Heartbeat:Connect(function()
        ...
        pcall(function()
            ...
            if v.Magnitude > maxSpd or math.abs(v.Y) > maxY then
                root.AssemblyLinearVelocity = Vector3.new(math.clamp(v.X, -maxSpd, maxSpd), math.clamp(v.Y, -maxY, maxY), math.clamp(v.Z, -maxSpd, maxSpd))
                bad = true
            end
            local ang = root.AssemblyAngularVelocity
            if ang.Magnitude > maxAng then root.AssemblyAngularVelocity = Vector3.zero; bad = true end
            if v ~= v or ang ~= ang then root.AssemblyLinearVelocity = Vector3.zero; root.AssemblyAngularVelocity = Vector3.zero; bad = true end
            if hum then
                if bad then hum.PlatformStand = false; hum.Sit = false; pcall(function() hum:ChangeState(Enum.HumanoidStateType.Running) end) end
                local st = hum:GetState()
                if st == Enum.HumanoidStateType.Flying or st == Enum.HumanoidStateType.Ragdoll or st == Enum.HumanoidStateType.FallingDown or st == Enum.HumanoidStateType.Physics then
                    if bad or v.Magnitude > maxSpd * 0.7 then pcall(function() hum:ChangeState(Enum.HumanoidStateType.Running); hum.PlatformStand = false; hum.Sit = false end) end
                end
            end
        end)
    end)
...
function M.enableBypassGodmode()
    ...
    M._bypassGodConn = RunService.Heartbeat:Connect(function()
        ...
        pcall(function()
            ...
            hum.Sit = false
            local st = hum:GetState()
            if st == Enum.HumanoidStateType.Dead or st == Enum.HumanoidStateType.Ragdoll or st == Enum.HumanoidStateType.FallingDown or st == Enum.HumanoidStateType.Physics then
                hum.Health = hum.MaxHealth
                hum:ChangeState(Enum.HumanoidStateType.Running)
            end
            if root then
...
function M.startBypassAimbot()
    ...
    if M.autoTPEnabled then M._autoTPWasEnabledForBypass = true; M.stopAutoTP(); if M.setAutoTPVisual then M.setAutoTPVisual(false) end end
    ...
    if char0 then
        ...
        if M._bypassHum then M.bypassPrevAutoRotate = M._bypassHum.AutoRotate; M._bypassHum.AutoRotate = false end
    end
    ...
    M.bypassAimbotConn = RunService.Heartbeat:Connect(function()
        ...
        if sethiddenproperty then pcall(function() sethiddenproperty(M._bypassHRP, "PhysicsRepRootPart", target) end) end

        local targetPos = target.Position + Vector3.new(0, 0.9, 0)
        if (M._bypassHRP.Position - targetPos).Magnitude > 8 then M._bypassHRP.CFrame = CFrame.new(targetPos) end
...
function M.stopBypassAimbot()
    ...
    if hum then hum.AutoRotate = (M.bypassPrevAutoRotate == nil) and true or M.bypassPrevAutoRotate end
...
function M.toggleBypassAimbot()
    ...
    if M.bypassAimbotEnabled then M.startBypassAimbot()
    else M.stopBypassAimbot() end
...
function M.showHardHitRing()
    ...
    cyl.Adornee = hrp2
    cyl.Color3 = (UI_ACCENT or Color3.fromRGB(235, 235, 235))
...
function M.hideHardHitRing()
    if M._hardHitRing then pcall(function() M._hardHitRing:Destroy() end); M._hardHitRing = nil end
end
...
function M.startHardHit()
    ...
    M._hardHitConn = RunService.Heartbeat:Connect(function()
        ...
        local root = char:FindFirstChild("HumanoidRootPart")
        if not root then return end
        if not M._hardHitRing or not M._hardHitRing.Parent then M.showHardHitRing() end
...
function M.stopHardHit()
    M.hardHitEnabled = false
    if M._hardHitConn then pcall(function() M._hardHitConn:Disconnect() end); M._hardHitConn = nil end
    M.hideHardHitRing()
end
...
function M.enableUltraMode()
    ...
    for _, obj in pairs(workspace:GetDescendants()) do M.applyUltraDerender(obj) end
...
function M.disableUltraMode()
    M.ultraModeEnabled = false
    if M._ultraDescConn then pcall(function() M._ultraDescConn:Disconnect() end); M._ultraDescConn = nil end
end
    ...
    for _, plot in ipairs(plots:GetChildren()) do
        ...
        if not isSummer then
            for _, d in ipairs(plot:GetDescendants()) do
                if M.isSummerBaseName(d.Name) then isSummer = true; break end
            end
        end
        if not isSummer then continue end
        for _, d in ipairs(plot:GetDescendants()) do
            if M.isAnchorName(d.Name) then M.stripBlockingAnchor(d) end
        end
    end
end
...
function M.enableAntiSummerBase()
    ...
    if M.antiSummerBaseConn then pcall(function() M.antiSummerBaseConn:Disconnect() end); M.antiSummerBaseConn = nil end
    M.antiSummerBaseConn = workspace.DescendantAdded:Connect(function(obj)
        ...
        task.defer(function()
            ...
            if underPlots and nearSummer then M.stripBlockingAnchor(obj) end
        end)
    end)
    task.spawn(function()
...
function M.applyAntiLagDerender(obj)
    ...
    pcall(function()
        ...
        elseif obj:IsA("BasePart") or obj:IsA("MeshPart") then
            obj.CastShadow = false
            if obj.Reflectance and obj.Reflectance > 0 then obj.Reflectance = 0 end
        end
    end)
end
...
function M.enableAntiLag()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr.Character then
            for _, obj in ipairs(plr.Character:GetDescendants()) do M.applyAntiLagDerender(obj) end
        end
    end
...
function M.enableNukeOptimizer()
    ...
    local function IsUnderPlots(obj)
        ...
        while p and p ~= game do
            if p.Name == "Plots" or p.Name == "AnimalPodiums" or p.Name == "PlotSign" then return true end
    ...
    local function StripObject(obj)
        pcall(function()
            if obj:IsA("Texture") or obj:IsA("Decal") or obj:IsA("SpecialMesh") then SafeDestroy(obj)
            elseif obj:IsA("Beam") then return
            elseif obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Smoke") or obj:IsA("Fire") or obj:IsA("Sparkles") then
                pcall(function() obj.Enabled = false end)
                SafeDestroy(obj)
            elseif obj:IsA("SurfaceAppearance") then SafeDestroy(obj)
    ...
    local function CleanObject(obj)
        pcall(function()
            ...
            elseif obj:IsA("SpecialMesh") then SafeDestroy(obj) end
        end)
    end
    ...
    table.insert(M._NukeThreads, task.spawn(function()
        ...
        for _, obj in ipairs(workspace:GetDescendants()) do
            ...
            elseif IsBase(obj) then MakeTransparent(obj)
        ...
        for _, obj in ipairs(workspace:GetDescendants()) do
            if not IsUnderPlots(obj) then MakeTransparent(obj) end
        end
    end))
...
function M.stopAntiRagdoll()
    if M.Conns.antiRag then M.Conns.antiRag:Disconnect(); M.Conns.antiRag = nil end
end
...
function M.getMyBaseCFrame()
    ...
    for _, plot in ipairs(plots:GetChildren()) do
        ...
        if yb and yb:IsA("BillboardGui") and yb.Enabled == true then
            ...
            if spawn and spawn:IsA("BasePart") then return spawn.CFrame + Vector3.new(0, 3, 0) end
            local ok, pivot = pcall(function() return plot:GetPivot() end)
            if ok and pivot then return pivot + Vector3.new(0, 5, 0) end
            if sign:IsA("BasePart") then return sign.CFrame + Vector3.new(0, 5, 0) end
            local pp = sign:FindFirstChildWhichIsA("BasePart", true)
            if pp then return pp.CFrame + Vector3.new(0, 5, 0) end
        end
    end
    return nil
end
        ...
        if plr ~= player and plr.Character then
            ...
            if tRoot and hum and hum.Health > 0 then
                ...
                if dist <= range then
                    ...
                    if tool then
                        local n = tool.Name:lower()
                        if n:find("bat") or n:find("slap") then return true end
                    end
                    return true
                end
            end
...
function M.tryRagdollTPBase()
    ...
    task.defer(function()
        ...
        if not hum or hum.Health <= 0 then return end
        local st = hum:GetState()
        local stillRag = hum.PlatformStand or st == Enum.HumanoidStateType.Physics or st == Enum.HumanoidStateType.Ragdoll or st == Enum.HumanoidStateType.FallingDown
        if stillRag or true then M.tpToMyBaseFromRagdoll() end
    end)
end

function M._hookRagdollTPOnChar(char)
    if M._ragdollTPCharConn then pcall(function() M._ragdollTPCharConn:Disconnect() end); M._ragdollTPCharConn = nil end
    if not char then return end
    local hum = char:FindFirstChildOfClass("Humanoid") or char:WaitForChild("Humanoid", 5)
    if not hum then return end
    M._ragdollTPCharConn = hum.StateChanged:Connect(function(_, new)
        if not M.ragdollTPBaseEnabled then return end
        if new == Enum.HumanoidStateType.Physics or new == Enum.HumanoidStateType.Ragdoll or new == Enum.HumanoidStateType.FallingDown then M.tryRagdollTPBase() end
    end)
    hum:GetPropertyChangedSignal("PlatformStand"):Connect(function()
        if not M.ragdollTPBaseEnabled then return end
        if hum.PlatformStand then M.tryRagdollTPBase() end
    end)
end

function M.startRagdollTPBase()
    M.ragdollTPBaseEnabled = true
    if player.Character then M._hookRagdollTPOnChar(player.Character) end
    if not M._ragdollTPAddedConn then
        M._ragdollTPAddedConn = player.CharacterAdded:Connect(function(char)
            task.wait(0.2)
            if M.ragdollTPBaseEnabled then M._hookRagdollTPOnChar(char) end
        end)
    end
end
...
local function M_applyInfJumpBoost(root)
    ...
    conn = RunService.Heartbeat:Connect(function()
        if frameCount < M.INF_JUMP_BOOST_FRAMES then
            frameCount = frameCount + 1
            if bv and bv.Parent then bv.Velocity = bv.Velocity + Vector3.new(0, 0.01, 0) end
        else
    ...
    if pg then
        local function hookJumpButton(btn)
            if btn:IsA("GuiButton") and btn.Name == "JumpButton" and not btn:GetAttribute("InfJumpHooked") then
                ...
                btn.MouseButton1Down:Connect(function() if M.infJumpEnabled then M.jumpHeld = true end end)
...
UIS.InputBegan:Connect(function(inp, gpe)
    ...
    if M.infJumpEnabled and inp.UserInputType == Enum.UserInputType.Keyboard and inp.KeyCode == Enum.KeyCode.Space then M.jumpHeld = true end
end)

UIS.InputEnded:Connect(function(inp)
    if inp.UserInputType == Enum.UserInputType.Keyboard and inp.KeyCode == Enum.KeyCode.Space then M.jumpHeld = false end
end)
...
function M.stopManualInfJumpLoop()
    if M.infJumpThread then M.infJumpThread:Disconnect(); M.infJumpThread = nil end
    M.jumpHeld = false
    M._infJumpBoosting = false
end
...
function M.startHoldInfJump()
    ...
    M.holdInfJumpConn = RunService.Heartbeat:Connect(function()
        ...
        if isJumpHeld and vel.Y < 35 then root.AssemblyLinearVelocity = Vector3.new(vel.X, 55, vel.Z) end

        vel = root.AssemblyLinearVelocity
        if vel.Y < -120 then root.AssemblyLinearVelocity = Vector3.new(vel.X, -55, vel.Z) end
    end)
end

function M.stopHoldInfJump()
    if M.holdInfJumpConn then M.holdInfJumpConn:Disconnect(); M.holdInfJumpConn = nil end
end
...
function M.forceRestoreWalkAnims(char)
    ...
    if not char:FindFirstChild("Animate") then
        if M.restoreOriginalAnimate then pcall(function() M.restoreOriginalAnimate(char) end) end
    end
    if not char:FindFirstChild("Animate") then
        pcall(function()
            ...
            if tmpl then local clone = tmpl:Clone(); clone.Parent = char end
        end)
    end
end
...
function M.cursedInstaReset() return end
...
function M.hasBrainrotInHand()
    ...
    for _, item in ipairs(char:GetChildren()) do
        if item:IsA("Tool") then
            ...
            if name:find("brainrot", 1, true) or name:find("skibidi", 1, true) or name:find("toilet", 1, true) or name:find("steal", 1, true) or name:find("animal", 1, true) or name:find("pet", 1, true) then
                if not (name:find("slap", 1, true) or name:find("bat", 1, true)) then return true end
            end
        end
    end
    return false
...
function M.forceLaggerCarryWhileHolding() return M.hasBrainrotInHand() end
...
function M.safeMoveDirection(hrp, hum, dir, spd, dt)
    ...
    if flat.Magnitude < 0.01 then if hum then pcall(function() hum:Move(Vector3.zero, false) end) end; return end
    flat = flat.Unit
...
function M.stopAutoLeft()
    ...
    M.alPhase = 1
    local char = player.Character
    if char then local h = char:FindFirstChildOfClass("Humanoid"); local root = char:FindFirstChild("HumanoidRootPart"); M.safeStopMove(root, h) end
        ...
        elseif M.alPhase == 2 then
            ...
            if (tgt - hrp.Position).Magnitude < 1 then
                ...
                if M.autoPlayEnabled and M.autoPlayLockedDir then
                    task.defer(function()
                        if M.autoPlayEnabled and M.autoPlayLockedDir then M._runLockedAutoPlayPath() end
                    end)
                end
                return
            end
...
function M.getPlayerSide()
    ...
    if hrp.Position.Z >= midZ then return "left" end
    return "right"
end
...
function M._runLockedAutoPlayPath()
    ...
    if dir == "right" then M.autoLeftEnabled = false; M.autoRightEnabled = true; M.startAutoRight()
    else M.autoRightEnabled = false; M.autoLeftEnabled = true; M.startAutoLeft() end
end
...
function M.startAutoPlay()
    ...
    if side == "left" then M.autoPlayLockedDir = "right"
    else M.autoPlayLockedDir = "left" end
...
function M.toggleAutoPlay()
    if M.autoPlayEnabled or M.autoLeftEnabled or M.autoRightEnabled then M.stopAutoPlay()
    else M.startAutoPlay() end
        ...
        while M.antiKickEnabled do
            ...
            if char then
                ...
                for _, tool in ipairs(char:GetChildren()) do
                    if tool:IsA("Tool") then
                        local n = tool.Name:lower()
                        if n:find("brainrot") or n:find("skibidi") or n:find("toilet") then found = true; break end
                    end
                end
...
function M.safeModeHoldingBrainrot()
    ...
    for _, child in ipairs(char:GetChildren()) do
        if child:IsA("Model") and child:FindFirstChildWhichIsA("BasePart", true) then
            ...
            if n:find("brainrot") or n:find("animal") or n:find("carry") or n:find("grab") or n:find("steal") or n:find("hold") then return true end
        end
    end
    return false
end
...
function M.safeModeForceStop(reason)
    local stopped = false
    if M.autoBatEnabled then M.stopBatAimbot(); stopped = true end
    if M.bypassAimbotEnabled then M.stopBypassAimbot(); stopped = true end
    if M.autoLeftEnabled then M.autoLeftEnabled = false; if M.autoLeftSetVisual then M.autoLeftSetVisual(false) end; M.stopAutoLeft(); stopped = true end
    if M.autoRightEnabled then M.autoRightEnabled = false; if M.autoRightSetVisual then M.autoRightSetVisual(false) end; M.stopAutoRight(); stopped = true end
    if stopped then pcall(function() if type(showActionNotification) == "function" then showActionNotification(reason or "SAFE MODE LOCK") end end) end
end

function M.safeModeTryStart()
    if M.safeModeIsLocked() then M.safeModeForceStop("SAFE MODE LOCK"); return false end
    return true
end
...
if not M._safeModeMonitorStarted then
    ...
    RunService.Heartbeat:Connect(function()
        if M.safeModeEnabled and M.safeModeIsLocked() then M.safeModeForceStop("SAFE MODE LOCK") end
    end)
end
...
function M.setAutoCarryEnemyBase(on)
    ...
    if M.autoCarryEnemyBaseEnabled then M.startAutoCarryEnemyBase()
    else M.stopAutoCarryEnemyBase() end
end
...
function M.getActiveMoveSpeed()
    ...
    local holding = false
    pcall(function() if M.hasBrainrotInHand and M.hasBrainrotInHand() then holding = true end end)
    ...
    if uiMode == "Original" then
        if M.autoSwitchSpeedEnabled then
            ...
            if M.laggerModeEnabled then return (isSteal or holding) and lcs or ls end
    ...
    if M.speedBoosterEnabled == false then return 16 end
    if M.laggerCarryActive then return lcs end
    if M.carrySpeedActive then return cs end
    local useLagger = (tostring(M.speedBoosterPath) == "Lagger") or (M.laggerModeEnabled == true)
    if useLagger then return holding and lcs or ls end
    return holding and cs or ns
end
...
function M.stopWalkSpeedAutoSwitch()
    if M._autoSwitchSpeedConn then pcall(function() M._autoSwitchSpeedConn:Disconnect() end); M._autoSwitchSpeedConn = nil end
end
...
function M.refreshWalkSpeedAutoSwitch()
    ...
    else M.stopWalkSpeedAutoSwitch() end
end
...
function M.runDrop()
    ...
    dropConn = RunService.Heartbeat:Connect(function()
        ...
        if not currentChar or not currentRoot then if dropConn then pcall(function() dropConn:Disconnect() end) end; M.dropActive = false; return end
        if tick() - startTime >= duration then
            ...
            pcall(function() currentRoot.AssemblyLinearVelocity = Vector3.zero; currentRoot.AssemblyAngularVelocity = Vector3.zero; currentRoot.Velocity = Vector3.zero end)
            M.dropActive = false
            return
        end
...
function M.stopAutoTPForAction()
    if M.autoTPEnabled then M.stopAutoTP(); pcall(function() if M.setAutoTPVisual then M.setAutoTPVisual(false) end end); pcall(function() if M.saveConfig then M.saveConfig() end end) end
end
...
function M.startRemoveAcc()
    ...
    local function removeAccDo()
        ...
        for _,obj in ipairs(char:GetDescendants()) do
            if obj:IsA("Accessory") or obj:IsA("Hat") then
                if not M.removedAccessories[obj] then M.removedAccessories[obj] = true; pcall(function() obj:Destroy() end) end
            end
        end
    end
    removeAccDo()
...
function M.stopRemoveAcc()
    M.removeAccEnabled = false
    if M.removeAccConn then M.removeAccConn:Disconnect(); M.removeAccConn = nil end
    M.removedAccessories = {}
end
...
function M.loadBtnPositions()
    ...
    if (not data or next(data) == nil) and type(M._btnPosCache) == "table" then data = M._btnPosCache end
    return data or {}
end
...
function M.saveBtnPositions()
    ...
    M._btnPosCache = out
    pcall(function() if writefile then writefile(M.MOB_POS_FILE, HS:JSONEncode(out)) end end)
    pcall(function() if type(saveCherryConfig) == "function" then saveCherryConfig() end end)
end

function M.resetMobilePositions()
    pcall(function()
        if type(delfile) == "function" then delfile(M.MOB_POS_FILE)
        elseif type(writefile) == "function" then writefile(M.MOB_POS_FILE, "{}") end
    end)
    pcall(function()
        if isfile and isfile(M.MOB_POS_FILE) and type(writefile) == "function" then writefile(M.MOB_POS_FILE, "{}") end
    end)
...
function M.buildMobileButtons()
    ...
    if (not savedPositions or next(savedPositions) == nil) and type(M._btnPosCache) == "table" then savedPositions = M._btnPosCache end
    ...
    local CORNER_R = 16
    if shape == "Circle" then CORNER_R = math.floor(side / 2) end
    ...
    if not pcall(function() mobGui.Parent = game:GetService("CoreGui") end) then mobGui.Parent = player:WaitForChild("PlayerGui") end
    M.mobGuiRef = mobGui
    ...
    local accent = (M.Theme and M.Theme.Accent) or UI_ACCENT or M.UI_ACCENT or Color3.fromRGB(165, 75, 255)
    ...
    if (M.speedUIMode or "Original") ~= "Original" then
        ...
        for _, def in ipairs(btnDefs) do
            local k = def[1]
            if k ~= "carrySpeed" and k ~= "lagger" and k ~= "laggerCarry" then table.insert(filtered, def) end
        end
        btnDefs = filtered
    end
    ...
    for i, def in ipairs(btnDefs) do
        ...
        local function updateStyle(state)
            ...
            if isOn then
                ...
                if hasImg then bgImg.ImageTransparency = 0.35; btn.BackgroundTransparency = 0.25 end
            else
                if state == "hover" then
                ...
                elseif state == "press" then
                    ...
                    contStroke.Color = C_BORDER
                    contStroke.Transparency = 0.3
                    if hasImg then bgImg.ImageTransparency = 0.15; btn.BackgroundTransparency = 0.55 end
                end
            end
        end
        ...
        UIS.InputChanged:Connect(function(input)
            ...
            if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
                ...
                if not moved and (math.abs(delta.X) + math.abs(delta.Y)) > 8 then moved = true; clickBlocked = true end
                if moved then container.Position = UDim2.new(0, startPos.X.Offset + delta.X, 0, startPos.Y.Offset + delta.Y) end
            end
        end)
        UIS.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                if dragging and moved then pcall(function() M.saveBtnPositions() end) end
                dragging = false
                dragStart = nil
                startPos = nil
            end
        ...
        btn.Activated:Connect(function()
            ...
            if key == "drop" then M.runDrop()
            elseif key == "tpDown" then M.runTPFloor()
            elseif key == "autoLeft" then
                if M.autoBatEnabled then M.stopBatAimbot(); if M.autoBatSetVisual then M.autoBatSetVisual(false) end; if M.mobBtnRefs.autoBat then M.mobBtnRefs.autoBat(false) end end
                if M.autoPlayEnabled then M.stopAutoPlay() end
                if M.autoRightEnabled then M.stopAutoRight(); if M.autoRightSetVisual then M.autoRightSetVisual(false) end; if M.mobBtnRefs.autoRight then M.mobBtnRefs.autoRight(false) end end
                if M.autoLeftEnabled then M.stopAutoLeft()
                else M.startAutoLeft() end
            ...
            elseif key == "autoRight" then
                if M.autoBatEnabled then M.stopBatAimbot(); if M.autoBatSetVisual then M.autoBatSetVisual(false) end; if M.mobBtnRefs.autoBat then M.mobBtnRefs.autoBat(false) end end
                if M.autoPlayEnabled then M.stopAutoPlay() end
                if M.autoLeftEnabled then M.stopAutoLeft(); if M.autoLeftSetVisual then M.autoLeftSetVisual(false) end; if M.mobBtnRefs.autoLeft then M.mobBtnRefs.autoLeft(false) end end
                if M.autoRightEnabled then M.stopAutoRight()
                else M.startAutoRight() end
            ...
            elseif key == "autoBat" then
                ...
                if not M.autoBatEnabled then M.queueAutoBatStart()
...
if M._savedTheme and CHERRY_THEMES[M._savedTheme] then CherryConfig.Theme = M._savedTheme end
...
M.BG_IMAGE_IDS = {
    78248012786524, 79737099962715, 71211662493854, 15556272558, 1471587689,
    14349182390, 108236541541009, 86526406500677, 125028284461731, 138241571198031,
}
M.MOB_BTN_IMAGE_IDS = {
    92132591931954, 15101684346, 39396, 109592813321691, 83661129801187, 94353803110527, 109100201685955,
}
...
local function loadCherryConfig()
    ...
    if ok and type(d)=="table" then
        ...
        do
            ...
            if loaded and loaded ~= oldDefault and loaded ~= forced then M.customBgId = loaded
            else M.customBgId = forced end
        end
        if type(d.customBgOpacity)=="number" then M.customBgOpacity=math.clamp(d.customBgOpacity,0,1) end
        if type(d.mobBtnBgId)=="number" then M.mobBtnBgId = d.mobBtnBgId
        elseif type(d.mobBtnBgId)=="string" and tonumber(d.mobBtnBgId) then M.mobBtnBgId = tonumber(d.mobBtnBgId)
        else M.mobBtnBgId = M.DEFAULT_MOB_BTN_BG_ID or 92132591931954 end
...
local function saveCherryConfig()
    ...
    local cfg = {
        ...
        hardHitEnabled=M.hardHitEnabled, hardHitRadius=M.hardHitRadius, ultraModeEnabled=M.ultraModeEnabled, antiDesyncPanelOpen=M.antiDesyncPanelOpen, antiDesyncShield=M.antiDesyncShield, antiDesyncVelocity=M.antiDesyncVelocity,
...
function M.collectSharePayload()
    ...
    local ok, disk = pcall(function()
        if isfile and isfile("CherryConfig.json") then return HS:JSONDecode(readfile("CherryConfig.json")) end
    end)
    if ok and type(disk) == "table" then cfg = disk end
    local btnPos = {}
    pcall(function()
        if M.mobGuiRef then
...
function M.applySharePayload(d)
    ...
    pcall(function() if writefile then writefile("CherryConfig.json", HS:JSONEncode(d)) end end)
    if type(d._btnPos) == "table" and writefile then pcall(function() writefile(M.MOB_POS_FILE, HS:JSONEncode(d._btnPos)) end) end
    ...
    pcall(function()
        ...
        if M.infJumpEnabled then M.infJumpMode = "hold"; M.startHoldInfJump() end
...
function M.importShareCode(code)
    ...
    code = code:gsub("%s+", "")
    if code:sub(1, 6) == "VYNX1:" then code = code:sub(7) end
    local ok, json = pcall(function() return _vynxB64Dec(code) end)
    if not ok or not json or json == "" then return false, "decode failed" end
...
function M.copyShareToClipboard()
    ...
    local copied = false
    pcall(function() if setclipboard then setclipboard(code); copied = true end end)
    if not copied then pcall(function() if toclipboard then toclipboard(code); copied = true end end) end
    if not copied then pcall(function() if set_clipboard then set_clipboard(code); copied = true end end) end
    return code, copied
end
...
local function themeDarkFromAccent(accent, amount)
    ...
    return Color3.new(math.clamp(accent.R * amount, 0, 1), math.clamp(accent.G * amount, 0, 1), math.clamp(accent.B * amount, 0, 1))
end
...
local function isPurpleishColor(c)
    ...
    if b >= 0.45 and b >= r * 0.9 and b > g * 1.05 and (b - math.min(r, g)) >= 0.12 then return true end
    if b >= 0.7 and r >= 0.4 and g >= 0.35 and b > r and b > g and (b - g) >= 0.12 then return true end
    if b < 0.35 and r < 0.28 and g < 0.28 and b > r and b > g and (b - r) >= 0.02 and (b - g) >= 0.015 then return true end
    return false
end
...
local function mapPurpleishToAccent(c, accent, dim)
    ...
    if lum < 0.22 then return themeDarkFromAccent(accent, math.clamp(lum * 1.35, 0.08, 0.28))
    elseif lum > 0.75 then return accent:Lerp(Color3.new(1, 1, 1), 0.35)
    else return accent:Lerp(dim or accent:Lerp(Color3.new(0, 0, 0), 0.35), 1 - lum) end
end
...
local function applyAccentFromTheme()
    ...
    M.UI_ROW_BG = row
    M.UI_BTN_BG = btn
    M.Theme = { Name = name, Accent = accent, AccentDim = dim, Bg = bg, Row = row, Btn = btn, }
    M.UI_TOGGLE_OFF = tog
...
local function liveToggleOff() return M.UI_TOGGLE_OFF or UI_TOGGLE_OFF or Color3.fromRGB(50, 50, 60) end
local function liveKnobOn() return M.UI_KNOB_ON or UI_KNOB_ON or Color3.fromRGB(255, 255, 255) end
local function liveKnobOff() return M.UI_TOGGLE_KNOB or UI_TOGGLE_KNOB or Color3.fromRGB(180, 180, 190) end
local function liveRowBg() return (M.Theme and M.Theme.Row) or M.UI_ROW_BG or UI_ROW_BG or Color3.fromRGB(28, 28, 34) end
...
function M.recolorBlacksToTheme(root)
    ...
    local function recolor(obj)
        if obj.Name == "Track" or obj.Name == "Fill" then return end
        local p = obj.Parent
        ...
        if obj:IsA("GuiObject") then
            ...
            if ok and typeof(col) == "Color3" then
                if isNearBlack(col) then
                    if obj:IsA("Frame") and (obj.Name == "Main" or obj.Name == "MainFrame" or obj.Name == "Frame" or obj.Size.X.Scale >= 0.9) then obj.BackgroundColor3 = bg
                    elseif obj:IsA("TextButton") or obj:IsA("ImageButton") then obj.BackgroundColor3 = btn
                    else obj.BackgroundColor3 = row end
                elseif isPurpleishColor(col) then obj.BackgroundColor3 = mapPurpleishToAccent(col, accent, dim) end
            end
            if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
                local okT, tc = pcall(function() return obj.TextColor3 end)
                if okT and isPurpleishColor(tc) then obj.TextColor3 = accent end
            end
        end
        if obj:IsA("UIStroke") then
            local ok, col = pcall(function() return obj.Color end)
            if ok and typeof(col) == "Color3" then
                if isNearBlack(col, 0.25) then obj.Color = dim
                elseif isPurpleishColor(col) then obj.Color = accent end
            end
        end
        if obj:IsA("UIGradient") then
            pcall(function()
                ...
                for i, kp in ipairs(keys) do
                    local c = kp.Value
                    if isPurpleishColor(c) then c = mapPurpleishToAccent(c, accent, dim); changed = true end
                    table.insert(newKeys, ColorSequenceKeypoint.new(kp.Time, c))
                end
                if changed then obj.Color = ColorSequence.new(newKeys)
                else obj.Color = ColorSequence.new({
    ...
    recolor(root)
    for _, d in ipairs(root:GetDescendants()) do recolor(d) end
end
...
function M.recolorAllVynxGuis()
    ...
    local parents = {}
    pcall(function() local pg = player:FindFirstChild("PlayerGui"); if pg then table.insert(parents, pg) end end)
    ...
    local names = { VynxHubUI=true, StealProgressWindow=true, VynxStatusUI=true, MoveeDuels=true, VynxxHubUI=true, }
    local skip = { VynxSpeedBooster=true, }
    ...
    pcall(function()
        if M.statusMain then
            for _, d in ipairs(M.statusMain:GetDescendants()) do
                if d:IsA("TextLabel") and (d.Text == "VYNX DUELS V2.1" or string.find(tostring(d.Text), "VYNX")) then d.TextColor3 = accent end
            end
        end
    end)
    pcall(function() if M.updateHeadTheme then M.updateHeadTheme() end end)
...
do
    ...
    if a then UI_ACCENT = a; CHERRY_ACCENT = a; UI_ACCENT_DIM = M.UI_ACCENT_DIM or a; UI_TEXT_SECTION = a end
...
local function uiNumberRow(parent, label, value, minV, maxV, callback)
    ...
    bx.FocusLost:Connect(function()
        ...
        if n and n >= minV and n <= maxV then
            ...
            saveCherryConfig()
        else bx.Text = tostring(value) end
    end)
    return r, bx
end
...
local function uiExpandToggleRow(parent, label, on, options, defaultIndex, onToggle, onOption)
    ...
    local function refreshOptionVisuals()
        for i, b in ipairs(optionBtns) do styleOptionChip(b, i == idx) end
    end
    ...
    for i, opt in ipairs(options) do
        local b = Instance.new("TextButton")
        b.LayoutOrder = i
        if useScroll then b.Size = UDim2.new(1, -4, 0, 30)
        else b.Size = UDim2.new(0, math.max(56, #tostring(opt) * 9 + 18), 0, 28) end
        b.BorderSizePixel = 0
        b.Text = tostring(opt)
    ...
    local function setOption(v)
        for i, o in ipairs(options) do
            if o == v then idx = i; refreshOptionVisuals(); refreshModeSettings(); break end
        end
    end
...
function M.buildAntiDesyncUI()
    ...
    local function makeToggleRow(y, label, getState, setState)
        ...
        local function refresh()
            ...
            TweenService:Create(btn, TweenInfo.new(0.2), { BackgroundColor3 = on and accent or white }):Play()
    ...
    pillBtn.MouseButton1Click:Connect(function() setOpen(true) end)
...
function M.setAntiDesyncPanelOpen(on)
    ...
    if not M.antiDesyncGui then M.buildAntiDesyncUI() end
    if M.antiDesyncMain then
        M.antiDesyncMain.Visible = M.antiDesyncPanelOpen
        if M.antiDesyncMain:FindFirstChild("Content") then M.antiDesyncMain.Content.Visible = true end
        M.antiDesyncMain.Size = UDim2.new(0, 210, 0, 195)
    end
    if M.antiDesyncPill then M.antiDesyncPill.Visible = false end
...
function M.findPingRemote()
    ...
    for _, name in ipairs({"SetPlayerBlockList","UpdatePlayerBlockList","SetBlockList","UpdateBlockList"}) do
        ...
        if r and (r:IsA("RemoteEvent") or r:IsA("UnreliableRemoteEvent") or r:IsA("RemoteFunction")) then remote = r; break end
    end
    if not remote then
        for _, c in ipairs(rrs:GetChildren()) do
            if (c:IsA("RemoteEvent") or c:IsA("UnreliableRemoteEvent") or c:IsA("RemoteFunction")) and c.Name:find("Block") then remote = c; break end
        end
    end
    return remote
end
...
function M._fontApplyOne(txt)
    ...
    if M._fontMy then pcall(function() txt.FontFace = M._fontMy end) end
end
...
function M.getFontForName(name)
    ...
    if name == "Coding Font" then
        ...
        return nil
    elseif name == "Summer" then return Font.new("rbxasset://fonts/families/PermanentMarker.json")
    elseif name == "Beachy" then return Font.new("rbxasset://fonts/families/DenkOne.json")
    elseif name == "Scary" then return Font.new("rbxasset://fonts/families/Creepster.json")
    elseif name == "Bangers" then return Font.new("rbxasset://fonts/families/Bangers.json") end
    return nil
end

function M.applyCustomFont(name)
    if M._fontConn then pcall(function() M._fontConn:Disconnect() end); M._fontConn = nil end
    for obj, orig in pairs(M._fontOrig) do pcall(function() if obj and obj.Parent then obj.FontFace = orig end end) end
    M._fontOrig = {}
    ...
    if name and name ~= "None" then
        ...
        if font then
            M._fontMy = font
            for _, v in ipairs(game:GetDescendants()) do M._fontApplyOne(v) end
            M._fontConn = game.DescendantAdded:Connect(function(obj)
                if M.customFontSelected ~= "None" then M._fontApplyOne(obj) end
            end)
        end
    else M._fontMy = nil end
    pcall(function() if type(saveCherryConfig) == "function" then saveCherryConfig() end end)
end

function M.resetSpeedBoosterPosition()
    M.speedBoosterPos = nil
    if M.speedBoosterMain then M.speedBoosterMain.Position = UDim2.new(0, 20, 0.35, 0) end
    pcall(saveCherryConfig)
end

function M.buildSpeedBoosterUI()
    if M.speedBoosterGui then pcall(function() M.speedBoosterGui:Destroy() end); M.speedBoosterGui = nil; M.speedBoosterMain = nil end
    ...
    else main.Position = UDim2.new(0, 20, 0.35, 0) end
    ...
    if M.laggerModeEnabled or M.laggerCarryActive then path = "Lagger"; M.speedBoosterPath = "Lagger" end
    ...
    local function refreshBoxes()
        if path == "Lagger" then speedBox.Text = tostring(M.LAGGER_SPEED); stealBox.Text = tostring(M.LAGGER_CARRY_SPEED)
        else speedBox.Text = tostring(M.NS); stealBox.Text = tostring(M.CS) end
    end
    refreshBoxes()
    ...
    local function applyPath(forcePath, fromSync)
        ...
        if not fromSync then M.speedBoosterEnabled = true; if paintStatus then paintStatus() end
        elseif M.speedBoosterEnabled == nil then M.speedBoosterEnabled = true end
        if path == "Lagger" then M.laggerModeEnabled = true
        else M.laggerModeEnabled = false end
        if M.laggerModeBtn then M.laggerModeBtn.Text = M.laggerModeEnabled and "Lag On" or "Lag Off" end
        if M.laggerCarryBtn then M.laggerCarryBtn.Text = M.laggerCarryActive and "L.Carry On" or "L.Carry Off" end
        if M.carryModeBtn then M.carryModeBtn.Text = M.carrySpeedActive and "Carry On" or "Carry Off" end
        if M.mobBtnRefs.lagger then pcall(function() M.mobBtnRefs.lagger(M.laggerModeEnabled) end) end
...
function M.applyCustomBackground(frame)
    ...
    if not id or id <= 0 then id = 0 end
...
function M.applyPingBackgrounds()
    local function applyVisible(frame)
        ...
        pcall(function() frame.BackgroundTransparency = 0.55 end)
    end
...
function M.playIntro()
    if M.introGUIEnabled == false and M.introSoundEnabled == false then return end
    ...
    local function loadFixedIntroMusic()
        local asset
        pcall(function()
            if isfile and isfile(MUSIC_FILE) and getcustomasset then asset = getcustomasset(MUSIC_FILE) end
        end)
        if not asset then
            local ok, data = pcall(function() return game:HttpGet(MUSIC_URL) end)
    ...
    local function playFixedSong()
        ...
        if introSoundInstance then pcall(function() introSoundInstance:Stop() end); pcall(function() introSoundInstance:Destroy() end); introSoundInstance = nil end
    ...
    if M.introGUIEnabled == false then playFixedSong(); return end
    ...
    task.spawn(function() introSound = playFixedSong() end)
    ...
    task.spawn(function()
        ...
        for i = 1, 8 do
            if not introActive then return end
            introTitle.Position = UDim2.fromScale(0.5 + math.random(-6, 6) / 1000, 0.5 + math.random(-6, 6) / 1000)
            task.wait(0.02)
        end
        introTitle.Position = UDim2.fromScale(0.5, 0.5)
...
do
    ...
    if not cur or cur == oldDefault or cur == 0 then M.customBgId = forced end
end

if M.speedUIMode ~= "Customizer" then M.speedUIMode = "Original" end
...
M.introSoundEnabled = true
do
    local okIntro, errIntro = pcall(function() M.playIntro() end)
    if not okIntro then warn("[VYNX] playIntro error: ", errIntro); print("[VYNX] playIntro error: ", errIntro) end
end
M.bodyLockEnabled = false
...
do
    ...
    if tn == "Default" then tn = "Red" elseif tn == "Forest" then tn = "Green" end
    if tn and CHERRY_THEMES[tn] then CherryConfig.Theme = tn; M.colorScheme = tn; M._savedTheme = tn end
end
...
M.menuOpen = true
local okGui, errGui = pcall(function() M.buildGui() end)
if not okGui then warn("[VYNX] buildGui error: ", errGui); print("[VYNX] buildGui error: ", errGui) end

pcall(function()
    if M.mainFrame then M.mainFrame.Visible = true; M.menuOpen = true end
end)
pcall(function()
    if M.mobileButtonsEnabled then M.buildMobileButtons() end
end)
...
pcall(function()
    ...
    M._autoSaveConn = RunService.Heartbeat:Connect(function()
        ...
        pcall(function() if M.mobGuiRef then M.saveBtnPositions() end end)
        pcall(saveCherryConfig)
    end)
end)
if M.antiRagdollEnabled then M.startAntiRagdoll() end
...
player.CharacterAdded:Connect(function(char)
    ...
    if M.autoResetOnDeath then setupDeathReset() end
    if M.bypassAimbotEnabled then task.wait(0.2); M.startBypassAimbot() end
    M.bodyLockEnabled = false
    pcall(function() M.stopBodyLock() end)
    if M.hardHitEnabled then task.wait(0.2); M.hideHardHitRing(); M.showHardHitRing(); if not M._hardHitConn then M.startHardHit() end end
end)
...
local function applySpeedMethod(hrp, hum, dir, spd, dt)
    ...
    if m == "Velocity" then massImpulse(dir, spd)
    elseif m == "AssemblyLinearVelocity" then massImpulse(dir, spd)
    ...
    elseif m == "AssemblyLinearVelocity Lerp" then
        ...
        local mass = hrp.AssemblyMass or 1
        pcall(function() hrp:ApplyImpulse(Vector3.new(blended.X - current.X, 0, blended.Z - current.Z) * mass) end)
    elseif m == "CFrame" then hrp.CFrame = hrp.CFrame + (dir * spd * step)
    elseif m == "CFrame Lerp" then hrp.CFrame = hrp.CFrame:Lerp(hrp.CFrame + (dir * spd * step), 0.5)
    elseif m == "Hyper CFrame" then hrp.CFrame = hrp.CFrame + (dir * spd * (M.hyperMult or 4) * step)
    elseif m == "Anchored CFrame" then
        if not hrp.Anchored then hrp.Anchored = true; M._anchoredBySpeed = hrp end
        hrp.CFrame = hrp.CFrame + (dir * spd * step)
    elseif m == "PivotTo" then hrp:PivotTo(hrp.CFrame + (dir * spd * step))
    elseif m == "Model PivotTo" then
        if char and char:IsA("Model") then char:PivotTo(char:GetPivot() + (dir * spd * step))
        else hrp:PivotTo(hrp.CFrame + (dir * spd * step)) end
    elseif m == "Tween CFrame" then
        ...
        M._speedTween:Play()
    elseif m == "WalkSpeed" then hum.WalkSpeed = spd
    elseif m == "Humanoid Move" then hum.WalkSpeed = spd; hum:Move(dir)
    elseif m == "Humanoid MoveTo" then hum:MoveTo(targetPos, hrp)
...
RunService.RenderStepped:Connect(function(dt)
    ...
    if not M.autoBatEnabled and not M.autoLeftEnabled and not M.autoRightEnabled then
        ...
        if md.Magnitude>0 then M.lastMoveDir=md; dir=md
        ...
        if dir.Magnitude>0 then applySpeedMethod(hrp, hum, dir, spd, dt)
        else destroySpeedObjects() end
    end
end)
    ...
    if hookfunction and newcclosure then
        ...
        oldFire=hookfunction(Instance.new("RemoteEvent").FireServer,newcclosure(function(self,...)
            if typeof(self)=="Instance" and self:IsA("RemoteEvent") and self.Name:sub(1,3)=="RE/" then
                ...
                for i = 1, select("#", ...) do
                    local a = args[i]
                    if a == M.CURSED_RESET_GUID or a == "balloon" or (type(a)=="string" and a:find("f888ee6e", 1, true)) then return end
                end
            end
            return oldFire(self,...)
        end))
...
M.applyFOV()
task.spawn(function()
    while true do task.wait(8); pcall(M.saveBtnPositions) end
end)
...
task.spawn(function()
    ...
    if plots then
        ...
        plots.ChildAdded:Connect(function(plot) if plot:IsA("Model") then task.wait(0.5); scanPlotNormal(plot) end end)
        while true do
            task.wait(10)
...
task.spawn(function()
    ...
    while true do
        task.wait(5)
        if M.Semi.enabled or M.stealMode == "Semi" then pcall(M.scanAllPlotsSemi) end
    end
end)
...
function M.refreshSpeedModeLabel()
    ...
    if M.laggerCarryActive then text = "LAGGER CARRY"
    elseif M.laggerModeEnabled then text = "LAGGER MODE"
    elseif M.carrySpeedActive then text = "CARRY MODE" end
    M.headIndicator.mode.Text = text
    M.headIndicator.mode.TextColor3 = Color3.fromRGB(255, 255, 255)
    if M.headIndicator.speed then M.headIndicator.speed.TextColor3 = Color3.fromRGB(255, 255, 255) end
end
...
if not M._voidSafetyStarted then
    ...
    RunService.Heartbeat:Connect(function(dt)
        ...
        if mag > hardCap then pcall(function() hrp.AssemblyLinearVelocity = v.Unit * math.min(mag, hardCap) end)
        elseif v.Y < -250 then pcall(function() hrp.AssemblyLinearVelocity = Vector3.new(v.X, -80, v.Z) end) end
