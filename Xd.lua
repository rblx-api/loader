...
task.spawn(function()
    ...
    env.__FOLK_HIGH_PING_RUN = (env.__FOLK_HIGH_PING_RUN or 0) + 1
    local thisRun = env.__FOLK_HIGH_PING_RUN
    env.__FOLK_INTRO_FINISHED_RUN = 0
    local shown = false
    ...
    pcall(function() local old = CoreGui:FindFirstChild("FolkHighPingAlert"); if old then old:Destroy() end end)
        pcall(function() local old = playerGui and playerGui:FindFirstChild("FolkHighPingAlert"); if old then old:Destroy() end end)
            local gui = Instance.new("ScreenGui")
            gui.Name = "FolkHighPingAlert"
            ...
            local parented = pcall(function() gui.Parent = CoreGui end)
                ...
                local stroke = Instance.new("UIStroke"); stroke.Color = Color3.fromRGB(200, 200, 200); stroke.Transparency = 0.2; stroke.Thickness = 1; stroke.Parent = bar
            ...
            task.wait(5)
            while env.__FOLK_HIGH_PING_RUN == thisRun and not shown do
        ...
        UI_NAME = "folkvs"
        MOBILE_UI_NAME = "folkvsMobileButtons"
        pcall(function()
        ...
        _G.folkvsRunning = true
        -- ============================================================
        --  BRANDING / THEME (Black & White)
        -- ============================================================
        FOLK_BRAND = "Folk.vs V3"
        FOLK_DISCORD = "Folk.vs"
        FOLK_LOGO_ID = "rbxassetid://102743270820395"  -- Logo ng Folk Hub
        FOLK_COLORS = {
        ...
        DARK_ACCENT = Color3.fromRGB(50,50,50),
...
if tryhardDefMusicSound then pcall(function() tryhardDefMusicSound:Stop();tryhardDefMusicSound:Destroy() end);tryhardDefMusicSound=nil end
    ...
    snd.Volume=0.7
    snd.Looped=true
    snd.Name="folkvsTryhardDefMusic"
    snd.Parent=SoundService
    pcall(function() snd:Play() end)
...
-- ============================================================
--  FEATURE BACKEND (blacklist kept, but renamed internal)
-- ============================================================
task.spawn(function()
        ...
        function() return game:HttpGet(url) end,
            function() return HS:GetAsync(url) end,
                function() return syn.request({Url=url,Method="GET"}).Body end,
                    ...
                    -- ============================================================
                    --  REMOTE DETECTION FOR RESET REMOVED (cursedResetRemote no longer used)
                    -- ============================================================
                    -- Bless Lagger backend (unchanged except naming)
                    BLESS_LAGGER_CFG = {
                    x=2.5,
                    y=1,
...
local targetPct,targetColor,status=0,FOLK_COLORS.ACCENT,CONFIG.AUTO_STEAL_ENABLED and "READY" or "IDLE"
if StealState.active then
    ...
    if StealState.phase=="waitingRange" then
        ...
        targetColor=FOLK_COLORS.ICE
    else
        status="STEALING"
        targetColor=FOLK_COLORS.ACCENT
    end
elseif recent then
...
local old=head:FindFirstChild(player==LP and "folkvsSpeedBB" or "folkvsOtherSpeedBB");if old then old:Destroy() end
local bb=Instance.new("BillboardGui",head)
bb.Name=player==LP and "folkvsSpeedBB" or "folkvsOtherSpeedBB"
...
if player==LP then
    ...
    tag.Text="Folk.vs V3";tag.TextColor3=FOLK_COLORS.ICE
...
val.Text=player==LP and "Speed: 0.0" or "0";val.TextColor3=player==LP and FOLK_COLORS.ACCENT or FOLK_COLORS.ICE
...
espObjects = {}
setEspVisual = nil
ESP_COLOR = Color3.fromRGB(200,200,200) -- white/gray
...
do
    ...
    for _, hl in pairs(_hlCache) do pcall(function() hl:Destroy() end) end; _hlCache = {}
        for _, bb in pairs(_bbCache) do pcall(function() bb:Destroy() end) end; _bbCache = {}
            ...
            bb.Name="FolkESPBB"; bb.Size=UDim2.new(0,110,0,18); bb.StudsOffset=Vector3.new(0,3.2,0)
                                ...
                                if tRoot and tHead and tHum and tHum.Health>0 then
                                    ...
                                    if not hl or not hl.Parent or hl.Parent~=char then
                                        if hl then pcall(function() hl:Destroy() end) end
                                            hl=Instance.new("Highlight"); hl.Name="FolkESP"; hl.FillColor=ESP_COLOR; hl.FillTransparency=0.62
...
-- Variables auxiliares del bat aimbot
BAT_SLAP_LIST = {
...
if bp then
            ...
            if plr.Character then task.spawn(function() setupSpeedIndicator(plr.Character,plr) end) end
                plr.CharacterAdded:Connect(function(char) task.wait(0.5);setupSpeedIndicator(char,plr) end)
                    ...
                    Players.PlayerRemoving:Connect(function(plr) otherSpeedLabels[plr]=nil end)
                        CANDY_SKY_TAG = "folkvsSkyTheme"
                        _G._folkvsSkyMode = _G._folkvsSkyMode or "Off"
                        candyOriginalLighting = nil
                        -- Sky presets unchanged
                        CANDY_SKY_PRESETS = {
                        ["Off"] = {kind = "off"},
                    ...
                    autoBatKey=ks(KB.AutoBat),laggerToggleKey=ks(KB.LaggerToggle),laggerCarryToggleKey=ks(KB.LaggerCarryToggle),blessLaggerKey=ks(KB.BlessLagger),tpFloorKey=ks(KB.TPFloor),guiHideKey=ks(KB.GuiHide),batV2Key=ks(KB.BatV2),tpBatKey=ks(KB.TpBat),
                    ...
                    if writefile then pcall(function() writefile("folkvs.json",HS:JSONEncode(cfg)) end) end
                    end
    ...
    -- ============================================================
    --  INTRO ANIMATION (renamed strings, logo added)
    -- ============================================================
    ...
    pcall(function()
        for _, n in ipairs({"VioletGifIntroVertical","RubyGifIntroVertical","FolkVSGifIntroVertical","IrishGifIntroVertical","SoulHubIntro","FolkVSIntro"}) do
            local old = parent:FindFirstChild(n); if old then old:Destroy() end
        end
    end)
    local introGui = Instance.new("ScreenGui")
    introGui.Name = "FolkVSIntro"
    ...
    skipStroke.Color = Color3.fromRGB(200,200,200); skipStroke.Thickness = 1; skipStroke.Transparency = 0.2
    local particles = {}
    ...
    for i = 1, 32 do
        ...
        local gray = math.random(100,220)
        p.BackgroundColor3 = Color3.fromRGB(gray, gray, gray)
        p.BackgroundTransparency = 1
        p.BorderSizePixel = 0
        p.ZIndex = 3 + i
        Instance.new("UICorner", p).CornerRadius = UDim.new(1,0)
        local stroke = Instance.new("UIStroke", p)
        stroke.Color = Color3.fromRGB(200,200,200); stroke.Thickness = 1; stroke.Transparency = 1
    ...
    local introLogo = Instance.new("ImageLabel", center)
    introLogo.Size = UDim2.new(0, 80, 0, 80)
    introLogo.Position = UDim2.new(0.5, -40, 0, -20)
    introLogo.BackgroundTransparency = 1
    introLogo.Image = FOLK_LOGO_ID
    introLogo.ScaleType = Enum.ScaleType.Crop
    introLogo.ZIndex = 44
    local lineTop = Instance.new("Frame", center)
    lineTop.AnchorPoint = Vector2.new(0.5,0); lineTop.Position = UDim2.new(0.5,0,0,68); lineTop.Size = UDim2.new(0,0,0,2)
    lineTop.BackgroundColor3 = Color3.fromRGB(200,200,200); lineTop.BorderSizePixel = 0; lineTop.ZIndex = 41
    ...
    lineBot.BackgroundColor3 = Color3.fromRGB(200,200,200); lineBot.BorderSizePixel = 0; lineBot.ZIndex = 41
    local titleShadow = Instance.new("TextLabel", center)
    titleShadow.Size = UDim2.new(1,0,0,86); titleShadow.Position = UDim2.new(0,4,0,98); titleShadow.BackgroundTransparency = 1
    titleShadow.Text = "Folk.vs V3"; titleShadow.TextColor3 = Color3.fromRGB(0,0,0); titleShadow.Font = Enum.Font.GothamBlack; titleShadow.TextSize = 72
    ...
    title.Size = UDim2.new(1,0,0,86); title.Position = UDim2.new(0,0,0,93); title.BackgroundTransparency = 1
    title.Text = "Folk.vs V3"; title.TextColor3 = Color3.fromRGB(200,200,200); title.Font = Enum.Font.GothamBlack; title.TextSize = 72
    title.TextTransparency = 1; title.TextStrokeTransparency = 1; title.TextStrokeColor3 = Color3.fromRGB(80,80,80); title.ZIndex = 43
    local subtitle = Instance.new("TextLabel", center)
    subtitle.Size = UDim2.new(1,0,0,26); subtitle.Position = UDim2.new(0,0,0,192); subtitle.BackgroundTransparency = 1
    subtitle.Text = "Migizin"; subtitle.TextColor3 = Color3.fromRGB(200,200,200); subtitle.Font = Enum.Font.GothamMedium; subtitle.TextSize = 19; subtitle.TextTransparency = 1; subtitle.ZIndex = 43
...
task.spawn(function()
    pcall(function()
        writefile("FolkVSIntro", game:HttpGet(INTRO_MUSIC_URL))
        ...
        pcall(function() if gethui then soundParent = gethui() end end)
            ...
            snd.SoundId = getcustomasset("FolkVSIntro")
        ...
        task.wait(0.85); if not introActive then pcall(function() driftConn:Disconnect() end); finishIntro(); return end
            ...
            TS:Create(introLogo, TweenInfo.new(0.5), {ImageTransparency = 0}):Play()
            task.wait(0.42)
            ...
            for i = 1, 3 do
                ...
                TS:Create(title, TweenInfo.new(0.06), {TextColor3 = Color3.fromRGB(200,200,200)}):Play(); task.wait(0.06)
            end
            task.wait(3.0); if not introActive then pcall(function() driftConn:Disconnect() end); return end
                ...
                TS:Create(introLogo, TweenInfo.new(0.32), {ImageTransparency = 1}):Play()
            ...
            -- ============================================================
            --  ANIMATION PACKS DATA (unchanged)
            -- ============================================================
            ANIM_PACKS = {
...
-- ============================================================
--  GUI BUILDER (with logo as background image - visible)
-- ============================================================
function buildGui()
local BG=FOLK_COLORS.BG
local PANEL=FOLK_COLORS.PANEL
local CARD=FOLK_COLORS.CARD
local ACCENT=FOLK_COLORS.ACCENT
local ICE=FOLK_COLORS.ICE
local HOVER=FOLK_COLORS.HOVER
local W=FOLK_COLORS.TEXT
local DIM=FOLK_COLORS.SECONDARY
local STROKE=FOLK_COLORS.STROKE
local INP=FOLK_COLORS.INPUT
local OFF=FOLK_COLORS.OFF
...
local old=game:GetService("CoreGui"):FindFirstChild("folkvs");if old then old:Destroy() end
local pg=LP:FindFirstChild("PlayerGui");if pg then local o=pg:FindFirstChild("folkvs");if o then o:Destroy() end end
local gui=Instance.new("ScreenGui")
gui.Name="folkvs";gui.ResetOnSpawn=false;gui.DisplayOrder=10;gui.IgnoreGuiInset=true
pcall(function() if syn and syn.protect_gui then syn.protect_gui(gui) end end)
    if not pcall(function() gui.Parent=game:GetService("CoreGui") end) then gui.Parent=LP:WaitForChild("PlayerGui") end
        ...
        title.Size=UDim2.new(0,104,1,0);title.Position=UDim2.new(0,26,0,0);title.BackgroundTransparency=1;title.Text="Folk Lagger";title.TextColor3=W;title.Font=Enum.Font.GothamBlack;title.TextSize=12;title.TextXAlignment=Enum.TextXAlignment.Left;title.ZIndex=63
        local disc=Instance.new("TextLabel",header)
        disc.Size=UDim2.new(0,104,1,0);disc.Position=UDim2.new(1,-138,0,0);disc.BackgroundTransparency=1;disc.Text=FOLK_DISCORD;disc.TextColor3=ICE;disc.Font=Enum.Font.Gotham;disc.TextSize=9;disc.TextXAlignment=Enum.TextXAlignment.Right;disc.ZIndex=63
    ...
    -- LOGO AS BACKGROUND IMAGE (visible)
    local bgLogo = Instance.new("ImageLabel", main)
    bgLogo.Size = UDim2.new(1, 0, 1, 0)
    bgLogo.Position = UDim2.new(0, 0, 0, 0)
    bgLogo.BackgroundTransparency = 1
    bgLogo.Image = FOLK_LOGO_ID
    bgLogo.ScaleType = Enum.ScaleType.Crop
    bgLogo.ImageTransparency = 0.15   -- 85% opacity, visible
    bgLogo.ZIndex = 1
    -- Lighter overlay para makita ang background
    local bgOverlay=Instance.new("Frame",main)
    bgOverlay.Size=UDim2.new(1,0,1,0);bgOverlay.BackgroundColor3=Color3.fromRGB(8,8,8)
    bgOverlay.BackgroundTransparency=0.25;bgOverlay.BorderSizePixel=0;bgOverlay.ZIndex=2
    ...
    local panel=Instance.new("Frame",gui)
    panel.Name="FolkAnimationsPanel"
    panel.Size=UDim2.new(0,PANEL_W,0,PANEL_H)
    ...
    for i,name in ipairs(sortedPacks) do
        ...
        btn.MouseEnter:Connect(function() TS:Create(btn,TweenInfo.new(0.08),{BackgroundColor3=Color3.fromRGB(20,20,28)}):Play();TS:Create(bstroke,TweenInfo.new(0.08),{Color=ACCENT,Transparency=0.2}):Play() end)
            ...
            hdr.Size=UDim2.new(1,0,0,HDR_H);hdr.BackgroundColor3=Color3.fromRGB(8,8,8);hdr.BackgroundTransparency=0.15;hdr.BorderSizePixel=0;hdr.ZIndex=3
            Instance.new("UICorner",hdr).CornerRadius=UDim.new(0,20)
            local hdrGrad=Instance.new("UIGradient",hdr)
            hdrGrad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(30,30,30)),ColorSequenceKeypoint.new(0.5,Color3.fromRGB(18,18,18)),ColorSequenceKeypoint.new(1,Color3.fromRGB(8,8,8))})
            ...
            task.spawn(function() while hdrStrokeGrad and hdrStrokeGrad.Parent do hdrStrokeGrad.Rotation=(hdrStrokeGrad.Rotation+1.5)%360;task.wait(0.04) end end)
                -- Logo sa header (maliit)
                local logo=Instance.new("ImageLabel",hdr)
                logo.Size=UDim2.new(0,LOGO,0,LOGO);logo.Position=UDim2.new(0,12,0,IsMobile and 11 or 12);logo.BackgroundColor3=Color3.fromRGB(20,20,20);logo.BorderSizePixel=0;logo.ZIndex=3
                logo.Image=FOLK_LOGO_ID;logo.ImageColor3=Color3.fromRGB(200,200,200);logo.ScaleType=Enum.ScaleType.Crop
                ...
                ttl.BackgroundTransparency=1;ttl.Text=FOLK_BRAND;ttl.TextColor3=ACCENT;ttl.Font=Enum.Font.GothamBlack;ttl.TextSize=IsMobile and 15 or 18;ttl.TextXAlignment=Enum.TextXAlignment.Left;ttl.ZIndex=3
                ttl.TextStrokeColor3=Color3.fromRGB(50,50,50);ttl.TextStrokeTransparency=0.5
                ...
                sub.BackgroundTransparency=1;sub.Text=FOLK_DISCORD;sub.TextColor3=ICE;sub.Font=Enum.Font.GothamMedium;sub.TextSize=IsMobile and 9 or 10;sub.TextXAlignment=Enum.TextXAlignment.Left;sub.ZIndex=3
                -- User chip
                ...
                userDot.BackgroundColor3=Color3.fromRGB(200,200,200)
                ...
                closeBtn.Size=UDim2.new(0,IsMobile and 26 or 30,0,IsMobile and 26 or 30);closeBtn.Position=UDim2.new(1,IsMobile and -34 or -42,0,IsMobile and 13 or 14);closeBtn.BackgroundColor3=Color3.fromRGB(60,60,60);closeBtn.BorderSizePixel=0;closeBtn.ZIndex=4
                closeBtn.Text="✕";closeBtn.TextColor3=Color3.fromRGB(200,200,200);closeBtn.Font=Enum.Font.GothamBlack;closeBtn.TextSize=IsMobile and 14 or 16
                ...
                lockBtn.Size=UDim2.new(0,IsMobile and 38 or 46,0,IsMobile and 26 or 30);lockBtn.Position=UDim2.new(1,IsMobile and -78 or -98,0,IsMobile and 12 or 14);lockBtn.BackgroundColor3=Color3.fromRGB(28,28,28);lockBtn.BorderSizePixel=0;lockBtn.ZIndex=4
                lockBtn.Text="🔓 LOCK";lockBtn.TextColor3=Color3.fromRGB(200,200,200);lockBtn.Font=Enum.Font.GothamBlack;lockBtn.TextSize=IsMobile and 9 or 10;lockBtn.AutoButtonColor=false
            ...
            setTopLockVisual=function(on)
                lockBtn.BackgroundColor3=on and ACCENT or Color3.fromRGB(28,28,28)
                lockBtn.TextColor3=on and Color3.fromRGB(0,0,0) or Color3.fromRGB(200,200,200)
            ...
            lockBtn.Activated:Connect(function() setGuiLock(not uiLocked) end)
                ...
                miniBtn.Text="☠ Folk.vs V3";miniBtn.TextColor3=ACCENT;miniBtn.Font=Enum.Font.GothamBlack;miniBtn.TextSize=12;miniBtn.ZIndex=20;miniBtn.Visible=false
                ...
                drag(miniBtn,function(active,moved) if moved then miniDragged=true end;if not active and moved then task.delay(0.12,function() miniDragged=false end) end end)
                    ...
                    showCandyGui=showGui;hideCandyGui=hideGui;isCandyGuiVisible=function() return main.Visible end
                        closeBtn.MouseButton1Click:Connect(hideGui);miniBtn.Activated:Connect(function() if miniDragged then return end;showGui() end)
                            -- ============================================================
                            --  AUTO STEAL — STATUS POD (may logo bilang background)
                            -- ============================================================
                            ...
                            -- Logo bilang background sa steal bar (visible)
                            local stealBgLogo = Instance.new("ImageLabel", pbFrame)
                            stealBgLogo.Size = UDim2.new(1, 0, 1, 0)
                            stealBgLogo.Position = UDim2.new(0, 0, 0, 0)
                            stealBgLogo.BackgroundTransparency = 1
                            stealBgLogo.Image = FOLK_LOGO_ID
                            stealBgLogo.ScaleType = Enum.ScaleType.Crop
                            stealBgLogo.ImageTransparency = 0.2   -- 80% opacity
                            stealBgLogo.ZIndex = 1
                            ...
                            local pbg=Instance.new("Frame",pbFrame);pbg.Size=UDim2.new(1,-40,0,9);pbg.Position=UDim2.new(0,20,1,-15)
                            pbg.BackgroundColor3=Color3.fromRGB(8,10,18);pbg.BorderSizePixel=0
                        ...
                        local base=toggleRow and Color3.fromRGB(20,20,20) or Color3.fromRGB(16,16,16)
                        local hover=toggleRow and Color3.fromRGB(35,35,35) or Color3.fromRGB(28,28,28)
                                                                                                                                                                                                ...
                                                                                                                                                                                                setBrasilVisual=mkToggle(mainPage,"brasil",function(on) setBrasil(on) end)
                                                                                                                                                                                                    setBrasil2Visual=mkToggle(mainPage,"brasil",function(on) setBrasil2(on) end)
                                                                                                                                                                                                        do
                                                                                                                                                                                                            ...
                                                                                                                                                                                                            local skyIndex=1
                                                                                                                                                                                                            local current=tostring(currentSkyTheme or _G._folkvsSkyMode or CandySkyOrder[1][2])
                                                                                                                                                                                                        ...
                                                                                                                                                                                                        setLockGuiVisual=mkToggle(mainPage,"Lock GUI",function(on) setGuiLock(on) end)
                                                                                                                                                                                                            setHideMobileVisual=mkToggle(mainPage,"Hide Mobile Buttons",function(on) hideMobileButtons=on;refreshMobileButtonUi();saveConfig() end)
                                                                                                                                                                                                                setEditMobileVisual=mkToggle(mainPage,"Edit Mobile Buttons",function(on) editMobileButtons=on;refreshMobileButtonUi();saveConfig() end)
                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                            do local row=mkRow(mainPage,ROW_H);mkLabel(row,"Reset Mobile Buttons");local btn=Instance.new("TextButton",row);btn.Size=UDim2.new(0,86,0,22);btn.Position=UDim2.new(1,-94,0.5,-11);btn.BackgroundColor3=ACCENT;btn.BorderSizePixel=0;btn.Text="RESET";btn.TextColor3=Color3.fromRGB(0,0,0);btn.Font=Enum.Font.GothamBlack;btn.TextSize=10;btn.AutoButtonColor=false;btn.ZIndex=3;Instance.new("UICorner",btn).CornerRadius=UDim.new(1,0);local rst=Instance.new("UIStroke",btn);rst.Color=ACCENT;rst.Thickness=1;rst.Transparency=0.34;btn.MouseEnter:Connect(function() TS:Create(rst,TweenInfo.new(0.12),{Color=ICE,Transparency=0.02}):Play() end);btn.MouseLeave:Connect(function() TS:Create(rst,TweenInfo.new(0.12),{Color=ACCENT,Transparency=0.34}):Play() end);btn.Activated:Connect(function() if _anyKeyListening then return end;resetMobileButtonLayout();saveConfig() end) end
                                                                                                                                                                                                                                do local row=mkRow(mainPage,ROW_H);mkLabel(row,"Hide UI Key");mkKB(row,KB.GuiHide,function(k,isGp) if isGp then KB.GuiHide.gp=k;KB.GuiHide.kb=nil else KB.GuiHide.kb=k;KB.GuiHide.gp=nil end;saveConfig() end) end
                                                                                                                                                                                                                                    do local row=mkRow(mainPage,ROW_H);mkLabel(row,"Folk Lagger");local btn=Instance.new("TextButton",row);btn.Size=UDim2.new(0,86,0,22);btn.Position=UDim2.new(1,-94,0.5,-11);btn.BackgroundColor3=ACCENT;btn.BorderSizePixel=0;btn.Text="OPEN";btn.TextColor3=Color3.fromRGB(0,0,0);btn.Font=Enum.Font.GothamBlack;btn.TextSize=10;btn.AutoButtonColor=false;btn.ZIndex=3;Instance.new("UICorner",btn).CornerRadius=UDim.new(1,0);local opn=Instance.new("UIStroke",btn);opn.Color=ICE;opn.Thickness=1;opn.Transparency=0.34;btn.MouseEnter:Connect(function() TS:Create(opn,TweenInfo.new(0.12),{Transparency=0.02}):Play() end);btn.MouseLeave:Connect(function() TS:Create(opn,TweenInfo.new(0.12),{Transparency=0.34}):Play() end);btn.Activated:Connect(function() if _anyKeyListening then return end;if setBlessLaggerPanelVisible then setBlessLaggerPanelVisible(true) end end) end
                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                        -- ============================================================
                                                                                                                                                                                                                                        --  MOBILE BUTTONS (SQUIRCLE logo background - visible)
                                                                                                                                                                                                                                        -- ============================================================
                                                                                                                                                                                                                                        function buildMobileButtons()
                                                                                                                                                                                                                                        local old=game:GetService("CoreGui"):FindFirstChild("folkvsMobileButtons");if old then old:Destroy() end
                                                                                                                                                                                                                                        local pg=LP:FindFirstChild("PlayerGui");if pg then local o=pg:FindFirstChild("folkvsMobileButtons");if o then o:Destroy() end end
                                                                                                                                                                                                                                        local screenGui2 = Instance.new("ScreenGui")
                                                                                                                                                                                                                                        screenGui2.Name = "folkvsMobileButtons"
                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                        pcall(function() screenGui2.Parent = game:GetService("CoreGui") end)
                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                            mobileButtonFrames = {}
                                                                                                                                                                                                                                            local MOBILE_BTN_BG     = Color3.fromRGB(18,18,18)
                                                                                                                                                                                                                                            local MOBILE_BTN_ACTIVE = Color3.fromRGB(200,200,200)
                                                                                                                                                                                                                                            local LIGHT_PINK        = Color3.fromRGB(220,220,220)
                                                                                                                                                                                                                                            local BLACK_PURE        = Color3.fromRGB(10,10,10)
                                                                                                                                                                                                                                            local PURPLE_ACCENT     = Color3.fromRGB(200,200,200)
                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                -- SQUIRCLE logo: kasinlaki ng button, may rounded corner (squircle)
                                                                                                                                                                                                                                local btnLogoBg = Instance.new("ImageLabel", btnFrame)
                                                                                                                                                                                                                                btnLogoBg.Size = UDim2.new(1, 0, 1, 0)  -- same size as button
                                                                                                                                                                                                                                btnLogoBg.Position = UDim2.new(0, 0, 0, 0)
                                                                                                                                                                                                                                btnLogoBg.BackgroundTransparency = 1
                                                                                                                                                                                                                                btnLogoBg.Image = FOLK_LOGO_ID
                                                                                                                                                                                                                                btnLogoBg.ScaleType = Enum.ScaleType.Crop
                                                                                                                                                                                                                                btnLogoBg.ImageTransparency = 0.1   -- 90% opacity, very visible
                                                                                                                                                                                                                                btnLogoBg.ZIndex = 1
                                                                                                                                                                                                                                -- Squircle effect: same corner radius as button (14)
                                                                                                                                                                                                                                local logoCorner = Instance.new("UICorner", btnLogoBg)
                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                    -- Mobile actions (removed InstaReset)
                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                    local configFile=(isfile and isfile("folkvs.json")) and "folkvs.json" or "cursedPC.json"
                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                    do
                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                        pcall(function() if State.BatAimbot and stopBatAimbot then stopBatAimbot() end end)
                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                            if autoCarryConn then return end
                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                if target and target.Character then
                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                    if tRoot then
                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                        if flatDir.Magnitude > 0.5 then
                                                                                                                                                                                                                                            local spd = getActiveMoveSpeed()
                                                                                                                                                                                                                                            hrp.Velocity = Vector3.new(flatDir.Unit.X * spd, hrp.Velocity.Y, flatDir.Unit.Z * spd)
                                                                                                                                                                                                                                        end
                                                                                                                                                                                                                                    end
                                                                                                                                                                                                                                end
                                                                                                                                                                                                                            end)
